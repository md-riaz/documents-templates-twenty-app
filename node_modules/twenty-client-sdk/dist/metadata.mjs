import { n as e } from "./chunk-Dqa2HsxW.mjs";
//#region src/metadata/generated/runtime/error.ts
var t = class extends Error {
	constructor(e, t) {
		let n = Array.isArray(e) ? e.map((e) => e?.message || "").join("\n") : "";
		n ||= "GraphQL error", super(n), this.errors = [], this.errors = e, this.data = t;
	}
};
//#endregion
//#region src/metadata/generated/runtime/batcher.ts
function n(e, n) {
	let r = n.map((e) => e.request);
	r.length === 1 && (r = r[0]), e.fetcher(r).then((e) => {
		if (n.length === 1 && !Array.isArray(e)) {
			if (e.errors && e.errors.length) {
				n[0].reject(new t(e.errors, e.data));
				return;
			}
			n[0].resolve(e);
			return;
		} else if (e.length !== n.length) throw Error("response length did not match query length");
		for (let r = 0; r < n.length; r++) e[r].errors && e[r].errors.length ? n[r].reject(new t(e[r].errors, e[r].data)) : n[r].resolve(e[r]);
	}).catch((e) => {
		for (let t of n) t.reject(e);
	});
}
function r(e, t) {
	let r = e._queue, i = t.maxBatchSize || 0;
	if (e._queue = [], i > 0 && i < r.length) for (let t = 0; t < r.length / i; t++) n(e, r.slice(t * i, (t + 1) * i));
	else n(e, r);
}
var i = class e {
	constructor(e, { batchInterval: t = 6, shouldBatch: n = !0, maxBatchSize: r = 0 } = {}) {
		this.fetcher = e, this._options = {
			batchInterval: t,
			shouldBatch: n,
			maxBatchSize: r
		}, this._queue = [];
	}
	fetch(e, t, n, i = {}) {
		let a = { query: e }, o = Object.assign({}, this._options, i);
		return t && (a.variables = t), n && (a.operationName = n), new Promise((e, t) => {
			this._queue.push({
				request: a,
				resolve: e,
				reject: t
			}), this._queue.length === 1 && (o.shouldBatch ? setTimeout(() => r(this, o), o.batchInterval) : r(this, o));
		});
	}
	forceFetch(t, n, i, a = {}) {
		let o = { query: t }, s = Object.assign({}, this._options, a, { shouldBatch: !1 });
		return n && (o.variables = n), i && (o.operationName = i), new Promise((t, n) => {
			let i = new e(this.fetcher, this._options);
			i._queue = [{
				request: o,
				resolve: t,
				reject: n
			}], r(i, s);
		});
	}
}, a = {
	maxBatchSize: 10,
	batchInterval: 40
}, o = ({ url: e, headers: n = {}, fetcher: r, fetch: o, batch: s = !1, ...c }) => {
	if (!e && !r) throw Error("url or fetcher is required");
	if (r ||= async (t) => {
		let r = typeof n == "function" ? await n() : n;
		if (r ||= {}, typeof fetch > "u" && !o) throw Error("Global `fetch` function is not available, pass a fetch polyfill to Genql `createClient`");
		let i = await (o || fetch)(e, {
			headers: {
				"Content-Type": "application/json",
				...r
			},
			method: "POST",
			body: JSON.stringify(t),
			...c
		});
		if (!i.ok) throw Error(`${i.statusText}: ${await i.text()}`);
		return await i.json();
	}, !s) return async (e) => {
		let n = await r(e);
		if (Array.isArray(n)) return n.map((e) => {
			if (e?.errors?.length) throw new t(e.errors || [], e.data);
			return e.data;
		});
		if (n?.errors?.length) throw new t(n.errors || [], n.data);
		return n.data;
	};
	let l = new i(async (e) => await r(e), s === !0 ? a : s);
	return async ({ query: e, variables: t }) => {
		let n = await l.fetch(e, t);
		if (n?.data) return n.data;
		throw Error("Genql batch fetcher returned unexpected result " + JSON.stringify(n));
	};
}, s = (e, t, n) => {
	if (typeof e == "object" && "__args" in e) {
		let r = e.__args, i = { ...e };
		delete i.__args;
		let a = Object.keys(r);
		if (a.length === 0) return s(i, t, n);
		let o = l(t.root, n);
		return `(${a.map((e) => {
			t.varCounter++;
			let i = `v${t.varCounter}`, a = o.args && o.args[e];
			if (!a) throw Error(`no typing defined for argument \`${e}\` in path \`${n.join(".")}\``);
			return t.variables[i] = {
				value: r[e],
				typing: a
			}, `${e}:$${i}`;
		})})${s(i, t, n)}`;
	} else if (typeof e == "object" && Object.keys(e).length > 0) {
		let r = e, i = Object.keys(r).filter((e) => !!r[e]);
		if (i.length === 0) throw Error(`field selection should not be empty: ${n.join(".")}`);
		let a = n.length > 0 ? l(t.root, n).type : t.root, o = a.scalar, c;
		if (i.includes("__scalar")) {
			let e = new Set(Object.keys(r).filter((e) => !r[e]));
			o?.length && (t.fragmentCounter++, c = `f${t.fragmentCounter}`, t.fragments.push(`fragment ${c} on ${a.name}{${o.filter((t) => !e.has(t)).join(",")}}`));
		}
		return `{${i.filter((e) => !["__scalar", "__name"].includes(e)).map((e) => {
			let i = s(r[e], t, [...n, e]);
			if (e.startsWith("on_")) {
				t.fragmentCounter++;
				let n = `f${t.fragmentCounter}`, r = e.match(/^on_(.+)/);
				if (!r || !r[1]) throw Error("match failed");
				return t.fragments.push(`fragment ${n} on ${r[1]}${i}`), `...${n}`;
			} else return `${e}${i}`;
		}).concat(c ? [`...${c}`] : []).join(",")}}`;
	} else return "";
}, c = (e, t, n) => {
	let r = {
		root: t,
		varCounter: 0,
		variables: {},
		fragmentCounter: 0,
		fragments: []
	}, i = s(n, r, []), a = Object.keys(r.variables), o = a.length > 0 ? `(${a.map((e) => `$${e}:${r.variables[e].typing[1]}`)})` : "", c = n?.__name || "";
	return {
		query: [`${e} ${c}${o}${i}`, ...r.fragments].join(","),
		variables: Object.keys(r.variables).reduce((e, t) => (e[t] = r.variables[t].value, e), {}),
		...c ? { operationName: c.toString() } : {}
	};
}, l = (e, t) => {
	let n;
	if (!e) throw Error("root type is not provided");
	if (t.length === 0) throw Error("path is empty");
	return t.forEach((t) => {
		let r = n ? n.type : e;
		if (!r.fields) throw Error(`type \`${r.name}\` does not have fields`);
		let i = Object.keys(r.fields).filter((e) => e.startsWith("on_")).reduce((e, t) => {
			let n = r.fields && r.fields[t];
			return n && e.push(n.type), e;
		}, [r]), a = null;
		if (i.forEach((e) => {
			let n = e.fields && e.fields[t];
			n && (a = n);
		}), !a) throw Error(`type \`${r.name}\` does not have a field \`${t}\``);
		n = a;
	}), n;
}, ee = ({ queryRoot: e, mutationRoot: t, subscriptionRoot: n, ...r }) => {
	let i = o(r), a = {};
	return e && (a.query = (t) => {
		if (!e) throw Error("queryRoot argument is missing");
		return i(c("query", e, t));
	}), t && (a.mutation = (e) => {
		if (!t) throw Error("mutationRoot argument is missing");
		return i(c("mutation", t, e));
	}), a;
}, te = (e) => {
	let t = Object.assign({}, ...Object.keys(e.types).map((e, t) => ({ [t]: e })));
	return ne(Object.assign({}, ...Object.keys(e.types || {}).map((n) => {
		let r = e.types[n] || {};
		return { [n]: {
			name: n,
			scalar: Object.keys(r).filter((t) => {
				let [n] = r[t] || [];
				return n && e.scalars.includes(n);
			}),
			fields: Object.assign({}, ...Object.keys(r).map((e) => {
				let [n, i] = r[e] || [];
				return n == null ? {} : { [e]: {
					type: t[n],
					args: Object.assign({}, ...Object.keys(i || {}).map((e) => {
						if (!i || !i[e]) return;
						let [n, r] = i[e];
						return { [e]: [t[n], r || t[n]] };
					}))
				} };
			}))
		} };
	})));
}, ne = (e) => (Object.keys(e).forEach((t) => {
	let n = e[t];
	if (!n.fields) return;
	let r = n.fields;
	Object.keys(r).forEach((t) => {
		let n = r[t];
		if (n.args) {
			let t = n.args;
			Object.keys(t).forEach((n) => {
				let r = t[n];
				if (r) {
					let [t] = r;
					typeof t == "string" && (e[t] || (e[t] = { name: t }), r[0] = e[t]);
				}
			});
		}
		let i = n.type;
		typeof i == "string" && (e[i] || (e[i] = { name: i }), n.type = e[i]);
	});
}), e), re = {
	scalars: [
		1,
		3,
		4,
		6,
		8,
		11,
		12,
		14,
		15,
		18,
		21,
		22,
		23,
		24,
		36,
		37,
		42,
		45,
		48,
		50,
		61,
		63,
		65,
		68,
		71,
		72,
		73,
		74,
		75,
		77,
		80,
		81,
		86,
		89,
		94,
		95,
		98,
		99,
		101,
		104,
		105,
		111,
		125,
		135,
		136,
		137,
		139,
		148,
		161,
		164,
		166,
		170,
		172,
		177,
		178,
		185,
		188,
		191,
		204,
		219,
		231,
		237,
		273,
		275,
		276,
		277,
		278,
		279,
		280,
		281,
		288,
		326,
		327,
		328,
		329,
		331,
		333,
		344,
		351,
		358,
		481,
		486,
		494
	],
	types: {
		BillingProductDTO: {
			name: [1],
			description: [1],
			images: [1],
			metadata: [134],
			on_BillingLicensedProduct: [143],
			on_BillingMeteredProduct: [144],
			__typename: [1]
		},
		String: {},
		ApiKey: {
			id: [3],
			name: [1],
			expiresAt: [4],
			revokedAt: [4],
			createdAt: [4],
			updatedAt: [4],
			role: [29],
			__typename: [1]
		},
		UUID: {},
		DateTime: {},
		ApplicationRegistrationVariable: {
			id: [3],
			key: [1],
			description: [1],
			isSecret: [6],
			isRequired: [6],
			isFilled: [6],
			createdAt: [4],
			updatedAt: [4],
			__typename: [1]
		},
		Boolean: {},
		ApplicationRegistration: {
			id: [3],
			universalIdentifier: [1],
			name: [1],
			oAuthClientId: [1],
			oAuthRedirectUris: [1],
			oAuthScopes: [1],
			ownerWorkspaceId: [3],
			sourceType: [8],
			sourcePackage: [1],
			latestAvailableVersion: [1],
			isListed: [6],
			isFeatured: [6],
			isPreInstalled: [6],
			logoUrl: [1],
			createdAt: [4],
			updatedAt: [4],
			isConfigured: [6],
			__typename: [1]
		},
		ApplicationRegistrationSourceType: {},
		TwoFactorAuthenticationMethodSummary: {
			twoFactorAuthenticationMethodId: [3],
			status: [1],
			strategy: [1],
			__typename: [1]
		},
		RowLevelPermissionPredicateGroup: {
			id: [1],
			parentRowLevelPermissionPredicateGroupId: [1],
			logicalOperator: [12],
			positionInRowLevelPermissionPredicateGroup: [11],
			roleId: [1],
			objectMetadataId: [1],
			__typename: [1]
		},
		Float: {},
		RowLevelPermissionPredicateGroupLogicalOperator: {},
		RowLevelPermissionPredicate: {
			id: [1],
			fieldMetadataId: [1],
			objectMetadataId: [1],
			operand: [14],
			subFieldName: [1],
			workspaceMemberFieldMetadataId: [1],
			workspaceMemberSubFieldName: [1],
			rowLevelPermissionPredicateGroupId: [1],
			positionInRowLevelPermissionPredicateGroup: [11],
			roleId: [1],
			value: [15],
			__typename: [1]
		},
		RowLevelPermissionPredicateOperand: {},
		JSON: {},
		ObjectPermission: {
			objectMetadataId: [3],
			canReadObjectRecords: [6],
			canUpdateObjectRecords: [6],
			canSoftDeleteObjectRecords: [6],
			canDestroyObjectRecords: [6],
			restrictedFields: [15],
			rowLevelPermissionPredicates: [13],
			rowLevelPermissionPredicateGroups: [10],
			__typename: [1]
		},
		UserWorkspace: {
			id: [3],
			user: [79],
			userId: [3],
			locale: [1],
			createdAt: [4],
			updatedAt: [4],
			deletedAt: [4],
			permissionFlags: [18],
			objectPermissions: [16],
			objectsPermissions: [16],
			twoFactorAuthenticationMethodSummary: [9],
			__typename: [1]
		},
		PermissionFlagType: {},
		FullName: {
			firstName: [1],
			lastName: [1],
			__typename: [1]
		},
		WorkspaceMember: {
			id: [3],
			name: [19],
			userEmail: [1],
			colorScheme: [1],
			avatarUrl: [1],
			locale: [1],
			calendarStartDay: [21],
			timeZone: [1],
			dateFormat: [22],
			timeFormat: [23],
			roles: [29],
			userWorkspaceId: [3],
			numberFormat: [24],
			__typename: [1]
		},
		Int: {},
		WorkspaceMemberDateFormatEnum: {},
		WorkspaceMemberTimeFormatEnum: {},
		WorkspaceMemberNumberFormatEnum: {},
		Agent: {
			id: [3],
			name: [1],
			label: [1],
			icon: [1],
			description: [1],
			prompt: [1],
			modelId: [1],
			responseFormat: [15],
			roleId: [3],
			isCustom: [6],
			applicationId: [3],
			createdAt: [4],
			updatedAt: [4],
			modelConfiguration: [15],
			evaluationInputs: [1],
			__typename: [1]
		},
		FieldPermission: {
			id: [3],
			objectMetadataId: [3],
			fieldMetadataId: [3],
			roleId: [3],
			canReadFieldValue: [6],
			canUpdateFieldValue: [6],
			__typename: [1]
		},
		RolePermissionFlag: {
			id: [3],
			roleId: [3],
			flag: [1],
			__typename: [1]
		},
		ApiKeyForRole: {
			id: [3],
			name: [1],
			expiresAt: [4],
			revokedAt: [4],
			__typename: [1]
		},
		Role: {
			id: [3],
			universalIdentifier: [3],
			label: [1],
			description: [1],
			icon: [1],
			isEditable: [6],
			canBeAssignedToUsers: [6],
			canBeAssignedToAgents: [6],
			canBeAssignedToApiKeys: [6],
			workspaceMembers: [20],
			agents: [25],
			apiKeys: [28],
			canUpdateAllSettings: [6],
			canAccessAllTools: [6],
			canReadAllObjectRecords: [6],
			canUpdateAllObjectRecords: [6],
			canSoftDeleteAllObjectRecords: [6],
			canDestroyAllObjectRecords: [6],
			permissionFlags: [27],
			objectPermissions: [16],
			fieldPermissions: [26],
			rowLevelPermissionPredicates: [13],
			rowLevelPermissionPredicateGroups: [10],
			__typename: [1]
		},
		ApplicationRegistrationSummary: {
			id: [3],
			latestAvailableVersion: [1],
			sourceType: [8],
			logoUrl: [1],
			__typename: [1]
		},
		ApplicationVariable: {
			id: [3],
			key: [1],
			value: [1],
			description: [1],
			isSecret: [6],
			__typename: [1]
		},
		AuthToken: {
			token: [1],
			expiresAt: [4],
			__typename: [1]
		},
		ApplicationTokenPair: {
			applicationAccessToken: [32],
			applicationRefreshToken: [32],
			__typename: [1]
		},
		FrontComponent: {
			id: [3],
			name: [1],
			description: [1],
			sourceComponentPath: [1],
			builtComponentPath: [1],
			componentName: [1],
			builtComponentChecksum: [1],
			universalIdentifier: [3],
			applicationId: [3],
			createdAt: [4],
			updatedAt: [4],
			isHeadless: [6],
			usesSdkClient: [6],
			applicationTokenPair: [33],
			applicationVariables: [15],
			__typename: [1]
		},
		CommandMenuItem: {
			id: [3],
			workflowVersionId: [3],
			frontComponentId: [3],
			frontComponent: [34],
			engineComponentKey: [36],
			label: [1],
			icon: [1],
			shortLabel: [1],
			position: [11],
			isPinned: [6],
			availabilityType: [37],
			payload: [38],
			hotKeys: [1],
			conditionalAvailabilityExpression: [1],
			availabilityObjectMetadataId: [3],
			pageLayoutId: [3],
			universalIdentifier: [3],
			applicationId: [3],
			isActive: [6],
			createdAt: [4],
			updatedAt: [4],
			__typename: [1]
		},
		EngineComponentKey: {},
		CommandMenuItemAvailabilityType: {},
		CommandMenuItemPayload: {
			on_PathCommandMenuItemPayload: [39],
			on_ObjectMetadataCommandMenuItemPayload: [40],
			__typename: [1]
		},
		PathCommandMenuItemPayload: {
			path: [1],
			__typename: [1]
		},
		ObjectMetadataCommandMenuItemPayload: {
			objectMetadataItemId: [3],
			__typename: [1]
		},
		LogicFunction: {
			id: [3],
			name: [1],
			description: [1],
			runtime: [1],
			timeoutSeconds: [11],
			executionMode: [42],
			sourceHandlerPath: [1],
			handlerName: [1],
			cronTriggerSettings: [15],
			databaseEventTriggerSettings: [15],
			httpRouteTriggerSettings: [15],
			toolTriggerSettings: [15],
			workflowActionTriggerSettings: [15],
			applicationId: [3],
			universalIdentifier: [3],
			createdAt: [4],
			updatedAt: [4],
			__typename: [1]
		},
		LogicFunctionExecutionMode: {},
		StandardOverrides: {
			label: [1],
			description: [1],
			icon: [1],
			translations: [15],
			__typename: [1]
		},
		Field: {
			id: [3],
			universalIdentifier: [1],
			type: [45],
			name: [1],
			label: [1],
			description: [1],
			icon: [1],
			standardOverrides: [43],
			isActive: [6],
			isSystem: [6],
			isUIEditable: [6],
			isUIReadOnly: [6],
			isNullable: [6],
			isUnique: [6],
			defaultValue: [15],
			options: [15],
			settings: [15],
			objectMetadataId: [3],
			isLabelSyncedWithName: [6],
			morphId: [3],
			createdAt: [4],
			updatedAt: [4],
			applicationId: [3],
			relation: [203],
			morphRelations: [203],
			object: [56],
			__typename: [1]
		},
		FieldMetadataType: {},
		IndexField: {
			id: [3],
			fieldMetadataId: [3],
			order: [11],
			subFieldName: [1],
			createdAt: [4],
			updatedAt: [4],
			__typename: [1]
		},
		Index: {
			id: [3],
			name: [1],
			isCustom: [6],
			isUnique: [6],
			indexWhereClause: [1],
			indexType: [48],
			createdAt: [4],
			updatedAt: [4],
			indexFieldMetadataList: [46],
			objectMetadata: [211, {
				paging: [49, "CursorPaging!"],
				filter: [51, "ObjectFilter!"]
			}],
			indexFieldMetadatas: [209, {
				paging: [49, "CursorPaging!"],
				filter: [54, "IndexFieldFilter!"]
			}],
			__typename: [1]
		},
		IndexType: {},
		CursorPaging: {
			before: [50],
			after: [50],
			first: [21],
			last: [21],
			__typename: [1]
		},
		ConnectionCursor: {},
		ObjectFilter: {
			and: [51],
			or: [51],
			id: [52],
			isRemote: [53],
			isActive: [53],
			isSystem: [53],
			isUIEditable: [53],
			isUICreatable: [53],
			isUIReadOnly: [53],
			isSearchable: [53],
			__typename: [1]
		},
		UUIDFilterComparison: {
			is: [6],
			isNot: [6],
			eq: [3],
			neq: [3],
			gt: [3],
			gte: [3],
			lt: [3],
			lte: [3],
			like: [3],
			notLike: [3],
			iLike: [3],
			notILike: [3],
			in: [3],
			notIn: [3],
			__typename: [1]
		},
		BooleanFieldComparison: {
			is: [6],
			isNot: [6],
			__typename: [1]
		},
		IndexFieldFilter: {
			and: [54],
			or: [54],
			id: [52],
			fieldMetadataId: [52],
			__typename: [1]
		},
		ObjectStandardOverrides: {
			labelSingular: [1],
			labelPlural: [1],
			description: [1],
			icon: [1],
			color: [1],
			translations: [15],
			__typename: [1]
		},
		Object: {
			id: [3],
			universalIdentifier: [1],
			nameSingular: [1],
			namePlural: [1],
			labelSingular: [1],
			labelPlural: [1],
			description: [1],
			icon: [1],
			standardOverrides: [55],
			shortcut: [1],
			color: [1],
			isRemote: [6],
			isActive: [6],
			isSystem: [6],
			isUIEditable: [6],
			isUICreatable: [6],
			isUIReadOnly: [6],
			isSearchable: [6],
			applicationId: [3],
			createdAt: [4],
			updatedAt: [4],
			labelIdentifierFieldMetadataId: [3],
			imageIdentifierFieldMetadataId: [3],
			isLabelSyncedWithName: [6],
			duplicateCriteria: [1],
			fieldsList: [44],
			indexMetadataList: [47],
			fields: [216, {
				paging: [49, "CursorPaging!"],
				filter: [57, "FieldFilter!"]
			}],
			indexMetadatas: [214, {
				paging: [49, "CursorPaging!"],
				filter: [58, "IndexFilter!"]
			}],
			__typename: [1]
		},
		FieldFilter: {
			and: [57],
			or: [57],
			id: [52],
			isActive: [53],
			isSystem: [53],
			isUIEditable: [53],
			isUIReadOnly: [53],
			objectMetadataId: [52],
			__typename: [1]
		},
		IndexFilter: {
			and: [58],
			or: [58],
			id: [52],
			isCustom: [53],
			__typename: [1]
		},
		Application: {
			id: [3],
			name: [1],
			description: [1],
			logo: [1],
			version: [1],
			universalIdentifier: [1],
			packageJsonChecksum: [1],
			packageJsonFileId: [3],
			yarnLockChecksum: [1],
			yarnLockFileId: [3],
			availablePackages: [15],
			applicationRegistrationId: [3],
			canBeUninstalled: [6],
			defaultRoleId: [1],
			settingsCustomTabFrontComponentId: [3],
			defaultLogicFunctionRole: [29],
			agents: [25],
			frontComponents: [34],
			commandMenuItems: [35],
			logicFunctions: [41],
			objects: [56],
			applicationVariables: [31],
			applicationRegistration: [30],
			__typename: [1]
		},
		ViewField: {
			id: [3],
			fieldMetadataId: [3],
			isVisible: [6],
			size: [11],
			position: [11],
			aggregateOperation: [61],
			viewId: [3],
			viewFieldGroupId: [3],
			workspaceId: [3],
			createdAt: [4],
			updatedAt: [4],
			isActive: [6],
			deletedAt: [4],
			isOverridden: [6],
			__typename: [1]
		},
		AggregateOperations: {},
		ViewFilterGroup: {
			id: [3],
			parentViewFilterGroupId: [3],
			logicalOperator: [63],
			positionInViewFilterGroup: [11],
			viewId: [3],
			workspaceId: [3],
			createdAt: [4],
			updatedAt: [4],
			deletedAt: [4],
			__typename: [1]
		},
		ViewFilterGroupLogicalOperator: {},
		ViewFilter: {
			id: [3],
			fieldMetadataId: [3],
			operand: [65],
			value: [15],
			viewFilterGroupId: [3],
			positionInViewFilterGroup: [11],
			subFieldName: [1],
			relationTargetFieldMetadataId: [3],
			viewId: [3],
			workspaceId: [3],
			createdAt: [4],
			updatedAt: [4],
			deletedAt: [4],
			__typename: [1]
		},
		ViewFilterOperand: {},
		ViewGroup: {
			id: [3],
			isVisible: [6],
			fieldValue: [1],
			position: [11],
			viewId: [3],
			workspaceId: [3],
			createdAt: [4],
			updatedAt: [4],
			deletedAt: [4],
			__typename: [1]
		},
		ViewSort: {
			id: [3],
			fieldMetadataId: [3],
			direction: [68],
			subFieldName: [1],
			viewId: [3],
			workspaceId: [3],
			createdAt: [4],
			updatedAt: [4],
			deletedAt: [4],
			__typename: [1]
		},
		ViewSortDirection: {},
		ViewFieldGroup: {
			id: [3],
			name: [1],
			position: [11],
			isVisible: [6],
			viewId: [3],
			workspaceId: [3],
			createdAt: [4],
			updatedAt: [4],
			isActive: [6],
			deletedAt: [4],
			viewFields: [60],
			isOverridden: [6],
			__typename: [1]
		},
		View: {
			id: [3],
			name: [1],
			objectMetadataId: [3],
			type: [71],
			key: [72],
			icon: [1],
			position: [11],
			isCompact: [6],
			isCustom: [6],
			openRecordIn: [73],
			kanbanAggregateOperation: [61],
			kanbanAggregateOperationFieldMetadataId: [3],
			mainGroupByFieldMetadataId: [3],
			shouldHideEmptyGroups: [6],
			kanbanColumnWidth: [21],
			calendarFieldMetadataId: [3],
			workspaceId: [3],
			anyFieldFilterValue: [1],
			calendarLayout: [74],
			createdAt: [4],
			updatedAt: [4],
			deletedAt: [4],
			viewFields: [60],
			viewFilters: [64],
			viewFilterGroups: [62],
			viewSorts: [67],
			viewGroups: [66],
			viewFieldGroups: [69],
			visibility: [75],
			createdByUserWorkspaceId: [3],
			isActive: [6],
			__typename: [1]
		},
		ViewType: {},
		ViewKey: {},
		ViewOpenRecordIn: {},
		ViewCalendarLayout: {},
		ViewVisibility: {},
		Workspace: {
			id: [3],
			displayName: [1],
			logo: [1],
			logoFileId: [3],
			inviteHash: [1],
			deletedAt: [4],
			createdAt: [4],
			updatedAt: [4],
			allowImpersonation: [6],
			isPublicInviteLinkEnabled: [6],
			trashRetentionDays: [11],
			eventLogRetentionDays: [11],
			workspaceMembersCount: [11],
			activationStatus: [77],
			views: [70],
			viewFields: [60],
			viewFilters: [64],
			viewFilterGroups: [62],
			viewGroups: [66],
			viewSorts: [67],
			metadataVersion: [11],
			databaseSchema: [1],
			subdomain: [1],
			customDomain: [1],
			isGoogleAuthEnabled: [6],
			isGoogleAuthBypassEnabled: [6],
			isTwoFactorAuthenticationEnforced: [6],
			isPasswordAuthEnabled: [6],
			isPasswordAuthBypassEnabled: [6],
			isMicrosoftAuthEnabled: [6],
			isMicrosoftAuthBypassEnabled: [6],
			isCustomDomainEnabled: [6],
			isInternalMessagesImportEnabled: [6],
			editableProfileFields: [1],
			defaultRole: [29],
			fastModel: [1],
			smartModel: [1],
			aiAdditionalInstructions: [1],
			enabledAiModelIds: [1],
			useRecommendedModels: [6],
			routerModel: [1],
			workspaceCustomApplication: [59],
			featureFlags: [171],
			billingSubscriptions: [147],
			installedApplications: [59],
			currentBillingSubscription: [147],
			billingCustomer: [146],
			billingEntitlements: [230],
			hasValidSignedEnterpriseKey: [6],
			hasValidEnterpriseValidityToken: [6],
			workspaceUrls: [173],
			workspaceCustomApplicationId: [1],
			__typename: [1]
		},
		WorkspaceActivationStatus: {},
		AppToken: {
			id: [3],
			type: [1],
			expiresAt: [4],
			createdAt: [4],
			updatedAt: [4],
			__typename: [1]
		},
		User: {
			id: [3],
			firstName: [1],
			lastName: [1],
			email: [1],
			isEmailVerified: [6],
			disabled: [6],
			canImpersonate: [6],
			canAccessFullAdminPanel: [6],
			createdAt: [4],
			updatedAt: [4],
			deletedAt: [4],
			locale: [1],
			workspaceMember: [20],
			userWorkspaces: [17],
			onboardingStatus: [80],
			currentWorkspace: [76],
			currentUserWorkspace: [17],
			userVars: [81],
			workspaceMembers: [20],
			deletedWorkspaceMembers: [229],
			hasPassword: [6],
			supportUserHash: [1],
			workspaces: [17],
			availableWorkspaces: [228],
			__typename: [1]
		},
		OnboardingStatus: {},
		JSONObject: {},
		RatioAggregateConfig: {
			fieldMetadataId: [3],
			optionValue: [1],
			__typename: [1]
		},
		RichTextBody: {
			blocknote: [1],
			markdown: [1],
			__typename: [1]
		},
		GridPosition: {
			row: [11],
			column: [11],
			rowSpan: [11],
			columnSpan: [11],
			__typename: [1]
		},
		PageLayoutWidget: {
			id: [3],
			applicationId: [3],
			pageLayoutTabId: [3],
			title: [1],
			type: [86],
			objectMetadataId: [3],
			gridPosition: [84],
			position: [87],
			configuration: [92],
			conditionalDisplay: [15],
			conditionalAvailabilityExpression: [1],
			createdAt: [4],
			updatedAt: [4],
			isActive: [6],
			deletedAt: [4],
			isOverridden: [6],
			__typename: [1]
		},
		WidgetType: {},
		PageLayoutWidgetPosition: {
			on_PageLayoutWidgetGridPosition: [88],
			on_PageLayoutWidgetVerticalListPosition: [90],
			on_PageLayoutWidgetCanvasPosition: [91],
			__typename: [1]
		},
		PageLayoutWidgetGridPosition: {
			layoutMode: [89],
			row: [21],
			column: [21],
			rowSpan: [21],
			columnSpan: [21],
			__typename: [1]
		},
		PageLayoutTabLayoutMode: {},
		PageLayoutWidgetVerticalListPosition: {
			layoutMode: [89],
			index: [21],
			__typename: [1]
		},
		PageLayoutWidgetCanvasPosition: {
			layoutMode: [89],
			__typename: [1]
		},
		WidgetConfiguration: {
			on_AggregateChartConfiguration: [93],
			on_StandaloneRichTextConfiguration: [96],
			on_PieChartConfiguration: [97],
			on_LineChartConfiguration: [100],
			on_IframeConfiguration: [102],
			on_BarChartConfiguration: [103],
			on_CalendarConfiguration: [106],
			on_FrontComponentConfiguration: [107],
			on_EmailsConfiguration: [108],
			on_EmailThreadConfiguration: [109],
			on_FieldConfiguration: [110],
			on_FieldRichTextConfiguration: [112],
			on_FieldsConfiguration: [113],
			on_FilesConfiguration: [114],
			on_NotesConfiguration: [115],
			on_TasksConfiguration: [116],
			on_TimelineConfiguration: [117],
			on_ViewConfiguration: [118],
			on_RecordTableConfiguration: [119],
			on_WorkflowConfiguration: [120],
			on_WorkflowRunConfiguration: [121],
			on_WorkflowVersionConfiguration: [122],
			__typename: [1]
		},
		AggregateChartConfiguration: {
			configurationType: [94],
			aggregateFieldMetadataId: [3],
			aggregateOperation: [61],
			label: [1],
			displayDataLabel: [6],
			numberFormat: [95],
			description: [1],
			filter: [15],
			timezone: [1],
			firstDayOfTheWeek: [21],
			prefix: [1],
			suffix: [1],
			ratioAggregateConfig: [82],
			__typename: [1]
		},
		WidgetConfigurationType: {},
		ChartNumberFormat: {},
		StandaloneRichTextConfiguration: {
			configurationType: [94],
			body: [83],
			__typename: [1]
		},
		PieChartConfiguration: {
			configurationType: [94],
			aggregateFieldMetadataId: [3],
			aggregateOperation: [61],
			groupByFieldMetadataId: [3],
			groupBySubFieldName: [1],
			dateGranularity: [98],
			orderBy: [99],
			manualSortOrder: [1],
			displayDataLabel: [6],
			showCenterMetric: [6],
			displayLegend: [6],
			hideEmptyCategory: [6],
			splitMultiValueFields: [6],
			description: [1],
			color: [1],
			filter: [15],
			timezone: [1],
			firstDayOfTheWeek: [21],
			__typename: [1]
		},
		ObjectRecordGroupByDateGranularity: {},
		GraphOrderBy: {},
		LineChartConfiguration: {
			configurationType: [94],
			aggregateFieldMetadataId: [3],
			aggregateOperation: [61],
			primaryAxisGroupByFieldMetadataId: [3],
			primaryAxisGroupBySubFieldName: [1],
			primaryAxisDateGranularity: [98],
			primaryAxisOrderBy: [99],
			primaryAxisManualSortOrder: [1],
			secondaryAxisGroupByFieldMetadataId: [3],
			secondaryAxisGroupBySubFieldName: [1],
			secondaryAxisGroupByDateGranularity: [98],
			secondaryAxisOrderBy: [99],
			secondaryAxisManualSortOrder: [1],
			omitNullValues: [6],
			splitMultiValueFields: [6],
			axisNameDisplay: [101],
			displayDataLabel: [6],
			displayLegend: [6],
			rangeMin: [11],
			rangeMax: [11],
			description: [1],
			color: [1],
			filter: [15],
			isStacked: [6],
			isCumulative: [6],
			timezone: [1],
			firstDayOfTheWeek: [21],
			__typename: [1]
		},
		AxisNameDisplay: {},
		IframeConfiguration: {
			configurationType: [94],
			url: [1],
			__typename: [1]
		},
		BarChartConfiguration: {
			configurationType: [94],
			aggregateFieldMetadataId: [3],
			aggregateOperation: [61],
			primaryAxisGroupByFieldMetadataId: [3],
			primaryAxisGroupBySubFieldName: [1],
			primaryAxisDateGranularity: [98],
			primaryAxisOrderBy: [99],
			primaryAxisManualSortOrder: [1],
			secondaryAxisGroupByFieldMetadataId: [3],
			secondaryAxisGroupBySubFieldName: [1],
			secondaryAxisGroupByDateGranularity: [98],
			secondaryAxisOrderBy: [99],
			secondaryAxisManualSortOrder: [1],
			omitNullValues: [6],
			splitMultiValueFields: [6],
			axisNameDisplay: [101],
			displayDataLabel: [6],
			displayLegend: [6],
			rangeMin: [11],
			rangeMax: [11],
			description: [1],
			color: [1],
			filter: [15],
			groupMode: [104],
			layout: [105],
			isCumulative: [6],
			timezone: [1],
			firstDayOfTheWeek: [21],
			__typename: [1]
		},
		BarChartGroupMode: {},
		BarChartLayout: {},
		CalendarConfiguration: {
			configurationType: [94],
			__typename: [1]
		},
		FrontComponentConfiguration: {
			configurationType: [94],
			frontComponentId: [3],
			__typename: [1]
		},
		EmailsConfiguration: {
			configurationType: [94],
			__typename: [1]
		},
		EmailThreadConfiguration: {
			configurationType: [94],
			__typename: [1]
		},
		FieldConfiguration: {
			configurationType: [94],
			fieldMetadataId: [1],
			fieldDisplayMode: [111],
			viewId: [1],
			__typename: [1]
		},
		FieldDisplayMode: {},
		FieldRichTextConfiguration: {
			configurationType: [94],
			__typename: [1]
		},
		FieldsConfiguration: {
			configurationType: [94],
			viewId: [1],
			newFieldDefaultVisibility: [6],
			shouldAllowUserToSeeHiddenFields: [6],
			__typename: [1]
		},
		FilesConfiguration: {
			configurationType: [94],
			__typename: [1]
		},
		NotesConfiguration: {
			configurationType: [94],
			__typename: [1]
		},
		TasksConfiguration: {
			configurationType: [94],
			__typename: [1]
		},
		TimelineConfiguration: {
			configurationType: [94],
			__typename: [1]
		},
		ViewConfiguration: {
			configurationType: [94],
			__typename: [1]
		},
		RecordTableConfiguration: {
			configurationType: [94],
			viewId: [1],
			recordLimit: [21],
			__typename: [1]
		},
		WorkflowConfiguration: {
			configurationType: [94],
			__typename: [1]
		},
		WorkflowRunConfiguration: {
			configurationType: [94],
			__typename: [1]
		},
		WorkflowVersionConfiguration: {
			configurationType: [94],
			__typename: [1]
		},
		PageLayoutTab: {
			id: [3],
			applicationId: [3],
			title: [1],
			position: [11],
			pageLayoutId: [3],
			widgets: [85],
			icon: [1],
			layoutMode: [89],
			createdAt: [4],
			updatedAt: [4],
			isActive: [6],
			deletedAt: [4],
			isOverridden: [6],
			__typename: [1]
		},
		PageLayout: {
			id: [3],
			name: [1],
			type: [125],
			objectMetadataId: [3],
			tabs: [123],
			defaultTabToFocusOnMobileAndSidePanelId: [3],
			universalIdentifier: [3],
			createdAt: [4],
			updatedAt: [4],
			deletedAt: [4],
			__typename: [1]
		},
		PageLayoutType: {},
		ApplicationConnectionProviderOAuthConfig: {
			scopes: [1],
			isClientCredentialsConfigured: [6],
			__typename: [1]
		},
		ApplicationConnectionProvider: {
			id: [3],
			applicationId: [1],
			type: [1],
			name: [1],
			displayName: [1],
			oauth: [126],
			__typename: [1]
		},
		EnterpriseLicenseInfoDTO: {
			isValid: [6],
			licensee: [1],
			expiresAt: [4],
			subscriptionId: [1],
			__typename: [1]
		},
		EnterpriseSubscriptionStatusDTO: {
			status: [1],
			licensee: [1],
			expiresAt: [4],
			cancelAt: [4],
			currentPeriodEnd: [4],
			isCancellationScheduled: [6],
			__typename: [1]
		},
		ApprovedAccessDomain: {
			id: [3],
			domain: [1],
			isValidated: [6],
			createdAt: [4],
			__typename: [1]
		},
		FileWithSignedUrl: {
			id: [3],
			path: [1],
			size: [11],
			createdAt: [4],
			url: [1],
			__typename: [1]
		},
		BillingSubscriptionSchedulePhaseItem: {
			price: [1],
			quantity: [11],
			__typename: [1]
		},
		BillingSubscriptionSchedulePhase: {
			start_date: [11],
			end_date: [11],
			items: [132],
			__typename: [1]
		},
		BillingProductMetadata: {
			planKey: [135],
			priceUsageBased: [136],
			productKey: [137],
			__typename: [1]
		},
		BillingPlanKey: {},
		BillingUsageType: {},
		BillingProductKey: {},
		BillingPriceLicensed: {
			recurringInterval: [139],
			unitAmount: [11],
			stripePriceId: [1],
			priceUsageType: [136],
			creditAmount: [11],
			__typename: [1]
		},
		SubscriptionInterval: {},
		BillingPriceTier: {
			upTo: [11],
			flatAmount: [11],
			unitAmount: [11],
			__typename: [1]
		},
		BillingPriceMetered: {
			tiers: [140],
			recurringInterval: [139],
			stripePriceId: [1],
			priceUsageType: [136],
			__typename: [1]
		},
		BillingProduct: {
			name: [1],
			description: [1],
			images: [1],
			metadata: [134],
			__typename: [1]
		},
		BillingLicensedProduct: {
			name: [1],
			description: [1],
			images: [1],
			metadata: [134],
			prices: [138],
			__typename: [1]
		},
		BillingMeteredProduct: {
			name: [1],
			description: [1],
			images: [1],
			metadata: [134],
			prices: [141],
			__typename: [1]
		},
		BillingSubscriptionItem: {
			id: [3],
			hasReachedCurrentPeriodCap: [6],
			quantity: [11],
			stripePriceId: [1],
			billingProduct: [0],
			__typename: [1]
		},
		BillingCustomer: {
			id: [3],
			hasPaymentMethod: [6],
			__typename: [1]
		},
		BillingSubscription: {
			id: [3],
			status: [148],
			interval: [139],
			billingSubscriptionItems: [145],
			currentPeriodEnd: [4],
			metadata: [15],
			phases: [133],
			__typename: [1]
		},
		SubscriptionStatus: {},
		BillingEndTrialPeriod: {
			status: [148],
			hasPaymentMethod: [6],
			billingPortalUrl: [1],
			__typename: [1]
		},
		BillingResourceCreditUsage: {
			productKey: [137],
			periodStart: [4],
			periodEnd: [4],
			usedCredits: [11],
			grantedCredits: [11],
			rolloverCredits: [11],
			totalGrantedCredits: [11],
			unitPriceCents: [11],
			__typename: [1]
		},
		BillingPlan: {
			planKey: [135],
			baseProducts: [143],
			resourceCreditProducts: [143],
			meteredProducts: [144],
			__typename: [1]
		},
		BillingPaymentIntent: {
			clientSecret: [1],
			paymentIntentType: [1],
			__typename: [1]
		},
		BillingSession: {
			url: [1],
			__typename: [1]
		},
		BillingUpdate: {
			currentBillingSubscription: [147],
			billingSubscriptions: [147],
			__typename: [1]
		},
		InviteSuggestion: {
			email: [1],
			displayName: [1],
			__typename: [1]
		},
		OnboardingStepSuccess: {
			success: [6],
			__typename: [1]
		},
		WorkspaceInvitation: {
			id: [3],
			email: [1],
			roleId: [3],
			expiresAt: [4],
			__typename: [1]
		},
		SendInvitations: {
			success: [6],
			errors: [1],
			result: [157],
			__typename: [1]
		},
		RecordIdentifier: {
			id: [3],
			labelIdentifier: [1],
			imageIdentifier: [1],
			__typename: [1]
		},
		NavigationMenuItem: {
			id: [3],
			userWorkspaceId: [3],
			targetRecordId: [3],
			targetObjectMetadataId: [3],
			viewId: [3],
			type: [161],
			name: [1],
			link: [1],
			icon: [1],
			color: [1],
			folderId: [3],
			pageLayoutId: [3],
			position: [11],
			applicationId: [3],
			createdAt: [4],
			updatedAt: [4],
			targetRecordIdentifier: [159],
			__typename: [1]
		},
		NavigationMenuItemType: {},
		ObjectRecordEventProperties: {
			updatedFields: [1],
			before: [15],
			after: [15],
			diff: [15],
			__typename: [1]
		},
		MetadataEvent: {
			type: [164],
			metadataName: [1],
			recordId: [1],
			properties: [162],
			updatedCollectionHash: [1],
			__typename: [1]
		},
		MetadataEventAction: {},
		ObjectRecordEvent: {
			action: [166],
			objectNameSingular: [1],
			recordId: [1],
			userId: [1],
			workspaceMemberId: [1],
			properties: [162],
			__typename: [1]
		},
		DatabaseEventAction: {},
		ObjectRecordEventWithQueryIds: {
			queryIds: [1],
			objectRecordEvent: [165],
			__typename: [1]
		},
		EventSubscription: {
			eventStreamId: [1],
			objectRecordEventsWithQueryIds: [167],
			metadataEvents: [163],
			__typename: [1]
		},
		LogicFunctionExecutionResult: {
			data: [15],
			logs: [1],
			duration: [11],
			status: [170],
			error: [15],
			__typename: [1]
		},
		LogicFunctionExecutionStatus: {},
		FeatureFlag: {
			key: [172],
			value: [6],
			__typename: [1]
		},
		FeatureFlagKey: {},
		WorkspaceUrls: {
			customUrl: [1],
			subdomainUrl: [1],
			__typename: [1]
		},
		ApplicationRegistrationVariableDTO: {
			id: [3],
			key: [1],
			value: [1],
			description: [1],
			isSecret: [6],
			isRequired: [6],
			isFilled: [6],
			createdAt: [4],
			updatedAt: [4],
			__typename: [1]
		},
		BillingTrialPeriod: {
			duration: [11],
			isCreditCardRequired: [6],
			__typename: [1]
		},
		SSOIdentityProvider: {
			id: [3],
			name: [1],
			type: [177],
			status: [178],
			issuer: [1],
			__typename: [1]
		},
		IdentityProviderType: {},
		SSOIdentityProviderStatus: {},
		AuthProviders: {
			sso: [176],
			google: [6],
			magicLink: [6],
			password: [6],
			microsoft: [6],
			__typename: [1]
		},
		AuthBypassProviders: {
			google: [6],
			password: [6],
			microsoft: [6],
			__typename: [1]
		},
		PublicWorkspaceData: {
			id: [3],
			authProviders: [179],
			authBypassProviders: [180],
			logo: [1],
			displayName: [1],
			workspaceUrls: [173],
			__typename: [1]
		},
		PublicWorkspaceDataSummary: {
			id: [3],
			logo: [1],
			displayName: [1],
			__typename: [1]
		},
		NativeModelCapabilities: {
			webSearch: [6],
			twitterSearch: [6],
			__typename: [1]
		},
		ClientAiModelConfig: {
			modelId: [1],
			label: [1],
			modelFamily: [185],
			modelFamilyLabel: [1],
			sdkPackage: [1],
			inputCostPerMillionTokens: [11],
			outputCostPerMillionTokens: [11],
			nativeCapabilities: [183],
			isDeprecated: [6],
			isRecommended: [6],
			providerName: [1],
			providerLabel: [1],
			contextWindowTokens: [11],
			maxOutputTokens: [11],
			dataResidency: [1],
			__typename: [1]
		},
		ModelFamily: {},
		Billing: {
			isBillingEnabled: [6],
			billingUrl: [1],
			stripePublishableKey: [1],
			trialPeriods: [175],
			__typename: [1]
		},
		Support: {
			supportDriver: [188],
			supportFrontChatId: [1],
			__typename: [1]
		},
		SupportDriver: {},
		Sentry: {
			environment: [1],
			release: [1],
			dsn: [1],
			__typename: [1]
		},
		Captcha: {
			provider: [191],
			siteKey: [1],
			__typename: [1]
		},
		CaptchaDriverType: {},
		ApiConfig: {
			mutationMaximumAffectedRecords: [11],
			__typename: [1]
		},
		PublicFeatureFlagMetadata: {
			label: [1],
			description: [1],
			imagePath: [1],
			__typename: [1]
		},
		PublicFeatureFlag: {
			key: [172],
			metadata: [193],
			__typename: [1]
		},
		ClientConfigMaintenanceMode: {
			startAt: [4],
			endAt: [4],
			link: [1],
			__typename: [1]
		},
		ClientConfig: {
			appVersion: [1],
			authProviders: [179],
			billing: [186],
			aiModels: [184],
			signInPrefilled: [6],
			isMultiWorkspaceEnabled: [6],
			isEmailVerificationRequired: [6],
			defaultSubdomain: [1],
			frontDomain: [1],
			publicFunctionDomain: [1],
			analyticsEnabled: [6],
			support: [187],
			isAttachmentPreviewEnabled: [6],
			sentry: [189],
			captcha: [190],
			api: [192],
			canManageFeatureFlags: [6],
			publicFeatureFlags: [194],
			isMicrosoftMessagingEnabled: [6],
			isMicrosoftCalendarEnabled: [6],
			isGoogleMessagingEnabled: [6],
			isGoogleCalendarEnabled: [6],
			isConfigVariablesInDbEnabled: [6],
			isImapSmtpCaldavEnabled: [6],
			isEmailingDomainInDemoMode: [6],
			allowRequestsToTwentyIcons: [6],
			calendarBookingPageId: [1],
			isCloudflareIntegrationEnabled: [6],
			isClickHouseConfigured: [6],
			isWorkspaceSchemaDDLLocked: [6],
			maintenance: [195],
			__typename: [1]
		},
		UsageBreakdownItem: {
			key: [1],
			label: [1],
			creditsUsed: [11],
			__typename: [1]
		},
		VersionDistributionEntry: {
			version: [1],
			count: [21],
			__typename: [1]
		},
		ApplicationRegistrationStats: {
			activeInstalls: [21],
			mostInstalledVersion: [1],
			versionDistribution: [198],
			__typename: [1]
		},
		CreateApplicationRegistration: {
			applicationRegistration: [7],
			clientSecret: [1],
			__typename: [1]
		},
		PublicApplicationRegistration: {
			id: [3],
			name: [1],
			logoUrl: [1],
			websiteUrl: [1],
			oAuthScopes: [1],
			__typename: [1]
		},
		RotateClientSecret: {
			clientSecret: [1],
			__typename: [1]
		},
		Relation: {
			type: [204],
			sourceObjectMetadata: [56],
			targetObjectMetadata: [56],
			sourceFieldMetadata: [44],
			targetFieldMetadata: [44],
			__typename: [1]
		},
		RelationType: {},
		IndexEdge: {
			node: [47],
			cursor: [50],
			__typename: [1]
		},
		PageInfo: {
			hasNextPage: [6],
			hasPreviousPage: [6],
			startCursor: [50],
			endCursor: [50],
			__typename: [1]
		},
		IndexConnection: {
			pageInfo: [206],
			edges: [205],
			__typename: [1]
		},
		IndexFieldEdge: {
			node: [46],
			cursor: [50],
			__typename: [1]
		},
		IndexIndexFieldMetadatasConnection: {
			pageInfo: [206],
			edges: [208],
			__typename: [1]
		},
		ObjectEdge: {
			node: [56],
			cursor: [50],
			__typename: [1]
		},
		IndexObjectMetadataConnection: {
			pageInfo: [206],
			edges: [210],
			__typename: [1]
		},
		ObjectRecordCount: {
			objectNamePlural: [1],
			totalCount: [21],
			__typename: [1]
		},
		ObjectConnection: {
			pageInfo: [206],
			edges: [210],
			__typename: [1]
		},
		ObjectIndexMetadatasConnection: {
			pageInfo: [206],
			edges: [205],
			__typename: [1]
		},
		FieldEdge: {
			node: [44],
			cursor: [50],
			__typename: [1]
		},
		ObjectFieldsConnection: {
			pageInfo: [206],
			edges: [215],
			__typename: [1]
		},
		FieldConnection: {
			pageInfo: [206],
			edges: [215],
			__typename: [1]
		},
		AppConnection: {
			id: [219],
			providerName: [1],
			name: [1],
			handle: [1],
			visibility: [1],
			userWorkspaceId: [1],
			accessToken: [1],
			scopes: [1],
			authFailedAt: [1],
			__typename: [1]
		},
		ID: {},
		ResendEmailVerificationToken: {
			success: [6],
			__typename: [1]
		},
		DeleteSso: {
			identityProviderId: [3],
			__typename: [1]
		},
		EditSso: {
			id: [3],
			type: [177],
			issuer: [1],
			name: [1],
			status: [178],
			__typename: [1]
		},
		WorkspaceNameAndId: {
			displayName: [1],
			id: [3],
			__typename: [1]
		},
		FindAvailableSSOIDP: {
			type: [177],
			id: [3],
			issuer: [1],
			name: [1],
			status: [178],
			workspace: [223],
			__typename: [1]
		},
		SetupSso: {
			id: [3],
			type: [177],
			issuer: [1],
			name: [1],
			status: [178],
			__typename: [1]
		},
		SSOConnection: {
			type: [177],
			id: [3],
			issuer: [1],
			name: [1],
			status: [178],
			__typename: [1]
		},
		AvailableWorkspace: {
			id: [3],
			displayName: [1],
			loginToken: [1],
			personalInviteToken: [1],
			inviteHash: [1],
			workspaceUrls: [173],
			logo: [1],
			sso: [226],
			__typename: [1]
		},
		AvailableWorkspaces: {
			availableWorkspacesForSignIn: [227],
			availableWorkspacesForSignUp: [227],
			__typename: [1]
		},
		DeletedWorkspaceMember: {
			id: [3],
			name: [19],
			userEmail: [1],
			avatarUrl: [1],
			userWorkspaceId: [3],
			__typename: [1]
		},
		BillingEntitlement: {
			key: [231],
			value: [6],
			__typename: [1]
		},
		BillingEntitlementKey: {},
		DomainRecord: {
			validationType: [1],
			type: [1],
			status: [1],
			key: [1],
			value: [1],
			__typename: [1]
		},
		DomainValidRecords: {
			id: [3],
			domain: [1],
			records: [232],
			isCustomDomainEnabled: [6],
			__typename: [1]
		},
		UpsertRowLevelPermissionPredicatesResult: {
			predicates: [13],
			predicateGroups: [10],
			__typename: [1]
		},
		LogicFunctionLogs: {
			logs: [1],
			__typename: [1]
		},
		PublicConnectionParametersOutput: {
			host: [1],
			port: [11],
			username: [1],
			connectionSecurity: [237],
			__typename: [1]
		},
		EmailConnectionSecurity: {},
		PublicImapSmtpCaldavConnectionParameters: {
			IMAP: [236],
			SMTP: [236],
			CALDAV: [236],
			__typename: [1]
		},
		ConnectedAccountPublicDTO: {
			id: [3],
			handle: [1],
			provider: [1],
			lastCredentialsRefreshedAt: [4],
			authFailedAt: [4],
			archivedAt: [4],
			handleAliases: [1],
			scopes: [1],
			lastSignedInAt: [4],
			userWorkspaceId: [3],
			connectionProviderId: [3],
			applicationId: [3],
			name: [1],
			visibility: [1],
			createdAt: [4],
			updatedAt: [4],
			connectionParameters: [238],
			__typename: [1]
		},
		DeleteTwoFactorAuthenticationMethod: {
			success: [6],
			__typename: [1]
		},
		InitiateTwoFactorAuthenticationProvisioning: {
			uri: [1],
			__typename: [1]
		},
		VerifyTwoFactorAuthenticationMethod: {
			success: [6],
			__typename: [1]
		},
		AuthorizeApp: {
			redirectUrl: [1],
			__typename: [1]
		},
		AuthTokenPair: {
			accessOrWorkspaceAgnosticToken: [32],
			refreshToken: [32],
			__typename: [1]
		},
		AvailableWorkspacesAndAccessTokens: {
			tokens: [244],
			availableWorkspaces: [228],
			__typename: [1]
		},
		EmailPasswordResetLink: {
			success: [6],
			__typename: [1]
		},
		GetAuthorizationUrlForSSO: {
			authorizationURL: [1],
			type: [1],
			id: [3],
			__typename: [1]
		},
		InvalidatePassword: {
			success: [6],
			__typename: [1]
		},
		WorkspaceUrlsAndId: {
			workspaceUrls: [173],
			id: [3],
			__typename: [1]
		},
		SignUp: {
			loginToken: [32],
			workspace: [249],
			__typename: [1]
		},
		TransientToken: {
			transientToken: [32],
			__typename: [1]
		},
		ValidatePasswordResetToken: {
			id: [3],
			email: [1],
			hasPassword: [6],
			__typename: [1]
		},
		VerifyEmailAndGetLoginToken: {
			loginToken: [32],
			workspaceUrls: [173],
			__typename: [1]
		},
		SubdomainAvailabilityDTO: {
			isValid: [6],
			available: [6],
			suggestedSubdomain: [1],
			__typename: [1]
		},
		WorkspaceCreationDefaultsDTO: {
			displayName: [1],
			subdomain: [1],
			__typename: [1]
		},
		ApiKeyToken: {
			token: [1],
			__typename: [1]
		},
		AuthTokens: {
			tokens: [244],
			__typename: [1]
		},
		LoginToken: {
			loginToken: [32],
			__typename: [1]
		},
		CheckUserExist: {
			exists: [6],
			availableWorkspacesCount: [11],
			isEmailVerified: [6],
			__typename: [1]
		},
		WorkspaceInviteHashValid: {
			isValid: [6],
			__typename: [1]
		},
		Impersonate: {
			loginToken: [32],
			workspace: [249],
			__typename: [1]
		},
		UsageTimeSeries: {
			date: [1],
			creditsUsed: [11],
			__typename: [1]
		},
		UsageUserDaily: {
			userWorkspaceId: [1],
			dailyUsage: [262],
			__typename: [1]
		},
		UsageAnalytics: {
			usageByUser: [197],
			usageByOperationType: [197],
			usageByModel: [197],
			timeSeries: [262],
			periodStart: [4],
			periodEnd: [4],
			userDailyUsage: [263],
			__typename: [1]
		},
		DevelopmentApplication: {
			id: [1],
			universalIdentifier: [1],
			__typename: [1]
		},
		WorkspaceMigration: {
			applicationUniversalIdentifier: [1],
			actions: [15],
			__typename: [1]
		},
		File: {
			id: [3],
			path: [1],
			size: [11],
			createdAt: [4],
			__typename: [1]
		},
		MarketplaceApp: {
			id: [1],
			name: [1],
			description: [1],
			author: [1],
			category: [1],
			logo: [1],
			sourcePackage: [1],
			isFeatured: [6],
			__typename: [1]
		},
		MarketplaceAppDetail: {
			universalIdentifier: [1],
			id: [1],
			name: [1],
			sourceType: [8],
			sourcePackage: [1],
			latestAvailableVersion: [1],
			isListed: [6],
			isFeatured: [6],
			manifest: [15],
			__typename: [1]
		},
		PublicDomain: {
			id: [3],
			domain: [1],
			isValidated: [6],
			applicationId: [3],
			createdAt: [4],
			__typename: [1]
		},
		VerificationRecord: {
			type: [1],
			key: [1],
			value: [1],
			priority: [11],
			__typename: [1]
		},
		EmailingDomain: {
			id: [3],
			createdAt: [4],
			updatedAt: [4],
			domain: [1],
			status: [273],
			verificationRecords: [271],
			verifiedAt: [4],
			__typename: [1]
		},
		EmailingDomainStatus: {},
		MessageChannel: {
			id: [3],
			visibility: [275],
			handle: [1],
			type: [276],
			isContactAutoCreationEnabled: [6],
			contactAutoCreationPolicy: [277],
			messageFolderImportPolicy: [278],
			excludeNonProfessionalEmails: [6],
			excludeGroupEmails: [6],
			pendingGroupEmailsAction: [279],
			isSyncEnabled: [6],
			syncedAt: [4],
			syncStatus: [280],
			syncStage: [281],
			syncStageStartedAt: [4],
			throttleFailureCount: [11],
			throttleRetryAfter: [4],
			connectedAccountId: [3],
			createdAt: [4],
			updatedAt: [4],
			connectedAccount: [239],
			__typename: [1]
		},
		MessageChannelVisibility: {},
		MessageChannelType: {},
		MessageChannelContactAutoCreationPolicy: {},
		MessageFolderImportPolicy: {},
		MessageChannelPendingGroupEmailsAction: {},
		MessageChannelSyncStatus: {},
		MessageChannelSyncStage: {},
		CreateEmailGroupChannelOutput: {
			messageChannel: [274],
			forwardingAddress: [1],
			__typename: [1]
		},
		CampaignAudiencePreviewDTO: {
			totalMembers: [21],
			withoutEmail: [21],
			duplicateEmails: [21],
			globallyUnsubscribed: [21],
			topicUnsubscribed: [21],
			sendable: [21],
			__typename: [1]
		},
		SendEmailViaDomainOutput: {
			messageId: [1],
			__typename: [1]
		},
		CampaignSkippedRecipientsDTO: {
			noEmail: [21],
			deduped: [21],
			overCap: [21],
			__typename: [1]
		},
		SendMessageCampaignOutputDTO: {
			campaignId: [1],
			queuedCount: [21],
			skipped: [285],
			__typename: [1]
		},
		UnsubscribeTopic: {
			id: [3],
			createdAt: [4],
			updatedAt: [4],
			name: [1],
			description: [1],
			visibility: [288],
			__typename: [1]
		},
		UnsubscribeTopicVisibility: {},
		AutocompleteResult: {
			text: [1],
			placeId: [1],
			__typename: [1]
		},
		Location: {
			lat: [11],
			lng: [11],
			__typename: [1]
		},
		PlaceDetailsResult: {
			street: [1],
			state: [1],
			postcode: [1],
			city: [1],
			country: [1],
			location: [290],
			__typename: [1]
		},
		ImapSmtpCaldavPublicConnectionParams: {
			host: [1],
			port: [11],
			username: [1],
			connectionSecurity: [237],
			__typename: [1]
		},
		ImapSmtpCaldavPublicConnectionParameters: {
			IMAP: [292],
			SMTP: [292],
			CALDAV: [292],
			__typename: [1]
		},
		ConnectedImapSmtpCaldavAccount: {
			id: [3],
			handle: [1],
			provider: [1],
			userWorkspaceId: [3],
			connectionParameters: [293],
			__typename: [1]
		},
		ImapSmtpCaldavConnectionSuccess: {
			success: [6],
			connectedAccountId: [1],
			__typename: [1]
		},
		Webhook: {
			id: [3],
			targetUrl: [1],
			operations: [1],
			description: [1],
			secret: [1],
			applicationId: [3],
			createdAt: [4],
			updatedAt: [4],
			deletedAt: [4],
			__typename: [1]
		},
		ToolIndexEntry: {
			name: [1],
			label: [1],
			description: [1],
			category: [1],
			objectName: [1],
			icon: [1],
			inputSchema: [15],
			__typename: [1]
		},
		AgentMessagePart: {
			id: [3],
			messageId: [3],
			orderIndex: [21],
			type: [1],
			textContent: [1],
			reasoningContent: [1],
			toolName: [1],
			toolCallId: [1],
			toolInput: [15],
			toolOutput: [15],
			state: [1],
			providerExecuted: [6],
			errorMessage: [1],
			errorDetails: [15],
			sourceUrlSourceId: [1],
			sourceUrlUrl: [1],
			sourceUrlTitle: [1],
			sourceDocumentSourceId: [1],
			sourceDocumentMediaType: [1],
			sourceDocumentTitle: [1],
			sourceDocumentFilename: [1],
			fileMediaType: [1],
			fileFilename: [1],
			fileId: [3],
			fileUrl: [1],
			providerMetadata: [15],
			createdAt: [4],
			__typename: [1]
		},
		RunAgentResult: {
			result: [15],
			error: [1],
			success: [6],
			__typename: [1]
		},
		ChannelSyncSuccess: {
			success: [6],
			__typename: [1]
		},
		BarChartSeries: {
			key: [1],
			label: [1],
			__typename: [1]
		},
		BarChartData: {
			data: [15],
			indexBy: [1],
			keys: [1],
			series: [301],
			xAxisLabel: [1],
			yAxisLabel: [1],
			showLegend: [6],
			showDataLabels: [6],
			layout: [105],
			groupMode: [104],
			hasTooManyGroups: [6],
			formattedToRawLookup: [15],
			__typename: [1]
		},
		LineChartDataPoint: {
			x: [1],
			y: [11],
			__typename: [1]
		},
		LineChartSeries: {
			key: [1],
			label: [1],
			data: [303],
			__typename: [1]
		},
		LineChartData: {
			series: [304],
			xAxisLabel: [1],
			yAxisLabel: [1],
			showLegend: [6],
			showDataLabels: [6],
			hasTooManyGroups: [6],
			formattedToRawLookup: [15],
			__typename: [1]
		},
		PieChartDataItem: {
			key: [1],
			value: [11],
			__typename: [1]
		},
		PieChartData: {
			data: [306],
			showLegend: [6],
			showDataLabels: [6],
			showCenterMetric: [6],
			hasTooManyGroups: [6],
			formattedToRawLookup: [15],
			__typename: [1]
		},
		DuplicatedDashboard: {
			id: [3],
			title: [1],
			pageLayoutId: [3],
			position: [11],
			createdAt: [1],
			updatedAt: [1],
			__typename: [1]
		},
		SendEmailOutput: {
			success: [6],
			error: [1],
			__typename: [1]
		},
		Analytics: {
			success: [6],
			__typename: [1]
		},
		EventLogRecord: {
			event: [1],
			timestamp: [4],
			userId: [1],
			properties: [15],
			recordId: [1],
			objectMetadataId: [1],
			isCustom: [6],
			__typename: [1]
		},
		EventLogPageInfo: {
			endCursor: [1],
			hasNextPage: [6],
			__typename: [1]
		},
		EventLogQueryResult: {
			records: [311],
			totalCount: [21],
			pageInfo: [312],
			__typename: [1]
		},
		Skill: {
			id: [3],
			name: [1],
			label: [1],
			icon: [1],
			description: [1],
			content: [1],
			isCustom: [6],
			isActive: [6],
			applicationId: [3],
			createdAt: [4],
			updatedAt: [4],
			__typename: [1]
		},
		AgentMessage: {
			id: [3],
			threadId: [3],
			turnId: [3],
			agentId: [3],
			role: [1],
			status: [1],
			parts: [298],
			processedAt: [4],
			createdAt: [4],
			__typename: [1]
		},
		AgentChatThread: {
			id: [219],
			title: [1],
			totalInputTokens: [21],
			totalOutputTokens: [21],
			contextWindowTokens: [21],
			conversationSize: [21],
			totalInputCredits: [11],
			totalOutputCredits: [11],
			createdAt: [4],
			updatedAt: [4],
			deletedAt: [4],
			lastMessageAt: [4],
			__typename: [1]
		},
		AiSystemPromptSection: {
			title: [1],
			content: [1],
			estimatedTokenCount: [21],
			__typename: [1]
		},
		AiSystemPromptPreview: {
			sections: [317],
			estimatedTokenCount: [21],
			__typename: [1]
		},
		ChatStreamCatchupChunks: {
			chunks: [15],
			maxSeq: [21],
			__typename: [1]
		},
		SendChatMessageResult: {
			messageId: [1],
			queued: [6],
			streamId: [1],
			__typename: [1]
		},
		AgentChatEvent: {
			threadId: [1],
			event: [15],
			__typename: [1]
		},
		AgentTurnEvaluation: {
			id: [3],
			turnId: [3],
			score: [21],
			comment: [1],
			createdAt: [4],
			__typename: [1]
		},
		AgentTurn: {
			id: [3],
			threadId: [3],
			agentId: [3],
			evaluations: [322],
			messages: [315],
			createdAt: [4],
			__typename: [1]
		},
		WorkspaceAiStats: {
			conversationsCount: [21],
			skillsCount: [21],
			toolsCount: [21],
			__typename: [1]
		},
		CalendarChannel: {
			id: [3],
			handle: [1],
			syncStatus: [326],
			syncStage: [327],
			visibility: [328],
			isContactAutoCreationEnabled: [6],
			contactAutoCreationPolicy: [329],
			isSyncEnabled: [6],
			syncedAt: [4],
			syncStageStartedAt: [4],
			throttleFailureCount: [11],
			connectedAccountId: [3],
			createdAt: [4],
			updatedAt: [4],
			__typename: [1]
		},
		CalendarChannelSyncStatus: {},
		CalendarChannelSyncStage: {},
		CalendarChannelVisibility: {},
		CalendarChannelContactAutoCreationPolicy: {},
		MessageFolder: {
			id: [3],
			name: [1],
			isSentFolder: [6],
			isSynced: [6],
			parentFolderId: [1],
			externalId: [1],
			pendingSyncAction: [331],
			messageChannelId: [3],
			createdAt: [4],
			updatedAt: [4],
			__typename: [1]
		},
		MessageFolderPendingSyncAction: {},
		CollectionHash: {
			collectionName: [333],
			hash: [1],
			__typename: [1]
		},
		AllMetadataName: {},
		MinimalObjectMetadata: {
			id: [3],
			nameSingular: [1],
			namePlural: [1],
			labelSingular: [1],
			labelPlural: [1],
			icon: [1],
			color: [1],
			isActive: [6],
			isSystem: [6],
			isRemote: [6],
			__typename: [1]
		},
		MinimalView: {
			id: [3],
			type: [71],
			key: [72],
			objectMetadataId: [3],
			__typename: [1]
		},
		MinimalMetadata: {
			objectMetadataItems: [334],
			views: [335],
			collectionHashes: [332],
			__typename: [1]
		},
		Query: {
			navigationMenuItems: [160],
			navigationMenuItem: [160, { id: [3, "UUID!"] }],
			enterprisePortalSession: [1, { returnUrlPath: [1] }],
			enterpriseCheckoutSession: [1, { billingInterval: [1] }],
			enterpriseSubscriptionStatus: [129],
			getViewFilterGroups: [62, { viewId: [1] }],
			getViewFilterGroup: [62, { id: [1, "String!"] }],
			getViewFilters: [64, { viewId: [1] }],
			getViewFilter: [64, { id: [1, "String!"] }],
			getViews: [70, {
				objectMetadataId: [1],
				viewTypes: [71, "[ViewType!]"]
			}],
			getView: [70, { id: [1, "String!"] }],
			getViewSorts: [67, { viewId: [1] }],
			getViewSort: [67, { id: [1, "String!"] }],
			getViewFields: [60, { viewId: [1, "String!"] }],
			getViewField: [60, { id: [1, "String!"] }],
			getViewFieldGroups: [69, { viewId: [1, "String!"] }],
			getViewFieldGroup: [69, { id: [1, "String!"] }],
			apiKeys: [2],
			apiKey: [2, { input: [338, "GetApiKeyInput!"] }],
			getInviteSuggestions: [155],
			applicationConnectionProviders: [127, { applicationId: [3, "UUID!"] }],
			billingPortalSession: [153, {
				returnUrlPath: [1],
				forPaymentMethodUpdate: [6]
			}],
			listPlans: [151],
			getResourceCreditUsage: [150],
			findWorkspaceInvitations: [157],
			getApprovedAccessDomains: [130],
			getPageLayoutTabs: [123, { pageLayoutId: [1, "String!"] }],
			getPageLayoutTab: [123, { id: [1, "String!"] }],
			getPageLayouts: [124, {
				objectMetadataId: [1],
				pageLayoutType: [125]
			}],
			getPageLayout: [124, { id: [1, "String!"] }],
			getPageLayoutWidgets: [85, { pageLayoutTabId: [1, "String!"] }],
			getPageLayoutWidget: [85, { id: [1, "String!"] }],
			findManyAgents: [25],
			findOneAgent: [25, { input: [339, "AgentIdInput!"] }],
			objectRecordCounts: [212],
			object: [56, { id: [3, "UUID!"] }],
			objects: [213, {
				paging: [49, "CursorPaging!"],
				filter: [51, "ObjectFilter!"]
			}],
			index: [47, { id: [3, "UUID!"] }],
			indexMetadatas: [207, {
				paging: [49, "CursorPaging!"],
				filter: [58, "IndexFilter!"]
			}],
			findOneLogicFunction: [41, { input: [340, "LogicFunctionIdInput!"] }],
			findManyLogicFunctions: [41],
			getAvailablePackages: [15, { input: [340, "LogicFunctionIdInput!"] }],
			getLogicFunctionSourceCode: [1, { input: [340, "LogicFunctionIdInput!"] }],
			commandMenuItems: [35],
			commandMenuItem: [35, { id: [3, "UUID!"] }],
			frontComponents: [34],
			frontComponent: [34, { id: [3, "UUID!"] }],
			previewMessageCampaignAudience: [283, { input: [341, "PreviewMessageCampaignAudienceInput!"] }],
			unsubscribeTopics: [287],
			unsubscribePagePreviewUrl: [1],
			myMessageChannels: [274, { connectedAccountId: [3] }],
			getEmailingDomains: [272],
			myConnectedAccounts: [239],
			getRoles: [29],
			getToolIndex: [297],
			getToolInputSchema: [15, { toolName: [1, "String!"] }],
			webhooks: [296],
			webhook: [296, { id: [3, "UUID!"] }],
			field: [44, { id: [3, "UUID!"] }],
			fields: [217, {
				paging: [49, "CursorPaging!"],
				filter: [57, "FieldFilter!"]
			}],
			getViewGroups: [66, { viewId: [1] }],
			getViewGroup: [66, { id: [1, "String!"] }],
			myMessageFolders: [330, { messageChannelId: [3] }],
			myCalendarChannels: [325, { connectedAccountId: [3] }],
			minimalMetadata: [336],
			appConnections: [218, { filter: [342] }],
			appConnection: [218, { id: [219, "ID!"] }],
			findWorkspaceAiStats: [324],
			chatThreads: [316],
			chatThread: [316, { id: [3, "UUID!"] }],
			chatMessages: [315, { threadId: [3, "UUID!"] }],
			chatStreamCatchupChunks: [319, { threadId: [3, "UUID!"] }],
			getAiSystemPromptPreview: [318],
			skills: [314],
			skill: [314, { id: [3, "UUID!"] }],
			agentTurns: [323, { agentId: [3, "UUID!"] }],
			checkUserExists: [259, {
				email: [1, "String!"],
				captchaToken: [1]
			}],
			checkWorkspaceInviteHashIsValid: [260, { inviteHash: [1, "String!"] }],
			findWorkspaceFromInviteHash: [76, { inviteHash: [1, "String!"] }],
			checkWorkspaceSubdomainAvailability: [254, { subdomain: [1, "String!"] }],
			getWorkspaceCreationDefaults: [255],
			validatePasswordResetToken: [252, { passwordResetToken: [1, "String!"] }],
			findApplicationRegistrationByClientId: [201, { clientId: [1, "String!"] }],
			findApplicationRegistrationByUniversalIdentifier: [7, { universalIdentifier: [1, "String!"] }],
			findManyApplicationRegistrations: [7],
			findOneApplicationRegistration: [7, { id: [1, "String!"] }],
			findApplicationRegistrationStats: [199, { id: [1, "String!"] }],
			findApplicationRegistrationVariables: [174, { applicationRegistrationId: [1, "String!"] }],
			applicationRegistrationTarballUrl: [1, { id: [1, "String!"] }],
			currentUser: [79],
			currentWorkspace: [76],
			getPublicWorkspaceDataByDomain: [181, { origin: [1] }],
			getPublicWorkspaceDataById: [182, { id: [3, "UUID!"] }],
			findManyApplications: [59],
			findOneApplication: [59, {
				id: [3],
				universalIdentifier: [3]
			}],
			getSSOIdentityProviders: [224],
			eventLogs: [313, { input: [343, "EventLogQueryInput!"] }],
			pieChartData: [307, { input: [347, "PieChartDataInput!"] }],
			lineChartData: [305, { input: [348, "LineChartDataInput!"] }],
			barChartData: [302, { input: [349, "BarChartDataInput!"] }],
			getConnectedImapSmtpCaldavAccount: [294, { id: [3, "UUID!"] }],
			getAutoCompleteAddress: [289, {
				address: [1, "String!"],
				token: [1, "String!"],
				country: [1],
				isFieldCity: [6]
			}],
			getAddressDetails: [291, {
				placeId: [1, "String!"],
				token: [1, "String!"]
			}],
			getUsageAnalytics: [264, { input: [350] }],
			findManyPublicDomains: [270],
			findManyMarketplaceApps: [268],
			findMarketplaceAppDetail: [269, { universalIdentifier: [1, "String!"] }],
			__typename: [1]
		},
		GetApiKeyInput: {
			id: [3],
			__typename: [1]
		},
		AgentIdInput: {
			id: [3],
			__typename: [1]
		},
		LogicFunctionIdInput: {
			id: [219],
			__typename: [1]
		},
		PreviewMessageCampaignAudienceInput: {
			listId: [1],
			unsubscribeTopicId: [1],
			__typename: [1]
		},
		ListAppConnectionsInput: {
			providerName: [1],
			userWorkspaceId: [1],
			visibility: [1],
			__typename: [1]
		},
		EventLogQueryInput: {
			table: [344],
			filters: [345],
			first: [21],
			after: [1],
			__typename: [1]
		},
		EventLogTable: {},
		EventLogFiltersInput: {
			eventType: [1],
			userWorkspaceId: [1],
			dateRange: [346],
			recordId: [1],
			objectMetadataId: [1],
			__typename: [1]
		},
		EventLogDateRangeInput: {
			start: [4],
			end: [4],
			__typename: [1]
		},
		PieChartDataInput: {
			objectMetadataId: [3],
			configuration: [15],
			__typename: [1]
		},
		LineChartDataInput: {
			objectMetadataId: [3],
			configuration: [15],
			__typename: [1]
		},
		BarChartDataInput: {
			objectMetadataId: [3],
			configuration: [15],
			__typename: [1]
		},
		UsageAnalyticsInput: {
			periodStart: [4],
			periodEnd: [4],
			userWorkspaceId: [1],
			operationTypes: [351],
			__typename: [1]
		},
		UsageOperationType: {},
		Mutation: {
			addQueryToEventStream: [6, { input: [353, "AddQuerySubscriptionInput!"] }],
			removeQueryFromEventStream: [6, { input: [354, "RemoveQueryFromEventStreamInput!"] }],
			createManyNavigationMenuItems: [160, { inputs: [355, "[CreateNavigationMenuItemInput!]!"] }],
			createNavigationMenuItem: [160, { input: [355, "CreateNavigationMenuItemInput!"] }],
			updateManyNavigationMenuItems: [160, { inputs: [356, "[UpdateOneNavigationMenuItemInput!]!"] }],
			updateNavigationMenuItem: [160, { input: [356, "UpdateOneNavigationMenuItemInput!"] }],
			deleteManyNavigationMenuItems: [160, { ids: [3, "[UUID!]!"] }],
			deleteNavigationMenuItem: [160, { id: [3, "UUID!"] }],
			uploadEmailAttachmentFile: [131, { file: [358, "Upload!"] }],
			refreshEnterpriseValidityToken: [6],
			setEnterpriseKey: [128, { enterpriseKey: [1, "String!"] }],
			uploadAiChatFile: [131, { file: [358, "Upload!"] }],
			uploadWorkflowFile: [131, { file: [358, "Upload!"] }],
			uploadWorkspaceLogo: [131, { file: [358, "Upload!"] }],
			uploadWorkspaceMemberProfilePicture: [131, { file: [358, "Upload!"] }],
			uploadFilesFieldFile: [131, {
				file: [358, "Upload!"],
				fieldMetadataId: [1, "String!"]
			}],
			uploadFilesFieldFileByUniversalIdentifier: [131, {
				file: [358, "Upload!"],
				fieldMetadataUniversalIdentifier: [1, "String!"]
			}],
			createViewFilterGroup: [62, { input: [359, "CreateViewFilterGroupInput!"] }],
			updateViewFilterGroup: [62, {
				id: [1, "String!"],
				input: [360, "UpdateViewFilterGroupInput!"]
			}],
			deleteViewFilterGroup: [6, { id: [1, "String!"] }],
			destroyViewFilterGroup: [6, { id: [1, "String!"] }],
			createViewFilter: [64, { input: [361, "CreateViewFilterInput!"] }],
			updateViewFilter: [64, { input: [362, "UpdateViewFilterInput!"] }],
			deleteViewFilter: [64, { input: [364, "DeleteViewFilterInput!"] }],
			destroyViewFilter: [64, { input: [365, "DestroyViewFilterInput!"] }],
			createView: [70, { input: [366, "CreateViewInput!"] }],
			updateView: [70, {
				id: [1, "String!"],
				input: [367, "UpdateViewInput!"]
			}],
			deleteView: [6, { id: [1, "String!"] }],
			destroyView: [6, { id: [1, "String!"] }],
			upsertViewWidget: [70, { input: [368, "UpsertViewWidgetInput!"] }],
			createViewSort: [67, { input: [373, "CreateViewSortInput!"] }],
			updateViewSort: [67, { input: [374, "UpdateViewSortInput!"] }],
			deleteViewSort: [6, { input: [376, "DeleteViewSortInput!"] }],
			destroyViewSort: [6, { input: [377, "DestroyViewSortInput!"] }],
			updateViewField: [60, { input: [378, "UpdateViewFieldInput!"] }],
			createViewField: [60, { input: [380, "CreateViewFieldInput!"] }],
			createManyViewFields: [60, { inputs: [380, "[CreateViewFieldInput!]!"] }],
			deleteViewField: [60, { input: [381, "DeleteViewFieldInput!"] }],
			destroyViewField: [60, { input: [382, "DestroyViewFieldInput!"] }],
			updateViewFieldGroup: [69, { input: [383, "UpdateViewFieldGroupInput!"] }],
			createViewFieldGroup: [69, { input: [385, "CreateViewFieldGroupInput!"] }],
			createManyViewFieldGroups: [69, { inputs: [385, "[CreateViewFieldGroupInput!]!"] }],
			deleteViewFieldGroup: [69, { input: [386, "DeleteViewFieldGroupInput!"] }],
			destroyViewFieldGroup: [69, { input: [387, "DestroyViewFieldGroupInput!"] }],
			upsertFieldsWidget: [70, { input: [388, "UpsertFieldsWidgetInput!"] }],
			createApiKey: [2, { input: [391, "CreateApiKeyInput!"] }],
			updateApiKey: [2, { input: [392, "UpdateApiKeyInput!"] }],
			revokeApiKey: [2, { input: [393, "RevokeApiKeyInput!"] }],
			assignRoleToApiKey: [6, {
				apiKeyId: [3, "UUID!"],
				roleId: [3, "UUID!"]
			}],
			skipSyncEmailOnboardingStep: [156],
			skipBookOnboardingStep: [156],
			updateOneApplicationVariable: [6, {
				key: [1, "String!"],
				value: [1, "String!"],
				applicationId: [3, "UUID!"]
			}],
			checkoutSession: [153, {
				recurringInterval: [139, "SubscriptionInterval!"],
				plan: [135, "BillingPlanKey!"],
				requirePaymentMethod: [6, "Boolean!"],
				successUrlPath: [1]
			}],
			createSubscriptionPaymentIntent: [152, {
				recurringInterval: [139, "SubscriptionInterval!"],
				plan: [135, "BillingPlanKey!"],
				requirePaymentMethod: [6, "Boolean!"],
				successUrlPath: [1],
				idempotencyKey: [1, "String!"]
			}],
			switchSubscriptionInterval: [154],
			switchBillingPlan: [154],
			cancelSwitchBillingPlan: [154],
			cancelSwitchBillingInterval: [154],
			setResourceCreditSubscriptionPrice: [154, { priceId: [1, "String!"] }],
			endSubscriptionTrialPeriod: [149],
			cancelSwitchResourceCreditPrice: [154],
			deleteWorkspaceInvitation: [1, { appTokenId: [1, "String!"] }],
			resendWorkspaceInvitation: [158, { appTokenId: [1, "String!"] }],
			sendInvitations: [158, {
				emails: [1, "[String!]!"],
				roleId: [3]
			}],
			createApprovedAccessDomain: [130, { input: [394, "CreateApprovedAccessDomainInput!"] }],
			deleteApprovedAccessDomain: [6, { input: [395, "DeleteApprovedAccessDomainInput!"] }],
			validateApprovedAccessDomain: [130, { input: [396, "ValidateApprovedAccessDomainInput!"] }],
			createPageLayoutTab: [123, { input: [397, "CreatePageLayoutTabInput!"] }],
			updatePageLayoutTab: [123, {
				id: [1, "String!"],
				input: [398, "UpdatePageLayoutTabInput!"]
			}],
			destroyPageLayoutTab: [6, { id: [1, "String!"] }],
			createPageLayout: [124, { input: [399, "CreatePageLayoutInput!"] }],
			updatePageLayout: [124, {
				id: [1, "String!"],
				input: [400, "UpdatePageLayoutInput!"]
			}],
			destroyPageLayout: [6, { id: [1, "String!"] }],
			updatePageLayoutWithTabsAndWidgets: [124, {
				id: [1, "String!"],
				input: [401, "UpdatePageLayoutWithTabsInput!"]
			}],
			resetPageLayoutToDefault: [124, { id: [1, "String!"] }],
			resetPageLayoutWidgetToDefault: [85, { id: [1, "String!"] }],
			resetPageLayoutTabToDefault: [123, { id: [1, "String!"] }],
			createPageLayoutWidget: [85, { input: [405, "CreatePageLayoutWidgetInput!"] }],
			updatePageLayoutWidget: [85, {
				id: [1, "String!"],
				input: [406, "UpdatePageLayoutWidgetInput!"]
			}],
			destroyPageLayoutWidget: [6, { id: [1, "String!"] }],
			createOneAgent: [25, { input: [407, "CreateAgentInput!"] }],
			updateOneAgent: [25, { input: [408, "UpdateAgentInput!"] }],
			deleteOneAgent: [25, { input: [339, "AgentIdInput!"] }],
			createOneObject: [56, { input: [409, "CreateOneObjectInput!"] }],
			deleteOneObject: [56, { input: [411, "DeleteOneObjectInput!"] }],
			updateOneObject: [56, { input: [412, "UpdateOneObjectInput!"] }],
			createOneIndex: [47, { input: [414, "CreateOneIndexInput!"] }],
			deleteOneIndex: [47, { input: [417, "DeleteOneIndexInput!"] }],
			deleteOneLogicFunction: [41, { input: [340, "LogicFunctionIdInput!"] }],
			createOneLogicFunction: [41, { input: [418, "CreateLogicFunctionFromSourceInput!"] }],
			executeOneLogicFunction: [169, { input: [419, "ExecuteOneLogicFunctionInput!"] }],
			updateOneLogicFunction: [6, { input: [420, "UpdateLogicFunctionFromSourceInput!"] }],
			createCommandMenuItem: [35, { input: [422, "CreateCommandMenuItemInput!"] }],
			updateCommandMenuItem: [35, { input: [423, "UpdateCommandMenuItemInput!"] }],
			resetCommandMenuItem: [35, { id: [3, "UUID!"] }],
			deleteCommandMenuItem: [35, { id: [3, "UUID!"] }],
			createFrontComponent: [34, { input: [424, "CreateFrontComponentInput!"] }],
			updateFrontComponent: [34, { input: [425, "UpdateFrontComponentInput!"] }],
			deleteFrontComponent: [34, { id: [3, "UUID!"] }],
			sendEmailViaEmailingDomain: [284, { input: [427, "SendEmailViaDomainInput!"] }],
			sendMessageCampaign: [286, { input: [428, "SendMessageCampaignInput!"] }],
			createUnsubscribeTopic: [287, { input: [429, "CreateUnsubscribeTopicInput!"] }],
			updateUnsubscribeTopic: [287, { input: [430, "UpdateUnsubscribeTopicInput!"] }],
			deleteUnsubscribeTopic: [6, { id: [1, "String!"] }],
			updateMessageChannel: [274, { input: [431, "UpdateMessageChannelInput!"] }],
			createEmailGroupChannel: [282, { input: [433, "CreateEmailGroupChannelInput!"] }],
			deleteEmailGroupChannel: [274, { id: [3, "UUID!"] }],
			createEmailingDomain: [272, { input: [434, "CreateEmailingDomainInput!"] }],
			deleteEmailingDomain: [6, { id: [1, "String!"] }],
			verifyEmailingDomain: [272, { id: [1, "String!"] }],
			deleteConnectedAccount: [239, { id: [3, "UUID!"] }],
			updateWorkspaceMemberRole: [20, {
				workspaceMemberId: [3, "UUID!"],
				roleId: [3, "UUID!"]
			}],
			createOneRole: [29, { createRoleInput: [435, "CreateRoleInput!"] }],
			updateOneRole: [29, { updateRoleInput: [436, "UpdateRoleInput!"] }],
			deleteOneRole: [1, { roleId: [3, "UUID!"] }],
			upsertObjectPermissions: [16, { upsertObjectPermissionsInput: [438, "UpsertObjectPermissionsInput!"] }],
			upsertPermissionFlags: [27, { upsertPermissionFlagsInput: [440, "UpsertPermissionFlagsInput!"] }],
			upsertFieldPermissions: [26, { upsertFieldPermissionsInput: [441, "UpsertFieldPermissionsInput!"] }],
			upsertRowLevelPermissionPredicates: [234, { input: [443, "UpsertRowLevelPermissionPredicatesInput!"] }],
			assignRoleToAgent: [6, {
				agentId: [3, "UUID!"],
				roleId: [3, "UUID!"]
			}],
			removeRoleFromAgent: [6, { agentId: [3, "UUID!"] }],
			runAgent: [299, { input: [446, "RunAgentInput!"] }],
			createWebhook: [296, { input: [447, "CreateWebhookInput!"] }],
			updateWebhook: [296, { input: [448, "UpdateWebhookInput!"] }],
			deleteWebhook: [296, { id: [3, "UUID!"] }],
			createOneField: [44, { input: [450, "CreateOneFieldMetadataInput!"] }],
			updateOneField: [44, { input: [452, "UpdateOneFieldMetadataInput!"] }],
			deleteOneField: [44, { input: [454, "DeleteOneFieldInput!"] }],
			createViewGroup: [66, { input: [455, "CreateViewGroupInput!"] }],
			createManyViewGroups: [66, { inputs: [455, "[CreateViewGroupInput!]!"] }],
			updateViewGroup: [66, { input: [456, "UpdateViewGroupInput!"] }],
			updateManyViewGroups: [66, { inputs: [456, "[UpdateViewGroupInput!]!"] }],
			deleteViewGroup: [66, { input: [458, "DeleteViewGroupInput!"] }],
			destroyViewGroup: [66, { input: [459, "DestroyViewGroupInput!"] }],
			updateMessageFolder: [330, { input: [460, "UpdateMessageFolderInput!"] }],
			updateMessageFolders: [330, { input: [462, "UpdateMessageFoldersInput!"] }],
			updateCalendarChannel: [325, { input: [463, "UpdateCalendarChannelInput!"] }],
			createChatThread: [316],
			sendChatMessage: [320, {
				threadId: [3, "UUID!"],
				text: [1, "String!"],
				messageId: [3, "UUID!"],
				browsingContext: [15],
				modelId: [1],
				fileAttachments: [465, "[FileAttachmentInput!]"]
			}],
			stopAgentChatStream: [6, { threadId: [3, "UUID!"] }],
			renameChatThread: [316, {
				id: [3, "UUID!"],
				title: [1, "String!"]
			}],
			archiveChatThread: [316, { id: [3, "UUID!"] }],
			unarchiveChatThread: [316, { id: [3, "UUID!"] }],
			deleteChatThread: [6, { id: [3, "UUID!"] }],
			deleteQueuedChatMessage: [6, { messageId: [3, "UUID!"] }],
			createSkill: [314, { input: [466, "CreateSkillInput!"] }],
			updateSkill: [314, { input: [467, "UpdateSkillInput!"] }],
			deleteSkill: [314, { id: [3, "UUID!"] }],
			activateSkill: [314, { id: [3, "UUID!"] }],
			deactivateSkill: [314, { id: [3, "UUID!"] }],
			evaluateAgentTurn: [322, { turnId: [3, "UUID!"] }],
			runEvaluationInput: [323, {
				agentId: [3, "UUID!"],
				input: [1, "String!"]
			}],
			getAuthorizationUrlForSSO: [247, { input: [468, "GetAuthorizationUrlForSSOInput!"] }],
			getLoginTokenFromCredentials: [258, {
				email: [1, "String!"],
				password: [1, "String!"],
				captchaToken: [1],
				locale: [1],
				verifyEmailRedirectPath: [1],
				origin: [1, "String!"]
			}],
			signIn: [245, {
				email: [1, "String!"],
				password: [1, "String!"],
				captchaToken: [1],
				locale: [1],
				verifyEmailRedirectPath: [1]
			}],
			verifyEmailAndGetLoginToken: [253, {
				emailVerificationToken: [1, "String!"],
				email: [1, "String!"],
				captchaToken: [1],
				origin: [1, "String!"]
			}],
			verifyEmailAndGetWorkspaceAgnosticToken: [245, {
				emailVerificationToken: [1, "String!"],
				email: [1, "String!"],
				captchaToken: [1]
			}],
			getAuthTokensFromOTP: [257, {
				otp: [1, "String!"],
				loginToken: [1, "String!"],
				captchaToken: [1],
				origin: [1, "String!"]
			}],
			signUp: [245, {
				email: [1, "String!"],
				password: [1, "String!"],
				captchaToken: [1],
				locale: [1],
				verifyEmailRedirectPath: [1]
			}],
			signUpInWorkspace: [250, {
				email: [1, "String!"],
				password: [1, "String!"],
				workspaceId: [3],
				workspaceInviteHash: [1],
				workspacePersonalInviteToken: [1],
				captchaToken: [1],
				locale: [1],
				verifyEmailRedirectPath: [1]
			}],
			signUpInNewWorkspace: [250, { input: [469] }],
			uploadNewWorkspaceLogo: [131, {
				workspaceId: [1, "String!"],
				file: [358, "Upload!"]
			}],
			generateTransientToken: [251],
			getAuthTokensFromLoginToken: [257, {
				loginToken: [1, "String!"],
				origin: [1, "String!"]
			}],
			authorizeApp: [243, {
				clientId: [1, "String!"],
				codeChallenge: [1],
				redirectUrl: [1, "String!"],
				state: [1],
				scope: [1]
			}],
			renewToken: [257, { appToken: [1, "String!"] }],
			generateApiKeyToken: [256, {
				apiKeyId: [3, "UUID!"],
				expiresAt: [1, "String!"]
			}],
			generatePlaygroundToken: [32],
			emailPasswordResetLink: [246, {
				email: [1, "String!"],
				workspaceId: [3]
			}],
			updatePasswordViaResetToken: [248, {
				passwordResetToken: [1, "String!"],
				newPassword: [1, "String!"]
			}],
			createApplicationRegistration: [200, { input: [470, "CreateApplicationRegistrationInput!"] }],
			updateApplicationRegistration: [7, { input: [471, "UpdateApplicationRegistrationInput!"] }],
			deleteApplicationRegistration: [6, { id: [1, "String!"] }],
			rotateApplicationRegistrationClientSecret: [202, { id: [1, "String!"] }],
			createApplicationRegistrationVariable: [5, { input: [473, "CreateApplicationRegistrationVariableInput!"] }],
			updateApplicationRegistrationVariable: [5, { input: [474, "UpdateApplicationRegistrationVariableInput!"] }],
			deleteApplicationRegistrationVariable: [6, { id: [1, "String!"] }],
			uploadAppTarball: [7, {
				file: [358, "Upload!"],
				universalIdentifier: [1]
			}],
			transferApplicationRegistrationOwnership: [7, {
				applicationRegistrationId: [1, "String!"],
				targetWorkspaceSubdomain: [1, "String!"]
			}],
			initiateOTPProvisioning: [241, {
				loginToken: [1, "String!"],
				origin: [1, "String!"]
			}],
			initiateOTPProvisioningForAuthenticatedUser: [241],
			deleteTwoFactorAuthenticationMethod: [240, { twoFactorAuthenticationMethodId: [3, "UUID!"] }],
			verifyTwoFactorAuthenticationMethodForAuthenticatedUser: [242, { otp: [1, "String!"] }],
			deleteUser: [79],
			deleteUserFromWorkspace: [17, { workspaceMemberIdToDelete: [1, "String!"] }],
			updateWorkspaceMemberSettings: [6, { input: [476, "UpdateWorkspaceMemberSettingsInput!"] }],
			updateUserEmail: [6, {
				newEmail: [1, "String!"],
				verifyEmailRedirectPath: [1]
			}],
			resendEmailVerificationToken: [220, {
				email: [1, "String!"],
				origin: [1, "String!"]
			}],
			activateWorkspace: [76, { data: [477, "ActivateWorkspaceInput!"] }],
			updateWorkspace: [76, { data: [478, "UpdateWorkspaceInput!"] }],
			deleteCurrentWorkspace: [76],
			checkCustomDomainValidRecords: [233],
			runWorkspaceMigration: [6, { workspaceMigration: [479, "WorkspaceMigrationInput!"] }],
			uninstallApplication: [6, { universalIdentifier: [1, "String!"] }],
			createOIDCIdentityProvider: [225, { input: [482, "SetupOIDCSsoInput!"] }],
			createSAMLIdentityProvider: [225, { input: [483, "SetupSAMLSsoInput!"] }],
			deleteSSOIdentityProvider: [221, { input: [484, "DeleteSsoInput!"] }],
			editSSOIdentityProvider: [222, { input: [485, "EditSsoInput!"] }],
			createObjectEvent: [310, {
				event: [1, "String!"],
				recordId: [3, "UUID!"],
				objectMetadataId: [3, "UUID!"],
				properties: [15]
			}],
			trackAnalytics: [310, {
				type: [486, "AnalyticsType!"],
				name: [1],
				event: [1],
				properties: [15]
			}],
			duplicateDashboard: [308, { id: [3, "UUID!"] }],
			impersonate: [261, {
				userId: [3, "UUID!"],
				workspaceId: [3, "UUID!"]
			}],
			sendEmail: [309, { input: [487, "SendEmailInput!"] }],
			startChannelSync: [300, { connectedAccountId: [3, "UUID!"] }],
			saveImapSmtpCaldavAccount: [295, {
				handle: [1, "String!"],
				connectionParameters: [489, "EmailAccountConnectionParameters!"],
				id: [3]
			}],
			updateLabPublicFeatureFlag: [171, { input: [491, "UpdateLabPublicFeatureFlagInput!"] }],
			createPublicDomain: [270, {
				domain: [1, "String!"],
				applicationId: [1, "String!"]
			}],
			deletePublicDomain: [6, { domain: [1, "String!"] }],
			checkPublicDomainValidRecords: [233, { domain: [1, "String!"] }],
			createOneAppToken: [78, { input: [492, "CreateOneAppTokenInput!"] }],
			installMarketplaceApp: [6, {
				universalIdentifier: [1, "String!"],
				version: [1]
			}],
			installApplication: [59, {
				universalIdentifier: [1, "String!"],
				version: [1]
			}],
			syncMarketplaceCatalog: [6],
			createDevelopmentApplication: [265, {
				universalIdentifier: [1, "String!"],
				name: [1, "String!"]
			}],
			generateApplicationToken: [33, { applicationId: [3, "UUID!"] }],
			syncApplication: [266, {
				manifest: [15, "JSON!"],
				dryRun: [6]
			}],
			uploadApplicationFile: [267, {
				file: [358, "Upload!"],
				applicationUniversalIdentifier: [1, "String!"],
				fileFolder: [494, "FileFolder!"],
				filePath: [1, "String!"]
			}],
			upgradeApplication: [6, {
				appRegistrationId: [1, "String!"],
				targetVersion: [1, "String!"]
			}],
			renewApplicationToken: [33, { applicationRefreshToken: [1, "String!"] }],
			__typename: [1]
		},
		AddQuerySubscriptionInput: {
			eventStreamId: [1],
			queryId: [1],
			operationSignature: [15],
			__typename: [1]
		},
		RemoveQueryFromEventStreamInput: {
			eventStreamId: [1],
			queryId: [1],
			__typename: [1]
		},
		CreateNavigationMenuItemInput: {
			id: [3],
			userWorkspaceId: [3],
			targetRecordId: [3],
			targetObjectMetadataId: [3],
			viewId: [3],
			type: [161],
			name: [1],
			link: [1],
			icon: [1],
			color: [1],
			folderId: [3],
			pageLayoutId: [3],
			position: [11],
			__typename: [1]
		},
		UpdateOneNavigationMenuItemInput: {
			id: [3],
			update: [357],
			__typename: [1]
		},
		UpdateNavigationMenuItemInput: {
			folderId: [3],
			position: [11],
			name: [1],
			link: [1],
			icon: [1],
			color: [1],
			pageLayoutId: [3],
			__typename: [1]
		},
		Upload: {},
		CreateViewFilterGroupInput: {
			id: [3],
			parentViewFilterGroupId: [3],
			logicalOperator: [63],
			positionInViewFilterGroup: [11],
			viewId: [3],
			__typename: [1]
		},
		UpdateViewFilterGroupInput: {
			id: [3],
			parentViewFilterGroupId: [3],
			logicalOperator: [63],
			positionInViewFilterGroup: [11],
			viewId: [3],
			__typename: [1]
		},
		CreateViewFilterInput: {
			id: [3],
			fieldMetadataId: [3],
			operand: [65],
			value: [15],
			viewFilterGroupId: [3],
			positionInViewFilterGroup: [11],
			subFieldName: [1],
			relationTargetFieldMetadataId: [3],
			viewId: [3],
			__typename: [1]
		},
		UpdateViewFilterInput: {
			id: [3],
			update: [363],
			__typename: [1]
		},
		UpdateViewFilterInputUpdates: {
			fieldMetadataId: [3],
			operand: [65],
			value: [15],
			viewFilterGroupId: [3],
			positionInViewFilterGroup: [11],
			subFieldName: [1],
			relationTargetFieldMetadataId: [3],
			__typename: [1]
		},
		DeleteViewFilterInput: {
			id: [3],
			__typename: [1]
		},
		DestroyViewFilterInput: {
			id: [3],
			__typename: [1]
		},
		CreateViewInput: {
			id: [3],
			name: [1],
			objectMetadataId: [3],
			type: [71],
			key: [72],
			icon: [1],
			position: [11],
			isCompact: [6],
			shouldHideEmptyGroups: [6],
			kanbanColumnWidth: [21],
			openRecordIn: [73],
			kanbanAggregateOperation: [61],
			kanbanAggregateOperationFieldMetadataId: [3],
			anyFieldFilterValue: [1],
			calendarLayout: [74],
			calendarFieldMetadataId: [3],
			mainGroupByFieldMetadataId: [3],
			visibility: [75],
			__typename: [1]
		},
		UpdateViewInput: {
			id: [3],
			name: [1],
			type: [71],
			icon: [1],
			position: [11],
			isCompact: [6],
			openRecordIn: [73],
			kanbanAggregateOperation: [61],
			kanbanAggregateOperationFieldMetadataId: [3],
			anyFieldFilterValue: [1],
			calendarLayout: [74],
			calendarFieldMetadataId: [3],
			visibility: [75],
			mainGroupByFieldMetadataId: [3],
			shouldHideEmptyGroups: [6],
			kanbanColumnWidth: [21],
			__typename: [1]
		},
		UpsertViewWidgetInput: {
			widgetId: [3],
			viewFields: [369],
			viewFilters: [370],
			viewFilterGroups: [371],
			viewSorts: [372],
			__typename: [1]
		},
		UpsertViewWidgetViewFieldInput: {
			viewFieldId: [3],
			fieldMetadataId: [3],
			isVisible: [6],
			position: [11],
			size: [11],
			__typename: [1]
		},
		UpsertViewWidgetViewFilterInput: {
			id: [3],
			fieldMetadataId: [3],
			operand: [65],
			value: [15],
			viewFilterGroupId: [3],
			positionInViewFilterGroup: [11],
			subFieldName: [1],
			relationTargetFieldMetadataId: [3],
			__typename: [1]
		},
		UpsertViewWidgetViewFilterGroupInput: {
			id: [3],
			parentViewFilterGroupId: [3],
			logicalOperator: [63],
			positionInViewFilterGroup: [11],
			__typename: [1]
		},
		UpsertViewWidgetViewSortInput: {
			id: [3],
			fieldMetadataId: [3],
			direction: [68],
			__typename: [1]
		},
		CreateViewSortInput: {
			id: [3],
			fieldMetadataId: [3],
			direction: [68],
			subFieldName: [1],
			viewId: [3],
			__typename: [1]
		},
		UpdateViewSortInput: {
			id: [3],
			update: [375],
			__typename: [1]
		},
		UpdateViewSortInputUpdates: {
			direction: [68],
			subFieldName: [1],
			__typename: [1]
		},
		DeleteViewSortInput: {
			id: [3],
			__typename: [1]
		},
		DestroyViewSortInput: {
			id: [3],
			__typename: [1]
		},
		UpdateViewFieldInput: {
			id: [3],
			update: [379],
			__typename: [1]
		},
		UpdateViewFieldInputUpdates: {
			isVisible: [6],
			size: [11],
			position: [11],
			aggregateOperation: [61],
			viewFieldGroupId: [3],
			__typename: [1]
		},
		CreateViewFieldInput: {
			id: [3],
			fieldMetadataId: [3],
			viewId: [3],
			isVisible: [6],
			size: [11],
			position: [11],
			aggregateOperation: [61],
			viewFieldGroupId: [3],
			__typename: [1]
		},
		DeleteViewFieldInput: {
			id: [3],
			__typename: [1]
		},
		DestroyViewFieldInput: {
			id: [3],
			__typename: [1]
		},
		UpdateViewFieldGroupInput: {
			id: [3],
			update: [384],
			__typename: [1]
		},
		UpdateViewFieldGroupInputUpdates: {
			name: [1],
			position: [11],
			isVisible: [6],
			deletedAt: [1],
			__typename: [1]
		},
		CreateViewFieldGroupInput: {
			id: [3],
			name: [1],
			viewId: [3],
			position: [11],
			isVisible: [6],
			__typename: [1]
		},
		DeleteViewFieldGroupInput: {
			id: [3],
			__typename: [1]
		},
		DestroyViewFieldGroupInput: {
			id: [3],
			__typename: [1]
		},
		UpsertFieldsWidgetInput: {
			widgetId: [3],
			groups: [389],
			fields: [390],
			__typename: [1]
		},
		UpsertFieldsWidgetGroupInput: {
			id: [3],
			name: [1],
			position: [11],
			isVisible: [6],
			fields: [390],
			__typename: [1]
		},
		UpsertFieldsWidgetFieldInput: {
			viewFieldId: [3],
			fieldMetadataId: [3],
			isVisible: [6],
			position: [11],
			__typename: [1]
		},
		CreateApiKeyInput: {
			name: [1],
			expiresAt: [1],
			revokedAt: [1],
			roleId: [3],
			__typename: [1]
		},
		UpdateApiKeyInput: {
			id: [3],
			name: [1],
			expiresAt: [1],
			revokedAt: [1],
			__typename: [1]
		},
		RevokeApiKeyInput: {
			id: [3],
			__typename: [1]
		},
		CreateApprovedAccessDomainInput: {
			domain: [1],
			email: [1],
			__typename: [1]
		},
		DeleteApprovedAccessDomainInput: {
			id: [3],
			__typename: [1]
		},
		ValidateApprovedAccessDomainInput: {
			validationToken: [1],
			approvedAccessDomainId: [3],
			__typename: [1]
		},
		CreatePageLayoutTabInput: {
			title: [1],
			position: [11],
			pageLayoutId: [3],
			layoutMode: [89],
			__typename: [1]
		},
		UpdatePageLayoutTabInput: {
			title: [1],
			position: [11],
			icon: [1],
			layoutMode: [89],
			__typename: [1]
		},
		CreatePageLayoutInput: {
			name: [1],
			type: [125],
			objectMetadataId: [3],
			__typename: [1]
		},
		UpdatePageLayoutInput: {
			name: [1],
			type: [125],
			objectMetadataId: [3],
			__typename: [1]
		},
		UpdatePageLayoutWithTabsInput: {
			name: [1],
			type: [125],
			objectMetadataId: [3],
			tabs: [402],
			__typename: [1]
		},
		UpdatePageLayoutTabWithWidgetsInput: {
			id: [3],
			title: [1],
			position: [11],
			icon: [1],
			layoutMode: [89],
			widgets: [403],
			__typename: [1]
		},
		UpdatePageLayoutWidgetWithIdInput: {
			id: [3],
			pageLayoutTabId: [3],
			title: [1],
			type: [86],
			objectMetadataId: [3],
			gridPosition: [404],
			position: [15],
			configuration: [15],
			conditionalDisplay: [15],
			conditionalAvailabilityExpression: [1],
			__typename: [1]
		},
		GridPositionInput: {
			row: [11],
			column: [11],
			rowSpan: [11],
			columnSpan: [11],
			__typename: [1]
		},
		CreatePageLayoutWidgetInput: {
			pageLayoutTabId: [3],
			title: [1],
			type: [86],
			objectMetadataId: [3],
			gridPosition: [404],
			position: [15],
			configuration: [15],
			__typename: [1]
		},
		UpdatePageLayoutWidgetInput: {
			pageLayoutTabId: [3],
			title: [1],
			type: [86],
			objectMetadataId: [3],
			gridPosition: [404],
			position: [15],
			configuration: [15],
			conditionalDisplay: [15],
			conditionalAvailabilityExpression: [1],
			__typename: [1]
		},
		CreateAgentInput: {
			name: [1],
			label: [1],
			icon: [1],
			description: [1],
			prompt: [1],
			modelId: [1],
			roleId: [3],
			responseFormat: [15],
			modelConfiguration: [15],
			evaluationInputs: [1],
			__typename: [1]
		},
		UpdateAgentInput: {
			id: [3],
			name: [1],
			label: [1],
			icon: [1],
			description: [1],
			prompt: [1],
			modelId: [1],
			roleId: [3],
			responseFormat: [15],
			modelConfiguration: [15],
			evaluationInputs: [1],
			__typename: [1]
		},
		CreateOneObjectInput: {
			object: [410],
			__typename: [1]
		},
		CreateObjectInput: {
			nameSingular: [1],
			namePlural: [1],
			labelSingular: [1],
			labelPlural: [1],
			description: [1],
			icon: [1],
			shortcut: [1],
			color: [1],
			skipNameField: [6],
			isRemote: [6],
			primaryKeyColumnType: [1],
			primaryKeyFieldMetadataSettings: [15],
			isLabelSyncedWithName: [6],
			__typename: [1]
		},
		DeleteOneObjectInput: {
			id: [3],
			__typename: [1]
		},
		UpdateOneObjectInput: {
			update: [413],
			id: [3],
			__typename: [1]
		},
		UpdateObjectPayload: {
			labelSingular: [1],
			labelPlural: [1],
			nameSingular: [1],
			namePlural: [1],
			description: [1],
			icon: [1],
			shortcut: [1],
			color: [1],
			isActive: [6],
			labelIdentifierFieldMetadataId: [3],
			imageIdentifierFieldMetadataId: [3],
			isLabelSyncedWithName: [6],
			isSearchable: [6],
			__typename: [1]
		},
		CreateOneIndexInput: {
			index: [415],
			__typename: [1]
		},
		CreateIndexInput: {
			objectMetadataId: [3],
			fields: [416],
			indexType: [48],
			__typename: [1]
		},
		CreateIndexFieldInput: {
			fieldMetadataId: [3],
			subFieldName: [1],
			__typename: [1]
		},
		DeleteOneIndexInput: {
			id: [3],
			__typename: [1]
		},
		CreateLogicFunctionFromSourceInput: {
			id: [3],
			universalIdentifier: [3],
			name: [1],
			description: [1],
			timeoutSeconds: [11],
			source: [15],
			cronTriggerSettings: [15],
			databaseEventTriggerSettings: [15],
			httpRouteTriggerSettings: [15],
			serverRouteTriggerSettings: [15],
			toolTriggerSettings: [15],
			workflowActionTriggerSettings: [15],
			__typename: [1]
		},
		ExecuteOneLogicFunctionInput: {
			id: [3],
			payload: [15],
			__typename: [1]
		},
		UpdateLogicFunctionFromSourceInput: {
			id: [3],
			update: [421],
			__typename: [1]
		},
		UpdateLogicFunctionFromSourceInputUpdates: {
			name: [1],
			description: [1],
			timeoutSeconds: [11],
			sourceHandlerCode: [1],
			handlerName: [1],
			sourceHandlerPath: [1],
			cronTriggerSettings: [15],
			databaseEventTriggerSettings: [15],
			httpRouteTriggerSettings: [15],
			toolTriggerSettings: [15],
			workflowActionTriggerSettings: [15],
			__typename: [1]
		},
		CreateCommandMenuItemInput: {
			workflowVersionId: [3],
			frontComponentId: [3],
			engineComponentKey: [36],
			label: [1],
			icon: [1],
			shortLabel: [1],
			position: [11],
			isPinned: [6],
			availabilityType: [37],
			hotKeys: [1],
			conditionalAvailabilityExpression: [1],
			availabilityObjectMetadataId: [3],
			payload: [15],
			pageLayoutId: [3],
			__typename: [1]
		},
		UpdateCommandMenuItemInput: {
			id: [3],
			label: [1],
			icon: [1],
			shortLabel: [1],
			position: [11],
			isPinned: [6],
			availabilityType: [37],
			availabilityObjectMetadataId: [3],
			engineComponentKey: [36],
			hotKeys: [1],
			pageLayoutId: [3],
			__typename: [1]
		},
		CreateFrontComponentInput: {
			id: [3],
			name: [1],
			description: [1],
			sourceComponentPath: [1],
			builtComponentPath: [1],
			componentName: [1],
			builtComponentChecksum: [1],
			__typename: [1]
		},
		UpdateFrontComponentInput: {
			id: [3],
			update: [426],
			__typename: [1]
		},
		UpdateFrontComponentInputUpdates: {
			name: [1],
			description: [1],
			__typename: [1]
		},
		SendEmailViaDomainInput: {
			emailingDomainId: [1],
			to: [1],
			cc: [1],
			bcc: [1],
			subject: [1],
			text: [1],
			html: [1],
			from: [1],
			replyTo: [1],
			__typename: [1]
		},
		SendMessageCampaignInput: {
			listId: [1],
			unsubscribeTopicId: [1],
			subject: [1],
			body: [1],
			fromAddress: [1],
			__typename: [1]
		},
		CreateUnsubscribeTopicInput: {
			name: [1],
			description: [1],
			visibility: [288],
			__typename: [1]
		},
		UpdateUnsubscribeTopicInput: {
			id: [1],
			name: [1],
			description: [1],
			visibility: [288],
			__typename: [1]
		},
		UpdateMessageChannelInput: {
			id: [3],
			update: [432],
			__typename: [1]
		},
		UpdateMessageChannelInputUpdates: {
			visibility: [275],
			isContactAutoCreationEnabled: [6],
			contactAutoCreationPolicy: [277],
			messageFolderImportPolicy: [278],
			isSyncEnabled: [6],
			excludeNonProfessionalEmails: [6],
			excludeGroupEmails: [6],
			__typename: [1]
		},
		CreateEmailGroupChannelInput: {
			handle: [1],
			__typename: [1]
		},
		CreateEmailingDomainInput: {
			domain: [1],
			__typename: [1]
		},
		CreateRoleInput: {
			id: [1],
			label: [1],
			description: [1],
			icon: [1],
			canUpdateAllSettings: [6],
			canAccessAllTools: [6],
			canReadAllObjectRecords: [6],
			canUpdateAllObjectRecords: [6],
			canSoftDeleteAllObjectRecords: [6],
			canDestroyAllObjectRecords: [6],
			canBeAssignedToUsers: [6],
			canBeAssignedToAgents: [6],
			canBeAssignedToApiKeys: [6],
			__typename: [1]
		},
		UpdateRoleInput: {
			update: [437],
			id: [3],
			__typename: [1]
		},
		UpdateRolePayload: {
			label: [1],
			description: [1],
			icon: [1],
			canUpdateAllSettings: [6],
			canAccessAllTools: [6],
			canReadAllObjectRecords: [6],
			canUpdateAllObjectRecords: [6],
			canSoftDeleteAllObjectRecords: [6],
			canDestroyAllObjectRecords: [6],
			canBeAssignedToUsers: [6],
			canBeAssignedToAgents: [6],
			canBeAssignedToApiKeys: [6],
			__typename: [1]
		},
		UpsertObjectPermissionsInput: {
			roleId: [3],
			objectPermissions: [439],
			__typename: [1]
		},
		ObjectPermissionInput: {
			objectMetadataId: [3],
			canReadObjectRecords: [6],
			canUpdateObjectRecords: [6],
			canSoftDeleteObjectRecords: [6],
			canDestroyObjectRecords: [6],
			__typename: [1]
		},
		UpsertPermissionFlagsInput: {
			roleId: [3],
			permissionFlagKeys: [1],
			__typename: [1]
		},
		UpsertFieldPermissionsInput: {
			roleId: [3],
			fieldPermissions: [442],
			__typename: [1]
		},
		FieldPermissionInput: {
			objectMetadataId: [3],
			fieldMetadataId: [3],
			canReadFieldValue: [6],
			canUpdateFieldValue: [6],
			__typename: [1]
		},
		UpsertRowLevelPermissionPredicatesInput: {
			roleId: [3],
			objectMetadataId: [3],
			predicates: [444],
			predicateGroups: [445],
			__typename: [1]
		},
		RowLevelPermissionPredicateInput: {
			id: [3],
			fieldMetadataId: [3],
			operand: [14],
			value: [15],
			subFieldName: [1],
			workspaceMemberFieldMetadataId: [1],
			workspaceMemberSubFieldName: [1],
			rowLevelPermissionPredicateGroupId: [3],
			positionInRowLevelPermissionPredicateGroup: [11],
			__typename: [1]
		},
		RowLevelPermissionPredicateGroupInput: {
			id: [3],
			objectMetadataId: [3],
			parentRowLevelPermissionPredicateGroupId: [3],
			logicalOperator: [12],
			positionInRowLevelPermissionPredicateGroup: [11],
			__typename: [1]
		},
		RunAgentInput: {
			agentUniversalIdentifier: [1],
			prompt: [1],
			__typename: [1]
		},
		CreateWebhookInput: {
			id: [3],
			targetUrl: [1],
			operations: [1],
			description: [1],
			secret: [1],
			__typename: [1]
		},
		UpdateWebhookInput: {
			id: [3],
			update: [449],
			__typename: [1]
		},
		UpdateWebhookInputUpdates: {
			targetUrl: [1],
			operations: [1],
			description: [1],
			secret: [1],
			__typename: [1]
		},
		CreateOneFieldMetadataInput: {
			field: [451],
			__typename: [1]
		},
		CreateFieldInput: {
			type: [45],
			name: [1],
			label: [1],
			description: [1],
			icon: [1],
			isActive: [6],
			isSystem: [6],
			isUIEditable: [6],
			isUIReadOnly: [6],
			isNullable: [6],
			isUnique: [6],
			defaultValue: [15],
			options: [15],
			settings: [15],
			objectMetadataId: [3],
			isLabelSyncedWithName: [6],
			isRemoteCreation: [6],
			relationCreationPayload: [15],
			morphRelationsCreationPayload: [15],
			__typename: [1]
		},
		UpdateOneFieldMetadataInput: {
			id: [3],
			update: [453],
			__typename: [1]
		},
		UpdateFieldInput: {
			universalIdentifier: [1],
			name: [1],
			label: [1],
			description: [1],
			icon: [1],
			isActive: [6],
			isSystem: [6],
			isUIEditable: [6],
			isUIReadOnly: [6],
			isNullable: [6],
			isUnique: [6],
			defaultValue: [15],
			options: [15],
			settings: [15],
			objectMetadataId: [3],
			isLabelSyncedWithName: [6],
			morphRelationsUpdatePayload: [15],
			__typename: [1]
		},
		DeleteOneFieldInput: {
			id: [3],
			__typename: [1]
		},
		CreateViewGroupInput: {
			id: [3],
			isVisible: [6],
			fieldValue: [1],
			position: [11],
			viewId: [3],
			__typename: [1]
		},
		UpdateViewGroupInput: {
			id: [3],
			update: [457],
			__typename: [1]
		},
		UpdateViewGroupInputUpdates: {
			fieldMetadataId: [3],
			isVisible: [6],
			fieldValue: [1],
			position: [11],
			__typename: [1]
		},
		DeleteViewGroupInput: {
			id: [3],
			__typename: [1]
		},
		DestroyViewGroupInput: {
			id: [3],
			__typename: [1]
		},
		UpdateMessageFolderInput: {
			id: [3],
			update: [461],
			__typename: [1]
		},
		UpdateMessageFolderInputUpdates: {
			isSynced: [6],
			__typename: [1]
		},
		UpdateMessageFoldersInput: {
			ids: [3],
			update: [461],
			__typename: [1]
		},
		UpdateCalendarChannelInput: {
			id: [3],
			update: [464],
			__typename: [1]
		},
		UpdateCalendarChannelInputUpdates: {
			visibility: [328],
			isContactAutoCreationEnabled: [6],
			contactAutoCreationPolicy: [329],
			isSyncEnabled: [6],
			__typename: [1]
		},
		FileAttachmentInput: {
			id: [3],
			filename: [1],
			__typename: [1]
		},
		CreateSkillInput: {
			id: [3],
			name: [1],
			label: [1],
			icon: [1],
			description: [1],
			content: [1],
			__typename: [1]
		},
		UpdateSkillInput: {
			id: [3],
			name: [1],
			label: [1],
			icon: [1],
			description: [1],
			content: [1],
			isActive: [6],
			__typename: [1]
		},
		GetAuthorizationUrlForSSOInput: {
			identityProviderId: [3],
			workspaceInviteHash: [1],
			__typename: [1]
		},
		SignUpInNewWorkspaceInput: {
			displayName: [1],
			subdomain: [1],
			__typename: [1]
		},
		CreateApplicationRegistrationInput: {
			name: [1],
			universalIdentifier: [1],
			oAuthRedirectUris: [1],
			oAuthScopes: [1],
			__typename: [1]
		},
		UpdateApplicationRegistrationInput: {
			id: [1],
			update: [472],
			__typename: [1]
		},
		UpdateApplicationRegistrationPayload: {
			name: [1],
			oAuthRedirectUris: [1],
			oAuthScopes: [1],
			isListed: [6],
			__typename: [1]
		},
		CreateApplicationRegistrationVariableInput: {
			applicationRegistrationId: [1],
			key: [1],
			value: [1],
			description: [1],
			isSecret: [6],
			__typename: [1]
		},
		UpdateApplicationRegistrationVariableInput: {
			id: [1],
			update: [475],
			__typename: [1]
		},
		UpdateApplicationRegistrationVariablePayload: {
			value: [1],
			resetValue: [6],
			description: [1],
			__typename: [1]
		},
		UpdateWorkspaceMemberSettingsInput: {
			workspaceMemberId: [3],
			update: [15],
			__typename: [1]
		},
		ActivateWorkspaceInput: {
			displayName: [1],
			__typename: [1]
		},
		UpdateWorkspaceInput: {
			subdomain: [1],
			customDomain: [1],
			displayName: [1],
			logo: [1],
			inviteHash: [1],
			isPublicInviteLinkEnabled: [6],
			allowImpersonation: [6],
			isGoogleAuthEnabled: [6],
			isMicrosoftAuthEnabled: [6],
			isPasswordAuthEnabled: [6],
			isGoogleAuthBypassEnabled: [6],
			isMicrosoftAuthBypassEnabled: [6],
			isPasswordAuthBypassEnabled: [6],
			defaultRoleId: [3],
			isTwoFactorAuthenticationEnforced: [6],
			trashRetentionDays: [11],
			eventLogRetentionDays: [11],
			fastModel: [1],
			smartModel: [1],
			aiAdditionalInstructions: [1],
			editableProfileFields: [1],
			enabledAiModelIds: [1],
			useRecommendedModels: [6],
			isInternalMessagesImportEnabled: [6],
			__typename: [1]
		},
		WorkspaceMigrationInput: {
			actions: [480],
			__typename: [1]
		},
		WorkspaceMigrationDeleteActionInput: {
			type: [481],
			metadataName: [333],
			universalIdentifier: [1],
			__typename: [1]
		},
		WorkspaceMigrationActionType: {},
		SetupOIDCSsoInput: {
			name: [1],
			issuer: [1],
			clientID: [1],
			clientSecret: [1],
			__typename: [1]
		},
		SetupSAMLSsoInput: {
			name: [1],
			issuer: [1],
			id: [3],
			ssoURL: [1],
			certificate: [1],
			fingerprint: [1],
			__typename: [1]
		},
		DeleteSsoInput: {
			identityProviderId: [3],
			__typename: [1]
		},
		EditSsoInput: {
			id: [3],
			status: [178],
			__typename: [1]
		},
		AnalyticsType: {},
		SendEmailInput: {
			connectedAccountId: [1],
			to: [1],
			cc: [1],
			bcc: [1],
			subject: [1],
			body: [1],
			inReplyTo: [1],
			files: [488],
			__typename: [1]
		},
		SendEmailAttachmentInput: {
			id: [1],
			name: [1],
			__typename: [1]
		},
		EmailAccountConnectionParameters: {
			IMAP: [490],
			SMTP: [490],
			CALDAV: [490],
			__typename: [1]
		},
		ConnectionParametersInput: {
			host: [1],
			port: [11],
			username: [1],
			password: [1],
			connectionSecurity: [237],
			__typename: [1]
		},
		UpdateLabPublicFeatureFlagInput: {
			publicFeatureFlag: [1],
			value: [6],
			__typename: [1]
		},
		CreateOneAppTokenInput: {
			appToken: [493],
			__typename: [1]
		},
		CreateAppTokenInput: {
			expiresAt: [4],
			__typename: [1]
		},
		FileFolder: {},
		Subscription: {
			onEventSubscription: [168, { eventStreamId: [1, "String!"] }],
			logicFunctionLogs: [235, { input: [496, "LogicFunctionLogsInput!"] }],
			onAgentChatEvent: [321, { threadId: [3, "UUID!"] }],
			eventLogsLive: [311, { table: [344, "EventLogTable!"] }],
			__typename: [1]
		},
		LogicFunctionLogsInput: {
			applicationId: [3],
			applicationUniversalIdentifier: [3],
			name: [1],
			id: [3],
			universalIdentifier: [3],
			__typename: [1]
		}
	}
}, ie = /* @__PURE__ */ e({
	enumAggregateOperations: () => al,
	enumAllMetadataName: () => au,
	enumAnalyticsType: () => lu,
	enumApplicationRegistrationSourceType: () => qc,
	enumAxisNameDisplay: () => Sl,
	enumBarChartGroupMode: () => Cl,
	enumBarChartLayout: () => wl,
	enumBillingEntitlementKey: () => Ul,
	enumBillingPlanKey: () => Dl,
	enumBillingProductKey: () => kl,
	enumBillingUsageType: () => Ol,
	enumCalendarChannelContactAutoCreationPolicy: () => ru,
	enumCalendarChannelSyncStage: () => tu,
	enumCalendarChannelSyncStatus: () => eu,
	enumCalendarChannelVisibility: () => nu,
	enumCaptchaDriverType: () => Vl,
	enumChartNumberFormat: () => yl,
	enumCommandMenuItemAvailabilityType: () => tl,
	enumDatabaseEventAction: () => Pl,
	enumEmailConnectionSecurity: () => Wl,
	enumEmailingDomainStatus: () => Gl,
	enumEngineComponentKey: () => el,
	enumEventLogTable: () => ou,
	enumFeatureFlagKey: () => Il,
	enumFieldDisplayMode: () => Tl,
	enumFieldMetadataType: () => rl,
	enumFileFolder: () => uu,
	enumGraphOrderBy: () => xl,
	enumIdentityProviderType: () => Ll,
	enumIndexType: () => il,
	enumLogicFunctionExecutionMode: () => nl,
	enumLogicFunctionExecutionStatus: () => Fl,
	enumMessageChannelContactAutoCreationPolicy: () => Jl,
	enumMessageChannelPendingGroupEmailsAction: () => Xl,
	enumMessageChannelSyncStage: () => Ql,
	enumMessageChannelSyncStatus: () => Zl,
	enumMessageChannelType: () => ql,
	enumMessageChannelVisibility: () => Kl,
	enumMessageFolderImportPolicy: () => Yl,
	enumMessageFolderPendingSyncAction: () => iu,
	enumMetadataEventAction: () => Nl,
	enumModelFamily: () => zl,
	enumNavigationMenuItemType: () => Ml,
	enumObjectRecordGroupByDateGranularity: () => bl,
	enumOnboardingStatus: () => hl,
	enumPageLayoutTabLayoutMode: () => _l,
	enumPageLayoutType: () => El,
	enumPermissionFlagType: () => Xc,
	enumRelationType: () => Hl,
	enumRowLevelPermissionPredicateGroupLogicalOperator: () => Jc,
	enumRowLevelPermissionPredicateOperand: () => Yc,
	enumSsoIdentityProviderStatus: () => Rl,
	enumSubscriptionInterval: () => Al,
	enumSubscriptionStatus: () => jl,
	enumSupportDriver: () => Bl,
	enumUnsubscribeTopicVisibility: () => $l,
	enumUsageOperationType: () => su,
	enumViewCalendarLayout: () => fl,
	enumViewFilterGroupLogicalOperator: () => ol,
	enumViewFilterOperand: () => sl,
	enumViewKey: () => ul,
	enumViewOpenRecordIn: () => dl,
	enumViewSortDirection: () => cl,
	enumViewType: () => ll,
	enumViewVisibility: () => pl,
	enumWidgetConfigurationType: () => vl,
	enumWidgetType: () => gl,
	enumWorkspaceActivationStatus: () => ml,
	enumWorkspaceMemberDateFormatEnum: () => Zc,
	enumWorkspaceMemberNumberFormatEnum: () => $c,
	enumWorkspaceMemberTimeFormatEnum: () => Qc,
	enumWorkspaceMigrationActionType: () => cu,
	isAgent: () => M,
	isAgentChatEvent: () => Sc,
	isAgentChatThread: () => fc,
	isAgentMessage: () => uc,
	isAgentMessagePart: () => As,
	isAgentTurn: () => Ec,
	isAgentTurnEvaluation: () => wc,
	isAggregateChartConfiguration: () => _t,
	isAiSystemPromptPreview: () => gc,
	isAiSystemPromptSection: () => mc,
	isAnalytics: () => ec,
	isApiConfig: () => oi,
	isApiKey: () => f,
	isApiKeyForRole: () => R,
	isApiKeyToken: () => vo,
	isAppConnection: () => na,
	isAppToken: () => Ye,
	isApplication: () => je,
	isApplicationConnectionProvider: () => mn,
	isApplicationConnectionProviderOAuthConfig: () => fn,
	isApplicationRegistration: () => g,
	isApplicationRegistrationStats: () => bi,
	isApplicationRegistrationSummary: () => H,
	isApplicationRegistrationVariable: () => m,
	isApplicationRegistrationVariableDTO: () => Pr,
	isApplicationTokenPair: () => J,
	isApplicationVariable: () => W,
	isApprovedAccessDomain: () => bn,
	isAuthBypassProviders: () => Hr,
	isAuthProviders: () => Br,
	isAuthToken: () => K,
	isAuthTokenPair: () => qa,
	isAuthTokens: () => bo,
	isAuthorizeApp: () => Ga,
	isAutocompleteResult: () => fs,
	isAvailableWorkspace: () => va,
	isAvailableWorkspaces: () => ba,
	isAvailableWorkspacesAndAccessTokens: () => Ya,
	isBarChartConfiguration: () => Dt,
	isBarChartData: () => Rs,
	isBarChartSeries: () => Is,
	isBilling: () => Qr,
	isBillingCustomer: () => Wn,
	isBillingEndTrialPeriod: () => Jn,
	isBillingEntitlement: () => wa,
	isBillingLicensedProduct: () => Rn,
	isBillingMeteredProduct: () => Bn,
	isBillingPaymentIntent: () => er,
	isBillingPlan: () => Qn,
	isBillingPriceLicensed: () => An,
	isBillingPriceMetered: () => Pn,
	isBillingPriceTier: () => Mn,
	isBillingProduct: () => In,
	isBillingProductDTO: () => u,
	isBillingProductMetadata: () => On,
	isBillingResourceCreditUsage: () => Xn,
	isBillingSession: () => nr,
	isBillingSubscription: () => Kn,
	isBillingSubscriptionItem: () => Hn,
	isBillingSubscriptionSchedulePhase: () => En,
	isBillingSubscriptionSchedulePhaseItem: () => wn,
	isBillingTrialPeriod: () => Ir,
	isBillingUpdate: () => ir,
	isCalendarChannel: () => Ac,
	isCalendarConfiguration: () => kt,
	isCampaignAudiencePreviewDTO: () => ns,
	isCampaignSkippedRecipientsDTO: () => os,
	isCaptcha: () => ii,
	isChannelSyncSuccess: () => Ps,
	isChatStreamCatchupChunks: () => vc,
	isCheckUserExist: () => wo,
	isClientAiModelConfig: () => Xr,
	isClientConfig: () => mi,
	isClientConfigMaintenanceMode: () => fi,
	isCollectionHash: () => Pc,
	isCommandMenuItem: () => le,
	isCommandMenuItemPayload: () => de,
	isConnectedAccountPublicDTO: () => La,
	isConnectedImapSmtpCaldavAccount: () => Ss,
	isCreateApplicationRegistration: () => Si,
	isCreateEmailGroupChannelOutput: () => es,
	isDeleteSso: () => oa,
	isDeleteTwoFactorAuthenticationMethod: () => za,
	isDeletedWorkspaceMember: () => Sa,
	isDevelopmentApplication: () => Io,
	isDomainRecord: () => Y,
	isDomainValidRecords: () => Da,
	isDuplicatedDashboard: () => Xs,
	isEditSso: () => ca,
	isEmailPasswordResetLink: () => Za,
	isEmailThreadConfiguration: () => Ft,
	isEmailingDomain: () => Xo,
	isEmailsConfiguration: () => Nt,
	isEnterpriseLicenseInfoDTO: () => gn,
	isEnterpriseSubscriptionStatusDTO: () => vn,
	isEventLogPageInfo: () => ic,
	isEventLogQueryResult: () => oc,
	isEventLogRecord: () => nc,
	isEventSubscription: () => Er,
	isFeatureFlag: () => Ar,
	isField: () => xe,
	isFieldConfiguration: () => Lt,
	isFieldConnection: () => ea,
	isFieldEdge: () => Xi,
	isFieldPermission: () => P,
	isFieldRichTextConfiguration: () => zt,
	isFieldsConfiguration: () => Vt,
	isFile: () => Bo,
	isFileWithSignedUrl: () => Sn,
	isFilesConfiguration: () => Ut,
	isFindAvailableSSOIDP: () => fa,
	isFrontComponent: () => se,
	isFrontComponentConfiguration: () => jt,
	isFullName: () => O,
	isGetAuthorizationUrlForSSO: () => $a,
	isGridPosition: () => rt,
	isIframeConfiguration: () => Tt,
	isImapSmtpCaldavConnectionSuccess: () => ws,
	isImapSmtpCaldavPublicConnectionParameters: () => bs,
	isImapSmtpCaldavPublicConnectionParams: () => vs,
	isImpersonate: () => Oo,
	isIndex: () => Te,
	isIndexConnection: () => Pi,
	isIndexEdge: () => Ai,
	isIndexField: () => Ce,
	isIndexFieldEdge: () => Ii,
	isIndexIndexFieldMetadatasConnection: () => Ri,
	isIndexObjectMetadataConnection: () => Hi,
	isInitiateTwoFactorAuthenticationProvisioning: () => Va,
	isInvalidatePassword: () => to,
	isInviteSuggestion: () => or,
	isLineChartConfiguration: () => Ct,
	isLineChartData: () => Ws,
	isLineChartDataPoint: () => Bs,
	isLineChartSeries: () => Hs,
	isLocation: () => ms,
	isLogicFunction: () => _e,
	isLogicFunctionExecutionResult: () => Or,
	isLogicFunctionLogs: () => ja,
	isLoginToken: () => So,
	isMarketplaceApp: () => Ho,
	isMarketplaceAppDetail: () => Wo,
	isMessageChannel: () => Qo,
	isMessageFolder: () => Mc,
	isMetadataEvent: () => br,
	isMinimalMetadata: () => Bc,
	isMinimalObjectMetadata: () => Ic,
	isMinimalView: () => Rc,
	isMutation: () => Wc,
	isNativeModelCapabilities: () => Jr,
	isNavigationMenuItem: () => gr,
	isNotesConfiguration: () => Gt,
	isObject: () => ke,
	isObjectConnection: () => Ki,
	isObjectEdge: () => Bi,
	isObjectFieldsConnection: () => Qi,
	isObjectIndexMetadatasConnection: () => Ji,
	isObjectMetadataCommandMenuItemPayload: () => he,
	isObjectPermission: () => w,
	isObjectRecordCount: () => Wi,
	isObjectRecordEvent: () => Sr,
	isObjectRecordEventProperties: () => vr,
	isObjectRecordEventWithQueryIds: () => wr,
	isObjectStandardOverrides: () => De,
	isOnboardingStepSuccess: () => cr,
	isPageInfo: () => Mi,
	isPageLayout: () => un,
	isPageLayoutTab: () => cn,
	isPageLayoutWidget: () => at,
	isPageLayoutWidgetCanvasPosition: () => pt,
	isPageLayoutWidgetGridPosition: () => lt,
	isPageLayoutWidgetPosition: () => st,
	isPageLayoutWidgetVerticalListPosition: () => dt,
	isPathCommandMenuItemPayload: () => pe,
	isPieChartConfiguration: () => xt,
	isPieChartData: () => Js,
	isPieChartDataItem: () => Ks,
	isPlaceDetailsResult: () => gs,
	isPublicApplicationRegistration: () => wi,
	isPublicConnectionParametersOutput: () => Na,
	isPublicDomain: () => Ko,
	isPublicFeatureFlag: () => ui,
	isPublicFeatureFlagMetadata: () => ci,
	isPublicImapSmtpCaldavConnectionParameters: () => Fa,
	isPublicWorkspaceData: () => Wr,
	isPublicWorkspaceDataSummary: () => Kr,
	isQuery: () => Hc,
	isRatioAggregateConfig: () => $e,
	isRecordIdentifier: () => mr,
	isRecordTableConfiguration: () => $t,
	isRelation: () => Oi,
	isResendEmailVerificationToken: () => ia,
	isRichTextBody: () => tt,
	isRole: () => B,
	isRolePermissionFlag: () => I,
	isRotateClientSecret: () => Ei,
	isRowLevelPermissionPredicate: () => S,
	isRowLevelPermissionPredicateGroup: () => b,
	isRunAgentResult: () => Ms,
	isSSOConnection: () => ga,
	isSSOIdentityProvider: () => Rr,
	isSendChatMessageResult: () => bc,
	isSendEmailOutput: () => Qs,
	isSendEmailViaDomainOutput: () => is,
	isSendInvitations: () => fr,
	isSendMessageCampaignOutputDTO: () => cs,
	isSentry: () => ni,
	isSetupSso: () => ma,
	isSignUp: () => ao,
	isSkill: () => cc,
	isStandaloneRichTextConfiguration: () => yt,
	isStandardOverrides: () => ye,
	isSubdomainAvailabilityDTO: () => mo,
	isSubscription: () => Kc,
	isSupport: () => ei,
	isTasksConfiguration: () => qt,
	isTimelineConfiguration: () => Yt,
	isToolIndexEntry: () => Os,
	isTransientToken: () => so,
	isTwoFactorAuthenticationMethodSummary: () => v,
	isUnsubscribeTopic: () => us,
	isUpsertRowLevelPermissionPredicatesResult: () => ka,
	isUsageAnalytics: () => Po,
	isUsageBreakdownItem: () => gi,
	isUsageTimeSeries: () => Ao,
	isUsageUserDaily: () => Mo,
	isUser: () => Ze,
	isUserWorkspace: () => E,
	isValidatePasswordResetToken: () => lo,
	isVerificationRecord: () => Jo,
	isVerifyEmailAndGetLoginToken: () => fo,
	isVerifyTwoFactorAuthenticationMethod: () => Ua,
	isVersionDistributionEntry: () => vi,
	isView: () => Ge,
	isViewConfiguration: () => Zt,
	isViewField: () => Ne,
	isViewFieldGroup: () => Ue,
	isViewFilter: () => Le,
	isViewFilterGroup: () => Fe,
	isViewGroup: () => ze,
	isViewSort: () => Ve,
	isWebhook: () => Es,
	isWidgetConfiguration: () => ht,
	isWorkflowConfiguration: () => tn,
	isWorkflowRunConfiguration: () => rn,
	isWorkflowVersionConfiguration: () => on,
	isWorkspace: () => qe,
	isWorkspaceAiStats: () => Oc,
	isWorkspaceCreationDefaultsDTO: () => go,
	isWorkspaceInvitation: () => ur,
	isWorkspaceInviteHashValid: () => Eo,
	isWorkspaceMember: () => A,
	isWorkspaceMigration: () => Ro,
	isWorkspaceNameAndId: () => ua,
	isWorkspaceUrls: () => Mr,
	isWorkspaceUrlsAndId: () => ro
}), ae = ["BillingLicensedProduct", "BillingMeteredProduct"], u = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isBillingProductDTO\"");
	return ae.includes(e.__typename);
}, d = ["ApiKey"], f = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isApiKey\"");
	return d.includes(e.__typename);
}, p = ["ApplicationRegistrationVariable"], m = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isApplicationRegistrationVariable\"");
	return p.includes(e.__typename);
}, h = ["ApplicationRegistration"], g = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isApplicationRegistration\"");
	return h.includes(e.__typename);
}, _ = ["TwoFactorAuthenticationMethodSummary"], v = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isTwoFactorAuthenticationMethodSummary\"");
	return _.includes(e.__typename);
}, y = ["RowLevelPermissionPredicateGroup"], b = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isRowLevelPermissionPredicateGroup\"");
	return y.includes(e.__typename);
}, x = ["RowLevelPermissionPredicate"], S = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isRowLevelPermissionPredicate\"");
	return x.includes(e.__typename);
}, C = ["ObjectPermission"], w = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isObjectPermission\"");
	return C.includes(e.__typename);
}, T = ["UserWorkspace"], E = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isUserWorkspace\"");
	return T.includes(e.__typename);
}, D = ["FullName"], O = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isFullName\"");
	return D.includes(e.__typename);
}, k = ["WorkspaceMember"], A = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isWorkspaceMember\"");
	return k.includes(e.__typename);
}, j = ["Agent"], M = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isAgent\"");
	return j.includes(e.__typename);
}, N = ["FieldPermission"], P = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isFieldPermission\"");
	return N.includes(e.__typename);
}, F = ["RolePermissionFlag"], I = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isRolePermissionFlag\"");
	return F.includes(e.__typename);
}, L = ["ApiKeyForRole"], R = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isApiKeyForRole\"");
	return L.includes(e.__typename);
}, z = ["Role"], B = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isRole\"");
	return z.includes(e.__typename);
}, V = ["ApplicationRegistrationSummary"], H = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isApplicationRegistrationSummary\"");
	return V.includes(e.__typename);
}, U = ["ApplicationVariable"], W = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isApplicationVariable\"");
	return U.includes(e.__typename);
}, G = ["AuthToken"], K = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isAuthToken\"");
	return G.includes(e.__typename);
}, q = ["ApplicationTokenPair"], J = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isApplicationTokenPair\"");
	return q.includes(e.__typename);
}, oe = ["FrontComponent"], se = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isFrontComponent\"");
	return oe.includes(e.__typename);
}, ce = ["CommandMenuItem"], le = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isCommandMenuItem\"");
	return ce.includes(e.__typename);
}, ue = ["PathCommandMenuItemPayload", "ObjectMetadataCommandMenuItemPayload"], de = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isCommandMenuItemPayload\"");
	return ue.includes(e.__typename);
}, fe = ["PathCommandMenuItemPayload"], pe = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isPathCommandMenuItemPayload\"");
	return fe.includes(e.__typename);
}, me = ["ObjectMetadataCommandMenuItemPayload"], he = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isObjectMetadataCommandMenuItemPayload\"");
	return me.includes(e.__typename);
}, ge = ["LogicFunction"], _e = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isLogicFunction\"");
	return ge.includes(e.__typename);
}, ve = ["StandardOverrides"], ye = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isStandardOverrides\"");
	return ve.includes(e.__typename);
}, be = ["Field"], xe = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isField\"");
	return be.includes(e.__typename);
}, Se = ["IndexField"], Ce = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isIndexField\"");
	return Se.includes(e.__typename);
}, we = ["Index"], Te = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isIndex\"");
	return we.includes(e.__typename);
}, Ee = ["ObjectStandardOverrides"], De = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isObjectStandardOverrides\"");
	return Ee.includes(e.__typename);
}, Oe = ["Object"], ke = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isObject\"");
	return Oe.includes(e.__typename);
}, Ae = ["Application"], je = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isApplication\"");
	return Ae.includes(e.__typename);
}, Me = ["ViewField"], Ne = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isViewField\"");
	return Me.includes(e.__typename);
}, Pe = ["ViewFilterGroup"], Fe = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isViewFilterGroup\"");
	return Pe.includes(e.__typename);
}, Ie = ["ViewFilter"], Le = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isViewFilter\"");
	return Ie.includes(e.__typename);
}, Re = ["ViewGroup"], ze = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isViewGroup\"");
	return Re.includes(e.__typename);
}, Be = ["ViewSort"], Ve = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isViewSort\"");
	return Be.includes(e.__typename);
}, He = ["ViewFieldGroup"], Ue = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isViewFieldGroup\"");
	return He.includes(e.__typename);
}, We = ["View"], Ge = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isView\"");
	return We.includes(e.__typename);
}, Ke = ["Workspace"], qe = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isWorkspace\"");
	return Ke.includes(e.__typename);
}, Je = ["AppToken"], Ye = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isAppToken\"");
	return Je.includes(e.__typename);
}, Xe = ["User"], Ze = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isUser\"");
	return Xe.includes(e.__typename);
}, Qe = ["RatioAggregateConfig"], $e = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isRatioAggregateConfig\"");
	return Qe.includes(e.__typename);
}, et = ["RichTextBody"], tt = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isRichTextBody\"");
	return et.includes(e.__typename);
}, nt = ["GridPosition"], rt = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isGridPosition\"");
	return nt.includes(e.__typename);
}, it = ["PageLayoutWidget"], at = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isPageLayoutWidget\"");
	return it.includes(e.__typename);
}, ot = [
	"PageLayoutWidgetGridPosition",
	"PageLayoutWidgetVerticalListPosition",
	"PageLayoutWidgetCanvasPosition"
], st = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isPageLayoutWidgetPosition\"");
	return ot.includes(e.__typename);
}, ct = ["PageLayoutWidgetGridPosition"], lt = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isPageLayoutWidgetGridPosition\"");
	return ct.includes(e.__typename);
}, ut = ["PageLayoutWidgetVerticalListPosition"], dt = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isPageLayoutWidgetVerticalListPosition\"");
	return ut.includes(e.__typename);
}, ft = ["PageLayoutWidgetCanvasPosition"], pt = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isPageLayoutWidgetCanvasPosition\"");
	return ft.includes(e.__typename);
}, mt = [
	"AggregateChartConfiguration",
	"StandaloneRichTextConfiguration",
	"PieChartConfiguration",
	"LineChartConfiguration",
	"IframeConfiguration",
	"BarChartConfiguration",
	"CalendarConfiguration",
	"FrontComponentConfiguration",
	"EmailsConfiguration",
	"EmailThreadConfiguration",
	"FieldConfiguration",
	"FieldRichTextConfiguration",
	"FieldsConfiguration",
	"FilesConfiguration",
	"NotesConfiguration",
	"TasksConfiguration",
	"TimelineConfiguration",
	"ViewConfiguration",
	"RecordTableConfiguration",
	"WorkflowConfiguration",
	"WorkflowRunConfiguration",
	"WorkflowVersionConfiguration"
], ht = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isWidgetConfiguration\"");
	return mt.includes(e.__typename);
}, gt = ["AggregateChartConfiguration"], _t = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isAggregateChartConfiguration\"");
	return gt.includes(e.__typename);
}, vt = ["StandaloneRichTextConfiguration"], yt = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isStandaloneRichTextConfiguration\"");
	return vt.includes(e.__typename);
}, bt = ["PieChartConfiguration"], xt = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isPieChartConfiguration\"");
	return bt.includes(e.__typename);
}, St = ["LineChartConfiguration"], Ct = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isLineChartConfiguration\"");
	return St.includes(e.__typename);
}, wt = ["IframeConfiguration"], Tt = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isIframeConfiguration\"");
	return wt.includes(e.__typename);
}, Et = ["BarChartConfiguration"], Dt = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isBarChartConfiguration\"");
	return Et.includes(e.__typename);
}, Ot = ["CalendarConfiguration"], kt = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isCalendarConfiguration\"");
	return Ot.includes(e.__typename);
}, At = ["FrontComponentConfiguration"], jt = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isFrontComponentConfiguration\"");
	return At.includes(e.__typename);
}, Mt = ["EmailsConfiguration"], Nt = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isEmailsConfiguration\"");
	return Mt.includes(e.__typename);
}, Pt = ["EmailThreadConfiguration"], Ft = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isEmailThreadConfiguration\"");
	return Pt.includes(e.__typename);
}, It = ["FieldConfiguration"], Lt = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isFieldConfiguration\"");
	return It.includes(e.__typename);
}, Rt = ["FieldRichTextConfiguration"], zt = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isFieldRichTextConfiguration\"");
	return Rt.includes(e.__typename);
}, Bt = ["FieldsConfiguration"], Vt = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isFieldsConfiguration\"");
	return Bt.includes(e.__typename);
}, Ht = ["FilesConfiguration"], Ut = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isFilesConfiguration\"");
	return Ht.includes(e.__typename);
}, Wt = ["NotesConfiguration"], Gt = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isNotesConfiguration\"");
	return Wt.includes(e.__typename);
}, Kt = ["TasksConfiguration"], qt = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isTasksConfiguration\"");
	return Kt.includes(e.__typename);
}, Jt = ["TimelineConfiguration"], Yt = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isTimelineConfiguration\"");
	return Jt.includes(e.__typename);
}, Xt = ["ViewConfiguration"], Zt = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isViewConfiguration\"");
	return Xt.includes(e.__typename);
}, Qt = ["RecordTableConfiguration"], $t = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isRecordTableConfiguration\"");
	return Qt.includes(e.__typename);
}, en = ["WorkflowConfiguration"], tn = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isWorkflowConfiguration\"");
	return en.includes(e.__typename);
}, nn = ["WorkflowRunConfiguration"], rn = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isWorkflowRunConfiguration\"");
	return nn.includes(e.__typename);
}, an = ["WorkflowVersionConfiguration"], on = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isWorkflowVersionConfiguration\"");
	return an.includes(e.__typename);
}, sn = ["PageLayoutTab"], cn = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isPageLayoutTab\"");
	return sn.includes(e.__typename);
}, ln = ["PageLayout"], un = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isPageLayout\"");
	return ln.includes(e.__typename);
}, dn = ["ApplicationConnectionProviderOAuthConfig"], fn = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isApplicationConnectionProviderOAuthConfig\"");
	return dn.includes(e.__typename);
}, pn = ["ApplicationConnectionProvider"], mn = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isApplicationConnectionProvider\"");
	return pn.includes(e.__typename);
}, hn = ["EnterpriseLicenseInfoDTO"], gn = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isEnterpriseLicenseInfoDTO\"");
	return hn.includes(e.__typename);
}, _n = ["EnterpriseSubscriptionStatusDTO"], vn = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isEnterpriseSubscriptionStatusDTO\"");
	return _n.includes(e.__typename);
}, yn = ["ApprovedAccessDomain"], bn = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isApprovedAccessDomain\"");
	return yn.includes(e.__typename);
}, xn = ["FileWithSignedUrl"], Sn = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isFileWithSignedUrl\"");
	return xn.includes(e.__typename);
}, Cn = ["BillingSubscriptionSchedulePhaseItem"], wn = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isBillingSubscriptionSchedulePhaseItem\"");
	return Cn.includes(e.__typename);
}, Tn = ["BillingSubscriptionSchedulePhase"], En = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isBillingSubscriptionSchedulePhase\"");
	return Tn.includes(e.__typename);
}, Dn = ["BillingProductMetadata"], On = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isBillingProductMetadata\"");
	return Dn.includes(e.__typename);
}, kn = ["BillingPriceLicensed"], An = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isBillingPriceLicensed\"");
	return kn.includes(e.__typename);
}, jn = ["BillingPriceTier"], Mn = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isBillingPriceTier\"");
	return jn.includes(e.__typename);
}, Nn = ["BillingPriceMetered"], Pn = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isBillingPriceMetered\"");
	return Nn.includes(e.__typename);
}, Fn = ["BillingProduct"], In = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isBillingProduct\"");
	return Fn.includes(e.__typename);
}, Ln = ["BillingLicensedProduct"], Rn = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isBillingLicensedProduct\"");
	return Ln.includes(e.__typename);
}, zn = ["BillingMeteredProduct"], Bn = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isBillingMeteredProduct\"");
	return zn.includes(e.__typename);
}, Vn = ["BillingSubscriptionItem"], Hn = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isBillingSubscriptionItem\"");
	return Vn.includes(e.__typename);
}, Un = ["BillingCustomer"], Wn = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isBillingCustomer\"");
	return Un.includes(e.__typename);
}, Gn = ["BillingSubscription"], Kn = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isBillingSubscription\"");
	return Gn.includes(e.__typename);
}, qn = ["BillingEndTrialPeriod"], Jn = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isBillingEndTrialPeriod\"");
	return qn.includes(e.__typename);
}, Yn = ["BillingResourceCreditUsage"], Xn = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isBillingResourceCreditUsage\"");
	return Yn.includes(e.__typename);
}, Zn = ["BillingPlan"], Qn = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isBillingPlan\"");
	return Zn.includes(e.__typename);
}, $n = ["BillingPaymentIntent"], er = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isBillingPaymentIntent\"");
	return $n.includes(e.__typename);
}, tr = ["BillingSession"], nr = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isBillingSession\"");
	return tr.includes(e.__typename);
}, rr = ["BillingUpdate"], ir = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isBillingUpdate\"");
	return rr.includes(e.__typename);
}, ar = ["InviteSuggestion"], or = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isInviteSuggestion\"");
	return ar.includes(e.__typename);
}, sr = ["OnboardingStepSuccess"], cr = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isOnboardingStepSuccess\"");
	return sr.includes(e.__typename);
}, lr = ["WorkspaceInvitation"], ur = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isWorkspaceInvitation\"");
	return lr.includes(e.__typename);
}, dr = ["SendInvitations"], fr = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isSendInvitations\"");
	return dr.includes(e.__typename);
}, pr = ["RecordIdentifier"], mr = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isRecordIdentifier\"");
	return pr.includes(e.__typename);
}, hr = ["NavigationMenuItem"], gr = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isNavigationMenuItem\"");
	return hr.includes(e.__typename);
}, _r = ["ObjectRecordEventProperties"], vr = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isObjectRecordEventProperties\"");
	return _r.includes(e.__typename);
}, yr = ["MetadataEvent"], br = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isMetadataEvent\"");
	return yr.includes(e.__typename);
}, xr = ["ObjectRecordEvent"], Sr = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isObjectRecordEvent\"");
	return xr.includes(e.__typename);
}, Cr = ["ObjectRecordEventWithQueryIds"], wr = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isObjectRecordEventWithQueryIds\"");
	return Cr.includes(e.__typename);
}, Tr = ["EventSubscription"], Er = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isEventSubscription\"");
	return Tr.includes(e.__typename);
}, Dr = ["LogicFunctionExecutionResult"], Or = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isLogicFunctionExecutionResult\"");
	return Dr.includes(e.__typename);
}, kr = ["FeatureFlag"], Ar = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isFeatureFlag\"");
	return kr.includes(e.__typename);
}, jr = ["WorkspaceUrls"], Mr = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isWorkspaceUrls\"");
	return jr.includes(e.__typename);
}, Nr = ["ApplicationRegistrationVariableDTO"], Pr = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isApplicationRegistrationVariableDTO\"");
	return Nr.includes(e.__typename);
}, Fr = ["BillingTrialPeriod"], Ir = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isBillingTrialPeriod\"");
	return Fr.includes(e.__typename);
}, Lr = ["SSOIdentityProvider"], Rr = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isSSOIdentityProvider\"");
	return Lr.includes(e.__typename);
}, zr = ["AuthProviders"], Br = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isAuthProviders\"");
	return zr.includes(e.__typename);
}, Vr = ["AuthBypassProviders"], Hr = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isAuthBypassProviders\"");
	return Vr.includes(e.__typename);
}, Ur = ["PublicWorkspaceData"], Wr = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isPublicWorkspaceData\"");
	return Ur.includes(e.__typename);
}, Gr = ["PublicWorkspaceDataSummary"], Kr = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isPublicWorkspaceDataSummary\"");
	return Gr.includes(e.__typename);
}, qr = ["NativeModelCapabilities"], Jr = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isNativeModelCapabilities\"");
	return qr.includes(e.__typename);
}, Yr = ["ClientAiModelConfig"], Xr = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isClientAiModelConfig\"");
	return Yr.includes(e.__typename);
}, Zr = ["Billing"], Qr = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isBilling\"");
	return Zr.includes(e.__typename);
}, $r = ["Support"], ei = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isSupport\"");
	return $r.includes(e.__typename);
}, ti = ["Sentry"], ni = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isSentry\"");
	return ti.includes(e.__typename);
}, ri = ["Captcha"], ii = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isCaptcha\"");
	return ri.includes(e.__typename);
}, ai = ["ApiConfig"], oi = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isApiConfig\"");
	return ai.includes(e.__typename);
}, si = ["PublicFeatureFlagMetadata"], ci = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isPublicFeatureFlagMetadata\"");
	return si.includes(e.__typename);
}, li = ["PublicFeatureFlag"], ui = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isPublicFeatureFlag\"");
	return li.includes(e.__typename);
}, di = ["ClientConfigMaintenanceMode"], fi = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isClientConfigMaintenanceMode\"");
	return di.includes(e.__typename);
}, pi = ["ClientConfig"], mi = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isClientConfig\"");
	return pi.includes(e.__typename);
}, hi = ["UsageBreakdownItem"], gi = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isUsageBreakdownItem\"");
	return hi.includes(e.__typename);
}, _i = ["VersionDistributionEntry"], vi = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isVersionDistributionEntry\"");
	return _i.includes(e.__typename);
}, yi = ["ApplicationRegistrationStats"], bi = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isApplicationRegistrationStats\"");
	return yi.includes(e.__typename);
}, xi = ["CreateApplicationRegistration"], Si = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isCreateApplicationRegistration\"");
	return xi.includes(e.__typename);
}, Ci = ["PublicApplicationRegistration"], wi = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isPublicApplicationRegistration\"");
	return Ci.includes(e.__typename);
}, Ti = ["RotateClientSecret"], Ei = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isRotateClientSecret\"");
	return Ti.includes(e.__typename);
}, Di = ["Relation"], Oi = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isRelation\"");
	return Di.includes(e.__typename);
}, ki = ["IndexEdge"], Ai = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isIndexEdge\"");
	return ki.includes(e.__typename);
}, ji = ["PageInfo"], Mi = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isPageInfo\"");
	return ji.includes(e.__typename);
}, Ni = ["IndexConnection"], Pi = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isIndexConnection\"");
	return Ni.includes(e.__typename);
}, Fi = ["IndexFieldEdge"], Ii = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isIndexFieldEdge\"");
	return Fi.includes(e.__typename);
}, Li = ["IndexIndexFieldMetadatasConnection"], Ri = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isIndexIndexFieldMetadatasConnection\"");
	return Li.includes(e.__typename);
}, zi = ["ObjectEdge"], Bi = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isObjectEdge\"");
	return zi.includes(e.__typename);
}, Vi = ["IndexObjectMetadataConnection"], Hi = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isIndexObjectMetadataConnection\"");
	return Vi.includes(e.__typename);
}, Ui = ["ObjectRecordCount"], Wi = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isObjectRecordCount\"");
	return Ui.includes(e.__typename);
}, Gi = ["ObjectConnection"], Ki = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isObjectConnection\"");
	return Gi.includes(e.__typename);
}, qi = ["ObjectIndexMetadatasConnection"], Ji = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isObjectIndexMetadatasConnection\"");
	return qi.includes(e.__typename);
}, Yi = ["FieldEdge"], Xi = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isFieldEdge\"");
	return Yi.includes(e.__typename);
}, Zi = ["ObjectFieldsConnection"], Qi = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isObjectFieldsConnection\"");
	return Zi.includes(e.__typename);
}, $i = ["FieldConnection"], ea = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isFieldConnection\"");
	return $i.includes(e.__typename);
}, ta = ["AppConnection"], na = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isAppConnection\"");
	return ta.includes(e.__typename);
}, ra = ["ResendEmailVerificationToken"], ia = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isResendEmailVerificationToken\"");
	return ra.includes(e.__typename);
}, aa = ["DeleteSso"], oa = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isDeleteSso\"");
	return aa.includes(e.__typename);
}, sa = ["EditSso"], ca = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isEditSso\"");
	return sa.includes(e.__typename);
}, la = ["WorkspaceNameAndId"], ua = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isWorkspaceNameAndId\"");
	return la.includes(e.__typename);
}, da = ["FindAvailableSSOIDP"], fa = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isFindAvailableSSOIDP\"");
	return da.includes(e.__typename);
}, pa = ["SetupSso"], ma = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isSetupSso\"");
	return pa.includes(e.__typename);
}, ha = ["SSOConnection"], ga = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isSSOConnection\"");
	return ha.includes(e.__typename);
}, _a = ["AvailableWorkspace"], va = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isAvailableWorkspace\"");
	return _a.includes(e.__typename);
}, ya = ["AvailableWorkspaces"], ba = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isAvailableWorkspaces\"");
	return ya.includes(e.__typename);
}, xa = ["DeletedWorkspaceMember"], Sa = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isDeletedWorkspaceMember\"");
	return xa.includes(e.__typename);
}, Ca = ["BillingEntitlement"], wa = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isBillingEntitlement\"");
	return Ca.includes(e.__typename);
}, Ta = ["DomainRecord"], Y = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isDomainRecord\"");
	return Ta.includes(e.__typename);
}, Ea = ["DomainValidRecords"], Da = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isDomainValidRecords\"");
	return Ea.includes(e.__typename);
}, Oa = ["UpsertRowLevelPermissionPredicatesResult"], ka = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isUpsertRowLevelPermissionPredicatesResult\"");
	return Oa.includes(e.__typename);
}, Aa = ["LogicFunctionLogs"], ja = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isLogicFunctionLogs\"");
	return Aa.includes(e.__typename);
}, Ma = ["PublicConnectionParametersOutput"], Na = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isPublicConnectionParametersOutput\"");
	return Ma.includes(e.__typename);
}, Pa = ["PublicImapSmtpCaldavConnectionParameters"], Fa = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isPublicImapSmtpCaldavConnectionParameters\"");
	return Pa.includes(e.__typename);
}, Ia = ["ConnectedAccountPublicDTO"], La = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isConnectedAccountPublicDTO\"");
	return Ia.includes(e.__typename);
}, Ra = ["DeleteTwoFactorAuthenticationMethod"], za = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isDeleteTwoFactorAuthenticationMethod\"");
	return Ra.includes(e.__typename);
}, Ba = ["InitiateTwoFactorAuthenticationProvisioning"], Va = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isInitiateTwoFactorAuthenticationProvisioning\"");
	return Ba.includes(e.__typename);
}, Ha = ["VerifyTwoFactorAuthenticationMethod"], Ua = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isVerifyTwoFactorAuthenticationMethod\"");
	return Ha.includes(e.__typename);
}, Wa = ["AuthorizeApp"], Ga = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isAuthorizeApp\"");
	return Wa.includes(e.__typename);
}, Ka = ["AuthTokenPair"], qa = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isAuthTokenPair\"");
	return Ka.includes(e.__typename);
}, Ja = ["AvailableWorkspacesAndAccessTokens"], Ya = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isAvailableWorkspacesAndAccessTokens\"");
	return Ja.includes(e.__typename);
}, Xa = ["EmailPasswordResetLink"], Za = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isEmailPasswordResetLink\"");
	return Xa.includes(e.__typename);
}, Qa = ["GetAuthorizationUrlForSSO"], $a = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isGetAuthorizationUrlForSSO\"");
	return Qa.includes(e.__typename);
}, eo = ["InvalidatePassword"], to = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isInvalidatePassword\"");
	return eo.includes(e.__typename);
}, no = ["WorkspaceUrlsAndId"], ro = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isWorkspaceUrlsAndId\"");
	return no.includes(e.__typename);
}, io = ["SignUp"], ao = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isSignUp\"");
	return io.includes(e.__typename);
}, oo = ["TransientToken"], so = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isTransientToken\"");
	return oo.includes(e.__typename);
}, co = ["ValidatePasswordResetToken"], lo = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isValidatePasswordResetToken\"");
	return co.includes(e.__typename);
}, uo = ["VerifyEmailAndGetLoginToken"], fo = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isVerifyEmailAndGetLoginToken\"");
	return uo.includes(e.__typename);
}, po = ["SubdomainAvailabilityDTO"], mo = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isSubdomainAvailabilityDTO\"");
	return po.includes(e.__typename);
}, ho = ["WorkspaceCreationDefaultsDTO"], go = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isWorkspaceCreationDefaultsDTO\"");
	return ho.includes(e.__typename);
}, _o = ["ApiKeyToken"], vo = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isApiKeyToken\"");
	return _o.includes(e.__typename);
}, yo = ["AuthTokens"], bo = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isAuthTokens\"");
	return yo.includes(e.__typename);
}, xo = ["LoginToken"], So = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isLoginToken\"");
	return xo.includes(e.__typename);
}, Co = ["CheckUserExist"], wo = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isCheckUserExist\"");
	return Co.includes(e.__typename);
}, To = ["WorkspaceInviteHashValid"], Eo = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isWorkspaceInviteHashValid\"");
	return To.includes(e.__typename);
}, Do = ["Impersonate"], Oo = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isImpersonate\"");
	return Do.includes(e.__typename);
}, ko = ["UsageTimeSeries"], Ao = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isUsageTimeSeries\"");
	return ko.includes(e.__typename);
}, jo = ["UsageUserDaily"], Mo = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isUsageUserDaily\"");
	return jo.includes(e.__typename);
}, No = ["UsageAnalytics"], Po = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isUsageAnalytics\"");
	return No.includes(e.__typename);
}, Fo = ["DevelopmentApplication"], Io = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isDevelopmentApplication\"");
	return Fo.includes(e.__typename);
}, Lo = ["WorkspaceMigration"], Ro = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isWorkspaceMigration\"");
	return Lo.includes(e.__typename);
}, zo = ["File"], Bo = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isFile\"");
	return zo.includes(e.__typename);
}, Vo = ["MarketplaceApp"], Ho = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isMarketplaceApp\"");
	return Vo.includes(e.__typename);
}, Uo = ["MarketplaceAppDetail"], Wo = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isMarketplaceAppDetail\"");
	return Uo.includes(e.__typename);
}, Go = ["PublicDomain"], Ko = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isPublicDomain\"");
	return Go.includes(e.__typename);
}, qo = ["VerificationRecord"], Jo = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isVerificationRecord\"");
	return qo.includes(e.__typename);
}, Yo = ["EmailingDomain"], Xo = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isEmailingDomain\"");
	return Yo.includes(e.__typename);
}, Zo = ["MessageChannel"], Qo = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isMessageChannel\"");
	return Zo.includes(e.__typename);
}, $o = ["CreateEmailGroupChannelOutput"], es = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isCreateEmailGroupChannelOutput\"");
	return $o.includes(e.__typename);
}, ts = ["CampaignAudiencePreviewDTO"], ns = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isCampaignAudiencePreviewDTO\"");
	return ts.includes(e.__typename);
}, rs = ["SendEmailViaDomainOutput"], is = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isSendEmailViaDomainOutput\"");
	return rs.includes(e.__typename);
}, as = ["CampaignSkippedRecipientsDTO"], os = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isCampaignSkippedRecipientsDTO\"");
	return as.includes(e.__typename);
}, ss = ["SendMessageCampaignOutputDTO"], cs = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isSendMessageCampaignOutputDTO\"");
	return ss.includes(e.__typename);
}, ls = ["UnsubscribeTopic"], us = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isUnsubscribeTopic\"");
	return ls.includes(e.__typename);
}, ds = ["AutocompleteResult"], fs = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isAutocompleteResult\"");
	return ds.includes(e.__typename);
}, ps = ["Location"], ms = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isLocation\"");
	return ps.includes(e.__typename);
}, hs = ["PlaceDetailsResult"], gs = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isPlaceDetailsResult\"");
	return hs.includes(e.__typename);
}, _s = ["ImapSmtpCaldavPublicConnectionParams"], vs = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isImapSmtpCaldavPublicConnectionParams\"");
	return _s.includes(e.__typename);
}, ys = ["ImapSmtpCaldavPublicConnectionParameters"], bs = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isImapSmtpCaldavPublicConnectionParameters\"");
	return ys.includes(e.__typename);
}, xs = ["ConnectedImapSmtpCaldavAccount"], Ss = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isConnectedImapSmtpCaldavAccount\"");
	return xs.includes(e.__typename);
}, Cs = ["ImapSmtpCaldavConnectionSuccess"], ws = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isImapSmtpCaldavConnectionSuccess\"");
	return Cs.includes(e.__typename);
}, Ts = ["Webhook"], Es = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isWebhook\"");
	return Ts.includes(e.__typename);
}, Ds = ["ToolIndexEntry"], Os = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isToolIndexEntry\"");
	return Ds.includes(e.__typename);
}, ks = ["AgentMessagePart"], As = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isAgentMessagePart\"");
	return ks.includes(e.__typename);
}, js = ["RunAgentResult"], Ms = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isRunAgentResult\"");
	return js.includes(e.__typename);
}, Ns = ["ChannelSyncSuccess"], Ps = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isChannelSyncSuccess\"");
	return Ns.includes(e.__typename);
}, Fs = ["BarChartSeries"], Is = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isBarChartSeries\"");
	return Fs.includes(e.__typename);
}, Ls = ["BarChartData"], Rs = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isBarChartData\"");
	return Ls.includes(e.__typename);
}, zs = ["LineChartDataPoint"], Bs = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isLineChartDataPoint\"");
	return zs.includes(e.__typename);
}, Vs = ["LineChartSeries"], Hs = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isLineChartSeries\"");
	return Vs.includes(e.__typename);
}, Us = ["LineChartData"], Ws = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isLineChartData\"");
	return Us.includes(e.__typename);
}, Gs = ["PieChartDataItem"], Ks = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isPieChartDataItem\"");
	return Gs.includes(e.__typename);
}, qs = ["PieChartData"], Js = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isPieChartData\"");
	return qs.includes(e.__typename);
}, Ys = ["DuplicatedDashboard"], Xs = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isDuplicatedDashboard\"");
	return Ys.includes(e.__typename);
}, Zs = ["SendEmailOutput"], Qs = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isSendEmailOutput\"");
	return Zs.includes(e.__typename);
}, $s = ["Analytics"], ec = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isAnalytics\"");
	return $s.includes(e.__typename);
}, tc = ["EventLogRecord"], nc = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isEventLogRecord\"");
	return tc.includes(e.__typename);
}, rc = ["EventLogPageInfo"], ic = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isEventLogPageInfo\"");
	return rc.includes(e.__typename);
}, ac = ["EventLogQueryResult"], oc = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isEventLogQueryResult\"");
	return ac.includes(e.__typename);
}, sc = ["Skill"], cc = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isSkill\"");
	return sc.includes(e.__typename);
}, lc = ["AgentMessage"], uc = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isAgentMessage\"");
	return lc.includes(e.__typename);
}, dc = ["AgentChatThread"], fc = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isAgentChatThread\"");
	return dc.includes(e.__typename);
}, pc = ["AiSystemPromptSection"], mc = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isAiSystemPromptSection\"");
	return pc.includes(e.__typename);
}, hc = ["AiSystemPromptPreview"], gc = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isAiSystemPromptPreview\"");
	return hc.includes(e.__typename);
}, _c = ["ChatStreamCatchupChunks"], vc = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isChatStreamCatchupChunks\"");
	return _c.includes(e.__typename);
}, yc = ["SendChatMessageResult"], bc = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isSendChatMessageResult\"");
	return yc.includes(e.__typename);
}, xc = ["AgentChatEvent"], Sc = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isAgentChatEvent\"");
	return xc.includes(e.__typename);
}, Cc = ["AgentTurnEvaluation"], wc = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isAgentTurnEvaluation\"");
	return Cc.includes(e.__typename);
}, Tc = ["AgentTurn"], Ec = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isAgentTurn\"");
	return Tc.includes(e.__typename);
}, Dc = ["WorkspaceAiStats"], Oc = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isWorkspaceAiStats\"");
	return Dc.includes(e.__typename);
}, kc = ["CalendarChannel"], Ac = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isCalendarChannel\"");
	return kc.includes(e.__typename);
}, jc = ["MessageFolder"], Mc = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isMessageFolder\"");
	return jc.includes(e.__typename);
}, Nc = ["CollectionHash"], Pc = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isCollectionHash\"");
	return Nc.includes(e.__typename);
}, Fc = ["MinimalObjectMetadata"], Ic = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isMinimalObjectMetadata\"");
	return Fc.includes(e.__typename);
}, Lc = ["MinimalView"], Rc = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isMinimalView\"");
	return Lc.includes(e.__typename);
}, zc = ["MinimalMetadata"], Bc = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isMinimalMetadata\"");
	return zc.includes(e.__typename);
}, Vc = ["Query"], Hc = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isQuery\"");
	return Vc.includes(e.__typename);
}, Uc = ["Mutation"], Wc = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isMutation\"");
	return Uc.includes(e.__typename);
}, Gc = ["Subscription"], Kc = (e) => {
	if (!e?.__typename) throw Error("__typename is missing in \"isSubscription\"");
	return Gc.includes(e.__typename);
}, qc = {
	NPM: "NPM",
	TARBALL: "TARBALL",
	LOCAL: "LOCAL",
	OAUTH_ONLY: "OAUTH_ONLY"
}, Jc = {
	AND: "AND",
	OR: "OR"
}, Yc = {
	IS: "IS",
	IS_NOT_NULL: "IS_NOT_NULL",
	IS_NOT: "IS_NOT",
	LESS_THAN_OR_EQUAL: "LESS_THAN_OR_EQUAL",
	GREATER_THAN_OR_EQUAL: "GREATER_THAN_OR_EQUAL",
	IS_BEFORE: "IS_BEFORE",
	IS_AFTER: "IS_AFTER",
	CONTAINS: "CONTAINS",
	DOES_NOT_CONTAIN: "DOES_NOT_CONTAIN",
	IS_EMPTY: "IS_EMPTY",
	IS_NOT_EMPTY: "IS_NOT_EMPTY",
	IS_RELATIVE: "IS_RELATIVE",
	IS_IN_PAST: "IS_IN_PAST",
	IS_IN_FUTURE: "IS_IN_FUTURE",
	IS_TODAY: "IS_TODAY",
	VECTOR_SEARCH: "VECTOR_SEARCH"
}, Xc = {
	API_KEYS_AND_WEBHOOKS: "API_KEYS_AND_WEBHOOKS",
	WORKSPACE: "WORKSPACE",
	WORKSPACE_MEMBERS: "WORKSPACE_MEMBERS",
	ROLES: "ROLES",
	DATA_MODEL: "DATA_MODEL",
	SECURITY: "SECURITY",
	WORKFLOWS: "WORKFLOWS",
	IMPERSONATE: "IMPERSONATE",
	SSO_BYPASS: "SSO_BYPASS",
	APPLICATIONS: "APPLICATIONS",
	MARKETPLACE_APPS: "MARKETPLACE_APPS",
	LAYOUTS: "LAYOUTS",
	BILLING: "BILLING",
	AI_SETTINGS: "AI_SETTINGS",
	AI: "AI",
	VIEWS: "VIEWS",
	UPLOAD_FILE: "UPLOAD_FILE",
	DOWNLOAD_FILE: "DOWNLOAD_FILE",
	SEND_EMAIL_TOOL: "SEND_EMAIL_TOOL",
	HTTP_REQUEST_TOOL: "HTTP_REQUEST_TOOL",
	CODE_INTERPRETER_TOOL: "CODE_INTERPRETER_TOOL",
	IMPORT_CSV: "IMPORT_CSV",
	EXPORT_CSV: "EXPORT_CSV",
	CONNECTED_ACCOUNTS: "CONNECTED_ACCOUNTS",
	PROFILE_INFORMATION: "PROFILE_INFORMATION"
}, Zc = {
	SYSTEM: "SYSTEM",
	MONTH_FIRST: "MONTH_FIRST",
	DAY_FIRST: "DAY_FIRST",
	YEAR_FIRST: "YEAR_FIRST"
}, Qc = {
	SYSTEM: "SYSTEM",
	HOUR_12: "HOUR_12",
	HOUR_24: "HOUR_24"
}, $c = {
	SYSTEM: "SYSTEM",
	COMMAS_AND_DOT: "COMMAS_AND_DOT",
	SPACES_AND_COMMA: "SPACES_AND_COMMA",
	DOTS_AND_COMMA: "DOTS_AND_COMMA",
	APOSTROPHE_AND_DOT: "APOSTROPHE_AND_DOT"
}, el = {
	NAVIGATE_TO_NEXT_RECORD: "NAVIGATE_TO_NEXT_RECORD",
	NAVIGATE_TO_PREVIOUS_RECORD: "NAVIGATE_TO_PREVIOUS_RECORD",
	CREATE_NEW_RECORD: "CREATE_NEW_RECORD",
	DELETE_RECORDS: "DELETE_RECORDS",
	RESTORE_RECORDS: "RESTORE_RECORDS",
	DESTROY_RECORDS: "DESTROY_RECORDS",
	ADD_TO_FAVORITES: "ADD_TO_FAVORITES",
	REMOVE_FROM_FAVORITES: "REMOVE_FROM_FAVORITES",
	EXPORT_NOTE_TO_PDF: "EXPORT_NOTE_TO_PDF",
	EXPORT_RECORDS: "EXPORT_RECORDS",
	UPDATE_MULTIPLE_RECORDS: "UPDATE_MULTIPLE_RECORDS",
	MERGE_MULTIPLE_RECORDS: "MERGE_MULTIPLE_RECORDS",
	IMPORT_RECORDS: "IMPORT_RECORDS",
	EXPORT_VIEW: "EXPORT_VIEW",
	SEE_DELETED_RECORDS: "SEE_DELETED_RECORDS",
	CREATE_NEW_VIEW: "CREATE_NEW_VIEW",
	HIDE_DELETED_RECORDS: "HIDE_DELETED_RECORDS",
	EDIT_RECORD_PAGE_LAYOUT: "EDIT_RECORD_PAGE_LAYOUT",
	EDIT_DASHBOARD_LAYOUT: "EDIT_DASHBOARD_LAYOUT",
	SAVE_DASHBOARD_LAYOUT: "SAVE_DASHBOARD_LAYOUT",
	CANCEL_DASHBOARD_LAYOUT: "CANCEL_DASHBOARD_LAYOUT",
	DUPLICATE_DASHBOARD: "DUPLICATE_DASHBOARD",
	ACTIVATE_WORKFLOW: "ACTIVATE_WORKFLOW",
	DEACTIVATE_WORKFLOW: "DEACTIVATE_WORKFLOW",
	DISCARD_DRAFT_WORKFLOW: "DISCARD_DRAFT_WORKFLOW",
	TEST_WORKFLOW: "TEST_WORKFLOW",
	SEE_ACTIVE_VERSION_WORKFLOW: "SEE_ACTIVE_VERSION_WORKFLOW",
	SEE_RUNS_WORKFLOW: "SEE_RUNS_WORKFLOW",
	SEE_VERSIONS_WORKFLOW: "SEE_VERSIONS_WORKFLOW",
	ADD_NODE_WORKFLOW: "ADD_NODE_WORKFLOW",
	TIDY_UP_WORKFLOW: "TIDY_UP_WORKFLOW",
	DUPLICATE_WORKFLOW: "DUPLICATE_WORKFLOW",
	SEE_VERSION_WORKFLOW_RUN: "SEE_VERSION_WORKFLOW_RUN",
	SEE_WORKFLOW_WORKFLOW_RUN: "SEE_WORKFLOW_WORKFLOW_RUN",
	STOP_WORKFLOW_RUN: "STOP_WORKFLOW_RUN",
	RETRY_WORKFLOW_RUN: "RETRY_WORKFLOW_RUN",
	SEE_RUNS_WORKFLOW_VERSION: "SEE_RUNS_WORKFLOW_VERSION",
	SEE_WORKFLOW_WORKFLOW_VERSION: "SEE_WORKFLOW_WORKFLOW_VERSION",
	USE_AS_DRAFT_WORKFLOW_VERSION: "USE_AS_DRAFT_WORKFLOW_VERSION",
	SEE_VERSIONS_WORKFLOW_VERSION: "SEE_VERSIONS_WORKFLOW_VERSION",
	SEARCH_RECORDS: "SEARCH_RECORDS",
	SEARCH_RECORDS_FALLBACK: "SEARCH_RECORDS_FALLBACK",
	ASK_AI: "ASK_AI",
	VIEW_PREVIOUS_AI_CHATS: "VIEW_PREVIOUS_AI_CHATS",
	NAVIGATION: "NAVIGATION",
	TRIGGER_WORKFLOW_VERSION: "TRIGGER_WORKFLOW_VERSION",
	FRONT_COMPONENT_RENDERER: "FRONT_COMPONENT_RENDERER",
	REPLY_TO_EMAIL_THREAD: "REPLY_TO_EMAIL_THREAD",
	COMPOSE_EMAIL: "COMPOSE_EMAIL",
	COMPOSE_CAMPAIGN: "COMPOSE_CAMPAIGN",
	GO_TO_PEOPLE: "GO_TO_PEOPLE",
	GO_TO_COMPANIES: "GO_TO_COMPANIES",
	GO_TO_DASHBOARDS: "GO_TO_DASHBOARDS",
	GO_TO_OPPORTUNITIES: "GO_TO_OPPORTUNITIES",
	GO_TO_SETTINGS: "GO_TO_SETTINGS",
	GO_TO_TASKS: "GO_TO_TASKS",
	GO_TO_NOTES: "GO_TO_NOTES",
	GO_TO_WORKFLOWS: "GO_TO_WORKFLOWS",
	GO_TO_RUNS: "GO_TO_RUNS",
	DELETE_SINGLE_RECORD: "DELETE_SINGLE_RECORD",
	DELETE_MULTIPLE_RECORDS: "DELETE_MULTIPLE_RECORDS",
	RESTORE_SINGLE_RECORD: "RESTORE_SINGLE_RECORD",
	RESTORE_MULTIPLE_RECORDS: "RESTORE_MULTIPLE_RECORDS",
	DESTROY_SINGLE_RECORD: "DESTROY_SINGLE_RECORD",
	DESTROY_MULTIPLE_RECORDS: "DESTROY_MULTIPLE_RECORDS",
	EXPORT_FROM_RECORD_INDEX: "EXPORT_FROM_RECORD_INDEX",
	EXPORT_FROM_RECORD_SHOW: "EXPORT_FROM_RECORD_SHOW",
	EXPORT_MULTIPLE_RECORDS: "EXPORT_MULTIPLE_RECORDS"
}, tl = {
	GLOBAL: "GLOBAL",
	GLOBAL_OBJECT_CONTEXT: "GLOBAL_OBJECT_CONTEXT",
	RECORD_SELECTION: "RECORD_SELECTION",
	FALLBACK: "FALLBACK"
}, nl = {
	LIVE: "LIVE",
	PREBUILT: "PREBUILT"
}, rl = {
	ACTOR: "ACTOR",
	ADDRESS: "ADDRESS",
	ARRAY: "ARRAY",
	BOOLEAN: "BOOLEAN",
	CURRENCY: "CURRENCY",
	DATE: "DATE",
	DATE_TIME: "DATE_TIME",
	EMAILS: "EMAILS",
	FILES: "FILES",
	FULL_NAME: "FULL_NAME",
	LINKS: "LINKS",
	MORPH_RELATION: "MORPH_RELATION",
	MULTI_SELECT: "MULTI_SELECT",
	NUMBER: "NUMBER",
	NUMERIC: "NUMERIC",
	PHONES: "PHONES",
	POSITION: "POSITION",
	RATING: "RATING",
	RAW_JSON: "RAW_JSON",
	RELATION: "RELATION",
	RICH_TEXT: "RICH_TEXT",
	SELECT: "SELECT",
	TEXT: "TEXT",
	TS_VECTOR: "TS_VECTOR",
	UUID: "UUID"
}, il = {
	BTREE: "BTREE",
	GIN: "GIN"
}, al = {
	MIN: "MIN",
	MAX: "MAX",
	AVG: "AVG",
	SUM: "SUM",
	COUNT: "COUNT",
	COUNT_UNIQUE_VALUES: "COUNT_UNIQUE_VALUES",
	COUNT_EMPTY: "COUNT_EMPTY",
	COUNT_NOT_EMPTY: "COUNT_NOT_EMPTY",
	COUNT_TRUE: "COUNT_TRUE",
	COUNT_FALSE: "COUNT_FALSE",
	PERCENTAGE_EMPTY: "PERCENTAGE_EMPTY",
	PERCENTAGE_NOT_EMPTY: "PERCENTAGE_NOT_EMPTY"
}, ol = {
	AND: "AND",
	OR: "OR",
	NOT: "NOT"
}, sl = {
	IS: "IS",
	IS_NOT_NULL: "IS_NOT_NULL",
	IS_NOT: "IS_NOT",
	LESS_THAN_OR_EQUAL: "LESS_THAN_OR_EQUAL",
	GREATER_THAN_OR_EQUAL: "GREATER_THAN_OR_EQUAL",
	IS_BEFORE: "IS_BEFORE",
	IS_AFTER: "IS_AFTER",
	CONTAINS: "CONTAINS",
	DOES_NOT_CONTAIN: "DOES_NOT_CONTAIN",
	IS_EMPTY: "IS_EMPTY",
	IS_NOT_EMPTY: "IS_NOT_EMPTY",
	IS_RELATIVE: "IS_RELATIVE",
	IS_IN_PAST: "IS_IN_PAST",
	IS_IN_FUTURE: "IS_IN_FUTURE",
	IS_TODAY: "IS_TODAY",
	VECTOR_SEARCH: "VECTOR_SEARCH"
}, cl = {
	ASC: "ASC",
	DESC: "DESC"
}, ll = {
	TABLE: "TABLE",
	KANBAN: "KANBAN",
	CALENDAR: "CALENDAR",
	FIELDS_WIDGET: "FIELDS_WIDGET",
	TABLE_WIDGET: "TABLE_WIDGET"
}, ul = { INDEX: "INDEX" }, dl = {
	SIDE_PANEL: "SIDE_PANEL",
	RECORD_PAGE: "RECORD_PAGE"
}, fl = {
	DAY: "DAY",
	WEEK: "WEEK",
	MONTH: "MONTH"
}, pl = {
	WORKSPACE: "WORKSPACE",
	UNLISTED: "UNLISTED"
}, ml = {
	ONGOING_CREATION: "ONGOING_CREATION",
	PENDING_CREATION: "PENDING_CREATION",
	ACTIVE: "ACTIVE",
	INACTIVE: "INACTIVE",
	SUSPENDED: "SUSPENDED"
}, hl = {
	PLAN_REQUIRED: "PLAN_REQUIRED",
	WORKSPACE_ACTIVATION: "WORKSPACE_ACTIVATION",
	PROFILE_CREATION: "PROFILE_CREATION",
	SYNC_EMAIL: "SYNC_EMAIL",
	INVITE_TEAM: "INVITE_TEAM",
	BOOK_ONBOARDING: "BOOK_ONBOARDING",
	COMPLETED: "COMPLETED"
}, gl = {
	VIEW: "VIEW",
	IFRAME: "IFRAME",
	FIELD: "FIELD",
	FIELDS: "FIELDS",
	GRAPH: "GRAPH",
	STANDALONE_RICH_TEXT: "STANDALONE_RICH_TEXT",
	TIMELINE: "TIMELINE",
	TASKS: "TASKS",
	NOTES: "NOTES",
	FILES: "FILES",
	EMAILS: "EMAILS",
	CALENDAR: "CALENDAR",
	FIELD_RICH_TEXT: "FIELD_RICH_TEXT",
	WORKFLOW: "WORKFLOW",
	WORKFLOW_VERSION: "WORKFLOW_VERSION",
	WORKFLOW_RUN: "WORKFLOW_RUN",
	FRONT_COMPONENT: "FRONT_COMPONENT",
	RECORD_TABLE: "RECORD_TABLE",
	EMAIL_THREAD: "EMAIL_THREAD"
}, _l = {
	GRID: "GRID",
	VERTICAL_LIST: "VERTICAL_LIST",
	CANVAS: "CANVAS"
}, vl = {
	AGGREGATE_CHART: "AGGREGATE_CHART",
	PIE_CHART: "PIE_CHART",
	BAR_CHART: "BAR_CHART",
	LINE_CHART: "LINE_CHART",
	IFRAME: "IFRAME",
	STANDALONE_RICH_TEXT: "STANDALONE_RICH_TEXT",
	VIEW: "VIEW",
	FIELD: "FIELD",
	FIELDS: "FIELDS",
	TIMELINE: "TIMELINE",
	TASKS: "TASKS",
	NOTES: "NOTES",
	FILES: "FILES",
	EMAILS: "EMAILS",
	CALENDAR: "CALENDAR",
	FIELD_RICH_TEXT: "FIELD_RICH_TEXT",
	WORKFLOW: "WORKFLOW",
	WORKFLOW_VERSION: "WORKFLOW_VERSION",
	WORKFLOW_RUN: "WORKFLOW_RUN",
	FRONT_COMPONENT: "FRONT_COMPONENT",
	RECORD_TABLE: "RECORD_TABLE",
	EMAIL_THREAD: "EMAIL_THREAD"
}, yl = {
	SHORT: "SHORT",
	FULL: "FULL"
}, bl = {
	DAY: "DAY",
	MONTH: "MONTH",
	QUARTER: "QUARTER",
	YEAR: "YEAR",
	WEEK: "WEEK",
	DAY_OF_THE_WEEK: "DAY_OF_THE_WEEK",
	MONTH_OF_THE_YEAR: "MONTH_OF_THE_YEAR",
	QUARTER_OF_THE_YEAR: "QUARTER_OF_THE_YEAR",
	NONE: "NONE"
}, xl = {
	FIELD_ASC: "FIELD_ASC",
	FIELD_DESC: "FIELD_DESC",
	FIELD_POSITION_ASC: "FIELD_POSITION_ASC",
	FIELD_POSITION_DESC: "FIELD_POSITION_DESC",
	VALUE_ASC: "VALUE_ASC",
	VALUE_DESC: "VALUE_DESC",
	MANUAL: "MANUAL"
}, Sl = {
	NONE: "NONE",
	X: "X",
	Y: "Y",
	BOTH: "BOTH"
}, Cl = {
	STACKED: "STACKED",
	GROUPED: "GROUPED"
}, wl = {
	VERTICAL: "VERTICAL",
	HORIZONTAL: "HORIZONTAL"
}, Tl = {
	CARD: "CARD",
	EDITOR: "EDITOR",
	FIELD: "FIELD",
	VIEW: "VIEW",
	TABLE: "TABLE"
}, El = {
	RECORD_INDEX: "RECORD_INDEX",
	RECORD_PAGE: "RECORD_PAGE",
	DASHBOARD: "DASHBOARD",
	STANDALONE_PAGE: "STANDALONE_PAGE"
}, Dl = {
	PRO: "PRO",
	ENTERPRISE: "ENTERPRISE"
}, Ol = {
	METERED: "METERED",
	LICENSED: "LICENSED"
}, kl = {
	BASE_PRODUCT: "BASE_PRODUCT",
	RESOURCE_CREDIT: "RESOURCE_CREDIT"
}, Al = {
	Month: "Month",
	Year: "Year"
}, jl = {
	Active: "Active",
	Canceled: "Canceled",
	Incomplete: "Incomplete",
	IncompleteExpired: "IncompleteExpired",
	PastDue: "PastDue",
	Paused: "Paused",
	Trialing: "Trialing",
	Unpaid: "Unpaid"
}, Ml = {
	VIEW: "VIEW",
	FOLDER: "FOLDER",
	LINK: "LINK",
	OBJECT: "OBJECT",
	RECORD: "RECORD",
	PAGE_LAYOUT: "PAGE_LAYOUT"
}, Nl = {
	CREATED: "CREATED",
	UPDATED: "UPDATED",
	DELETED: "DELETED"
}, Pl = {
	CREATED: "CREATED",
	UPDATED: "UPDATED",
	DELETED: "DELETED",
	DESTROYED: "DESTROYED",
	RESTORED: "RESTORED",
	UPSERTED: "UPSERTED"
}, Fl = {
	IDLE: "IDLE",
	SUCCESS: "SUCCESS",
	ERROR: "ERROR"
}, Il = {
	IS_UNIQUE_INDEXES_ENABLED: "IS_UNIQUE_INDEXES_ENABLED",
	IS_JSON_FILTER_ENABLED: "IS_JSON_FILTER_ENABLED",
	IS_MARKETPLACE_SETTING_TAB_VISIBLE: "IS_MARKETPLACE_SETTING_TAB_VISIBLE",
	IS_EMAIL_GROUP_ENABLED: "IS_EMAIL_GROUP_ENABLED",
	IS_JUNCTION_RELATIONS_ENABLED: "IS_JUNCTION_RELATIONS_ENABLED",
	IS_REST_METADATA_API_NEW_FORMAT_DIRECT: "IS_REST_METADATA_API_NEW_FORMAT_DIRECT",
	IS_LOGIC_FUNCTION_PREBUILT_MODE_ENABLED: "IS_LOGIC_FUNCTION_PREBUILT_MODE_ENABLED",
	IS_SETTINGS_DISCOVERY_HERO_ENABLED: "IS_SETTINGS_DISCOVERY_HERO_ENABLED",
	IS_CALL_RECORDING_ENABLED: "IS_CALL_RECORDING_ENABLED",
	IS_MESSAGING_CALENDAR_WEBHOOK_ENABLED: "IS_MESSAGING_CALENDAR_WEBHOOK_ENABLED"
}, Ll = {
	OIDC: "OIDC",
	SAML: "SAML"
}, Rl = {
	Active: "Active",
	Inactive: "Inactive",
	Error: "Error"
}, zl = {
	GPT: "GPT",
	CLAUDE: "CLAUDE",
	GEMINI: "GEMINI",
	MISTRAL: "MISTRAL",
	GROK: "GROK"
}, Bl = {
	NONE: "NONE",
	FRONT: "FRONT"
}, Vl = {
	GOOGLE_RECAPTCHA: "GOOGLE_RECAPTCHA",
	TURNSTILE: "TURNSTILE"
}, Hl = {
	ONE_TO_MANY: "ONE_TO_MANY",
	MANY_TO_ONE: "MANY_TO_ONE"
}, Ul = {
	SSO: "SSO",
	CUSTOM_DOMAIN: "CUSTOM_DOMAIN",
	RLS: "RLS",
	AUDIT_LOGS: "AUDIT_LOGS"
}, Wl = {
	NONE: "NONE",
	STARTTLS: "STARTTLS",
	SSL_TLS: "SSL_TLS"
}, Gl = {
	PENDING: "PENDING",
	VERIFIED: "VERIFIED",
	FAILED: "FAILED",
	TEMPORARY_FAILURE: "TEMPORARY_FAILURE"
}, Kl = {
	METADATA: "METADATA",
	SUBJECT: "SUBJECT",
	SHARE_EVERYTHING: "SHARE_EVERYTHING"
}, ql = {
	EMAIL: "EMAIL",
	SMS: "SMS",
	EMAIL_GROUP: "EMAIL_GROUP"
}, Jl = {
	SENT_AND_RECEIVED: "SENT_AND_RECEIVED",
	SENT: "SENT",
	NONE: "NONE"
}, Yl = {
	ALL_FOLDERS: "ALL_FOLDERS",
	SELECTED_FOLDERS: "SELECTED_FOLDERS"
}, Xl = {
	GROUP_EMAILS_DELETION: "GROUP_EMAILS_DELETION",
	GROUP_EMAILS_IMPORT: "GROUP_EMAILS_IMPORT",
	NONE: "NONE"
}, Zl = {
	NOT_SYNCED: "NOT_SYNCED",
	ONGOING: "ONGOING",
	ACTIVE: "ACTIVE",
	FAILED_INSUFFICIENT_PERMISSIONS: "FAILED_INSUFFICIENT_PERMISSIONS",
	FAILED_UNKNOWN: "FAILED_UNKNOWN"
}, Ql = {
	PENDING_CONFIGURATION: "PENDING_CONFIGURATION",
	MESSAGE_LIST_FETCH_PENDING: "MESSAGE_LIST_FETCH_PENDING",
	MESSAGE_LIST_FETCH_SCHEDULED: "MESSAGE_LIST_FETCH_SCHEDULED",
	MESSAGE_LIST_FETCH_ONGOING: "MESSAGE_LIST_FETCH_ONGOING",
	MESSAGES_IMPORT_PENDING: "MESSAGES_IMPORT_PENDING",
	MESSAGES_IMPORT_SCHEDULED: "MESSAGES_IMPORT_SCHEDULED",
	MESSAGES_IMPORT_ONGOING: "MESSAGES_IMPORT_ONGOING",
	FAILED: "FAILED"
}, $l = {
	PUBLIC: "PUBLIC",
	PRIVATE: "PRIVATE"
}, eu = {
	NOT_SYNCED: "NOT_SYNCED",
	ONGOING: "ONGOING",
	ACTIVE: "ACTIVE",
	FAILED_INSUFFICIENT_PERMISSIONS: "FAILED_INSUFFICIENT_PERMISSIONS",
	FAILED_UNKNOWN: "FAILED_UNKNOWN"
}, tu = {
	PENDING_CONFIGURATION: "PENDING_CONFIGURATION",
	CALENDAR_EVENT_LIST_FETCH_PENDING: "CALENDAR_EVENT_LIST_FETCH_PENDING",
	CALENDAR_EVENT_LIST_FETCH_SCHEDULED: "CALENDAR_EVENT_LIST_FETCH_SCHEDULED",
	CALENDAR_EVENT_LIST_FETCH_ONGOING: "CALENDAR_EVENT_LIST_FETCH_ONGOING",
	CALENDAR_EVENTS_IMPORT_PENDING: "CALENDAR_EVENTS_IMPORT_PENDING",
	CALENDAR_EVENTS_IMPORT_SCHEDULED: "CALENDAR_EVENTS_IMPORT_SCHEDULED",
	CALENDAR_EVENTS_IMPORT_ONGOING: "CALENDAR_EVENTS_IMPORT_ONGOING",
	FAILED: "FAILED"
}, nu = {
	METADATA: "METADATA",
	SHARE_EVERYTHING: "SHARE_EVERYTHING"
}, ru = {
	AS_PARTICIPANT_AND_ORGANIZER: "AS_PARTICIPANT_AND_ORGANIZER",
	AS_PARTICIPANT: "AS_PARTICIPANT",
	AS_ORGANIZER: "AS_ORGANIZER",
	NONE: "NONE"
}, iu = {
	FOLDER_DELETION: "FOLDER_DELETION",
	FOLDER_IMPORT: "FOLDER_IMPORT",
	NONE: "NONE"
}, au = {
	fieldMetadata: "fieldMetadata",
	objectMetadata: "objectMetadata",
	view: "view",
	viewField: "viewField",
	viewFieldGroup: "viewFieldGroup",
	viewGroup: "viewGroup",
	viewSort: "viewSort",
	rowLevelPermissionPredicate: "rowLevelPermissionPredicate",
	rowLevelPermissionPredicateGroup: "rowLevelPermissionPredicateGroup",
	viewFilterGroup: "viewFilterGroup",
	index: "index",
	logicFunction: "logicFunction",
	viewFilter: "viewFilter",
	role: "role",
	roleTarget: "roleTarget",
	agent: "agent",
	skill: "skill",
	pageLayout: "pageLayout",
	pageLayoutWidget: "pageLayoutWidget",
	pageLayoutTab: "pageLayoutTab",
	commandMenuItem: "commandMenuItem",
	navigationMenuItem: "navigationMenuItem",
	rolePermissionFlag: "rolePermissionFlag",
	permissionFlag: "permissionFlag",
	objectPermission: "objectPermission",
	fieldPermission: "fieldPermission",
	frontComponent: "frontComponent",
	webhook: "webhook",
	applicationVariable: "applicationVariable",
	connectionProvider: "connectionProvider",
	searchFieldMetadata: "searchFieldMetadata"
}, ou = {
	WORKSPACE_EVENT: "WORKSPACE_EVENT",
	PAGEVIEW: "PAGEVIEW",
	OBJECT_EVENT: "OBJECT_EVENT",
	USAGE_EVENT: "USAGE_EVENT",
	APPLICATION_LOG: "APPLICATION_LOG"
}, su = {
	AI_CHAT_TOKEN: "AI_CHAT_TOKEN",
	AI_WORKFLOW_TOKEN: "AI_WORKFLOW_TOKEN",
	WORKFLOW_EXECUTION: "WORKFLOW_EXECUTION",
	CODE_EXECUTION: "CODE_EXECUTION",
	WEB_SEARCH: "WEB_SEARCH",
	CALL_RECORDING: "CALL_RECORDING"
}, cu = {
	delete: "delete",
	create: "create",
	update: "update"
}, lu = {
	PAGEVIEW: "PAGEVIEW",
	TRACK: "TRACK"
}, uu = {
	ProfilePicture: "ProfilePicture",
	WorkspaceLogo: "WorkspaceLogo",
	Attachment: "Attachment",
	PersonPicture: "PersonPicture",
	CorePicture: "CorePicture",
	File: "File",
	AgentChat: "AgentChat",
	BuiltLogicFunction: "BuiltLogicFunction",
	BuiltFrontComponent: "BuiltFrontComponent",
	PublicAsset: "PublicAsset",
	Source: "Source",
	FilesField: "FilesField",
	Dependencies: "Dependencies",
	Workflow: "Workflow",
	EmailAttachment: "EmailAttachment",
	AppTarball: "AppTarball",
	GeneratedSdkClient: "GeneratedSdkClient"
}, X = te(re), du = function(e) {
	return ee({
		url: void 0,
		...e,
		queryRoot: X.Query,
		mutationRoot: X.Mutation,
		subscriptionRoot: X.Subscription
	});
}, Z = "TWENTY_APP_ACCESS_TOKEN", fu = "TWENTY_API_KEY", Q = () => globalThis.process?.env ?? {}, $ = (e) => {
	if (typeof e != "string") return null;
	let t = e.trim();
	return t.length === 0 || t === "Bearer" ? null : t.startsWith("Bearer ") ? t.slice(7).trim() : t;
}, pu = (e) => {
	if (!e) return null;
	if (e instanceof Headers) return $(e.get("Authorization") ?? void 0);
	if (Array.isArray(e)) return $(e.find(([e]) => e.toLowerCase() === "authorization")?.[1]);
	let t = e;
	return $(t.Authorization ?? t.authorization);
}, mu = (e) => e?.errors ? e.errors.some((e) => e.extensions?.code === "UNAUTHENTICATED" || e.message?.toLowerCase() === "unauthorized") : !1, hu = {
	url: `${process.env.TWENTY_API_URL}/metadata`,
	headers: { "Content-Type": "application/json" }
}, gu = class {
	constructor(e) {
		this.refreshAccessTokenPromise = null;
		let t = {
			...hu,
			...e
		}, { url: n, headers: r, fetch: i, fetcher: a, batch: o, ...s } = t;
		this.url = n ?? "", this.requestOptions = s, this.headers = r ?? {}, this.fetchImplementation = i ?? globalThis.fetch ?? null;
		let c = Q(), l = pu(typeof r == "function" ? void 0 : r);
		this.authorizationToken = l ?? c[Z] ?? c[fu] ?? null, this.client = du({
			...t,
			headers: void 0,
			fetcher: async (e) => this.executeGraphqlRequestWithOptionalRefresh({ operation: e })
		});
	}
	query(e) {
		return this.client.query(e);
	}
	mutation(e) {
		return this.client.mutation(e);
	}
	async uploadFile(e, n, r = "application/octet-stream", i) {
		let a = new FormData();
		a.append("operations", JSON.stringify({
			query: "mutation UploadFilesFieldFileByUniversalIdentifier($file: Upload!, $fieldMetadataUniversalIdentifier: String!) {\n        uploadFilesFieldFileByUniversalIdentifier(file: $file, fieldMetadataUniversalIdentifier: $fieldMetadataUniversalIdentifier) { id path size createdAt url }\n      }",
			variables: {
				file: null,
				fieldMetadataUniversalIdentifier: i
			}
		})), a.append("map", JSON.stringify({ 0: ["variables.file"] })), a.append("0", new Blob([e], { type: r }), n);
		let o = await this.executeGraphqlRequestWithOptionalRefresh({
			operation: a,
			headers: {},
			requestInit: { method: "POST" }
		});
		if (o.errors) throw new t(o.errors, o.data);
		return o.data.uploadFilesFieldFileByUniversalIdentifier;
	}
	async executeGraphqlRequestWithOptionalRefresh({ operation: e, headers: t, requestInit: n }) {
		let r = await this.executeGraphqlRequest({
			operation: e,
			headers: t,
			requestInit: n,
			token: this.authorizationToken
		});
		if (this.shouldRefreshToken(r)) {
			let r = await this.requestRefreshedAccessToken();
			if (r) {
				let i = await this.executeGraphqlRequest({
					operation: e,
					headers: t,
					requestInit: n,
					token: r
				});
				return this.assertResponseIsSuccessful(i);
			}
		}
		return this.assertResponseIsSuccessful(r);
	}
	async executeGraphqlRequest({ operation: e, headers: t, requestInit: n, token: r }) {
		if (!this.fetchImplementation) throw Error("Global `fetch` function is not available, pass a fetch implementation to the Twenty client");
		let i = await this.resolveHeaders(), a = new Headers(i);
		t && new Headers(t).forEach((e, t) => a.set(t, e)), e instanceof FormData ? a.delete("Content-Type") : a.set("Content-Type", "application/json"), r ? a.set("Authorization", `Bearer ${r}`) : a.delete("Authorization");
		let o = await this.fetchImplementation.call(globalThis, this.url, {
			...this.requestOptions,
			...n,
			method: n?.method ?? "POST",
			headers: a,
			body: e instanceof FormData ? e : JSON.stringify(e)
		}), s = await o.text(), c = null;
		if (s.trim().length > 0) try {
			c = JSON.parse(s);
		} catch {
			c = null;
		}
		return {
			status: o.status,
			statusText: o.statusText,
			payload: c,
			rawBody: s
		};
	}
	async resolveHeaders() {
		return typeof this.headers == "function" ? await this.headers() ?? {} : this.headers ?? {};
	}
	shouldRefreshToken(e) {
		return e.status === 401 ? !0 : mu(e.payload);
	}
	assertResponseIsSuccessful(e) {
		if (e.status < 200 || e.status >= 300) throw Error(`${e.statusText}: ${e.rawBody}`);
		if (e.payload === null) throw Error("Invalid JSON response");
		return e.payload;
	}
	async requestRefreshedAccessToken() {
		let e = globalThis.frontComponentHostCommunicationApi?.requestAccessTokenRefresh;
		return typeof e == "function" ? (this.refreshAccessTokenPromise ||= e().then((e) => typeof e != "string" || e.length === 0 ? null : (this.setAuthorizationToken(e), e)).catch((e) => (console.error("Twenty client: token refresh failed", e), null)).finally(() => {
			this.refreshAccessTokenPromise = null;
		}), this.refreshAccessTokenPromise) : null;
	}
	setAuthorizationToken(e) {
		this.authorizationToken = e;
		let t = Q();
		t[Z] = e;
	}
};
//#endregion
export { gu as MetadataApiClient, ie as MetadataSchema };
